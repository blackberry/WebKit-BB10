...
static const size_t bytesWorthGCUnderMemoryPressure = 1 * 1024 * 1024;
...
void DefaultGCActivityCallback::didReportExtraMemoryCost(unsigned bytesAllocated)
{
    if (!BlackBerry::Platform::isMemoryLow())
        return;

    if (bytesAllocated < bytesWorthGCUnderMemoryPressure)
        return;

    Heap& heap = m_vm->heap;

    if (heap.isBusy() || !heap.isSafeToCollect())
        return;
