...
static JSCell* formatLocaleNumber(ExecState* exec, JSValue v)
{
    ...
    unum_setAttribute(nf, UNUM_MAX_SIGNIFICANT_DIGITS, WTF::NumberToStringBufferLength);
    unum_setAttribute(nf, UNUM_SIGNIFICANT_DIGITS_USED, 1);

    ASSERT(v.isNumber());
    UChar buffer[WTF::NumberToStringBufferLength];
    int32_t length;

    if (v.isInt32()) {
        int32_t num = v.asInt32();
        length = unum_format(nf, num, buffer, WTF::NumberToStringBufferLength, 0, &status);
    } else {
        double num;
        if (v.isDouble())
            num = v.asDouble();
        ...
        DecimalNumber d(num);
        char numberString[WTF::NumberToStringBufferLength];
        length = d.toStringDecimal(numberString, WTF::NumberToStringBufferLength);
...
static inline JSCell* formatLocaleNumber(ExecState* exec, JSValue v)
{
...
EncodedJSValue JSC_HOST_CALL numberProtoFuncToLocaleString(ExecState* exec)
{
    ...
    return JSValue::encode(formatLocaleNumber(exec, jsNumber(x)));
}
