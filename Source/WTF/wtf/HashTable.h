...
#define CHECK_HASHTABLE_USE_AFTER_DESTRUCTION 1
    ...
    public:
        ...
        void reserveIfEmpty(int items)
        {
            ASSERT(items >= 0);
            if (items <= 0)
                return;
            ASSERT(!m_keyCount);
            int newCapacity = m_tableSize;
            if (!newCapacity)
                newCapacity = KeyTraits::minimumTableSize;
            int minCapacity = items * m_maxLoad;
            while (minCapacity > newCapacity)
                newCapacity *= 2;
            if (newCapacity > m_tableSize)
                rehash(newCapacity);
        }
    ...
    private:
        ...
        bool shouldShrink() const { return m_deletedCount && m_keyCount * m_minLoad < m_tableSize && m_tableSize > KeyTraits::minimumTableSize; }
        void expand();
