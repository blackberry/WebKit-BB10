...
BatteryManager::BatteryManager(Navigator* navigator)
    ...
    , m_batteryStatus(0)
{
    if (BatteryController* controller = batteryController())
...
BatteryController* BatteryManager::batteryController() const
{
    BatteryController* controller = 0;
    if (m_frame && m_frame->page())
        controller = BatteryController::from(m_frame->page());
    return controller;
}
