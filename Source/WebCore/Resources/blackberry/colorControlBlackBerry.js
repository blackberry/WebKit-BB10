...
var rangeUpdateColor = function (output, input) {
    ...
    var range = input.type === 'range' ? input : output;
...
var calculateSwatchBlockPadding = function () {
    var swatchBlock = document.getElementById('color-picker-swatch-block'),
        padding = Math.max(0, (swatchBlock.clientWidth - 4 * COLOR_WELL_WIDTH) / 2) + 'px';
...
var openSwatches = function () {
    ...
    var midpoint = document.getElementById('switcher').offsetWidth / 2;
    document.getElementById('slide-button').style.webkitTransform = 'translate3d(' + midpoint + 'px, 0, 0)';
...
var touchMoveHandler = function (evt) {
    ...
    var button = document.getElementById('slide-button'),
        offset = button.translation + _dragDelta,
        midpoint = document.getElementById('switcher').offsetWidth / 2;
    button.style.webkitTransform = 'translate3d(' + Math.max(0, Math.min(offset, midpoint)) + 'px, 0, 0)';
}
...
var ok = function () {
    hide(document.getElementById('color-picker-hex-input').value);
};
...
var createRange = function (color) {
    ...
    range.setAttribute('aria-valuemax', '255');
...
var layout = function (pickerParams) {
    ...
    document.body.classList.add('hidden');
    popup.className = 'popup-area color-picker-popup-area half-hidden';
    popup.id = 'popup-area';
    ...
    sliders.setAttribute('role', 'button');
    ...
    var SWATCH_COLORS = [[
        {hex: '#ffffff', name: pickerParams.uiText.whiteSwatchLabel},
        {hex: '#ffff00', name: pickerParams.uiText.yellowSwatchLabel},
        {hex: '#00ff00', name: pickerParams.uiText.limeSwatchLabel},
        {hex: '#00ffff', name: pickerParams.uiText.aquaSwatchLabel}], [
        {hex: '#ff00ff', name: pickerParams.uiText.fuchsiaSwatchLabel},
        {hex: '#0000ff', name: pickerParams.uiText.blueSwatchLabel},
        {hex: '#ff0000', name: pickerParams.uiText.redSwatchLabel},
        {hex: '#000080', name: pickerParams.uiText.navySwatchLabel}], [
        {hex: '#008080', name: pickerParams.uiText.tealSwatchLabel},
        {hex: '#008000', name: pickerParams.uiText.greenSwatchLabel},
        {hex: '#800080', name: pickerParams.uiText.purpleSwatchLabel},
        {hex: '#800000', name: pickerParams.uiText.maroonSwatchLabel}], [
        {hex: '#808000', name: pickerParams.uiText.oliveSwatchLabel},
        {hex: '#808080', name: pickerParams.uiText.graySwatchLabel},
        {hex: '#c0c0c0', name: pickerParams.uiText.silverSwatchLabel},
        {hex: '#000000', name: pickerParams.uiText.blackSwatchLabel}]];
    ...
    for (var row = 1; row <= 4; row++) {
        ...
        for (var col = 1; col <= 4; col++) {
            ...
            cell.id = row + '' + col;
            cell.style.backgroundColor = SWATCH_COLORS[row - 1][col - 1].hex;
            cell.mycolor = cutHex(SWATCH_COLORS[row - 1][col - 1].hex);
            cell.setAttribute('aria-label', SWATCH_COLORS[row - 1][col - 1].name);
    ...
    userInput.setAttribute('aria-label', pickerParams.uiText.hexInputLabel);
    ...
    redRange.setAttribute('aria-label', pickerParams.uiText.redRangeLabel);
    redSliderRow.appendChild(redRange);

    var redOutput = createOutput('red');
    redOutput.setAttribute('aria-label', pickerParams.uiText.redInputLabel);
    ...
    greenRange.setAttribute('aria-label', pickerParams.uiText.greenRangeLabel);
    greenSliderRow.appendChild(greenRange);

    var greenOutput = createOutput('green');
    greenOutput.setAttribute('aria-label', pickerParams.uiText.greenInputLabel);
    ...
    blueRange.setAttribute('aria-label', pickerParams.uiText.blueRangeLabel);
    blueSliderRow.appendChild(blueRange);

    var blueOutput = createOutput('blue');
    blueOutput.setAttribute('aria-label', pickerParams.uiText.blueInputLabel);
