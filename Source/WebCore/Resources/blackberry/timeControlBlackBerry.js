...
if (screen.height <= 768) {
    ITEM_HEIGHT = 91;
} else if ((screen.height == 1920 || screen.width == 1080) || (screen.height == 1080 && screen.width == 1920) || screen.height == 1440) {
    ITEM_HEIGHT = 110;
}
...
var hide = function (result) {
    document.getElementById('popup-area').classList.add('half-hidden');
...
function adjustColumnLabelsForValue(col) {
    ...
    selected.setAttribute('aria-label', selected.innerText + '. ' + _ariaHintText);
}
...
function show(pickerParams) {
    ...
    _ariaHintText = pickerParams.uiText.ariaHintText;
