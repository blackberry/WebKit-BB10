...
PassRefPtr<Document> DOMImplementation::createDocument(const String& type, Frame* frame, const KURL& url, bool inViewSourceMode)
{
    ...
    // Certain mime types should always use a plugin document so that the
    // "missing plugin" icon gets painted, etc.
    if ((pluginData && pluginData->supportsMimeType(type)) || (frame && frame->loader()->client()->shouldUsePluginDocument(type)))
