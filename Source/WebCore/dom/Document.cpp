...
void Document::processCursorEventMode(const String& features)
{
    ASSERT(!features.isNull());
    ...
    CursorEventMode mode;
    if (equalIgnoringCase(features, "processed"))
        mode = ProcessedCursorEvents;
    else if (equalIgnoringCase(features, "native"))
        mode = NativeCursorEvents;
    else {
        // FIXME: log an error
        return;
    }

    frame->page()->chrome()->client()->didReceiveCursorEventMode(frame, mode);
}

void Document::processTouchEventMode(const String& features)
{
    ASSERT(!features.isNull());
    ...
    TouchEventMode mode;
    if (equalIgnoringCase(features, "processed"))
        mode = ProcessedTouchEvents;
    else if (equalIgnoringCase(features, "native"))
        mode = NativeTouchEvents;
    else if (equalIgnoringCase(features, "pure-with-mouse-conversion"))
        mode = PureTouchEventsWithMouseConversion;
    else {
        // FIXME: log an error
        return;
    }

    frame->page()->chrome()->client()->didReceiveTouchEventMode(frame, mode);
}
#endif
...
#if PLATFORM(BLACKBERRY)
    setSecurityOrigin(isSandboxed(SandboxOrigin) ? SecurityOrigin::createUnique() : m_frame->loader()->client()->securityOriginForNewDocument(m_url));
#else
