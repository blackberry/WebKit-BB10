...
void DocumentMarkerController::addMarker(Range* range, DocumentMarker::MarkerType type, const String& description, const AttributeTextStyle& attributeTextStyle)
{
    ...
    for (TextIterator markedText(range); !markedText.atEnd(); markedText.advance()) {
        ...
        addMarker(textPiece->startContainer(exception),
                  DocumentMarker(type, textPiece->startOffset(exception), textPiece->endOffset(exception), description, attributeTextStyle));
    }
}
...
void DocumentMarkerController::addMarker(Range* range, DocumentMarker::MarkerType type, const String& description)
{
    ...
    for (TextIterator markedText(range); !markedText.atEnd(); markedText.advance()) {
        RefPtr<Range> textPiece = markedText.range();
        addMarker(textPiece->startContainer(), DocumentMarker(type, textPiece->startOffset(), textPiece->endOffset(), description, AttributeTextStyle()));
    }
}
    ...
    if (!list) {
        ...
        for (i = 0; i < numMarkers; ++i) {
            ...
            if (marker.startOffset() > toInsert.startOffset())
                break;
            if (marker.typesMatch(toInsert) && marker.endOffset() >= toInsert.startOffset()) {
