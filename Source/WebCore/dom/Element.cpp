...
class CheckForVisibilityChangeOnRecalcStyle {
    public:
        CheckForVisibilityChangeOnRecalcStyle(Element *element, RenderStyle* currentStyle, ChromeClient* client)
            : m_element(element)
            , m_previousDisplay(currentStyle ? currentStyle->display() : NONE)
            , m_previousVisibility(currentStyle ? currentStyle->visibility() : HIDDEN)
            , m_previousImplicitVisibility(m_element->implicitVisibility())
            , m_client(client)
        { }
        ...
        ~CheckForVisibilityChangeOnRecalcStyle()
        {
            ...
            if ((m_previousDisplay == NONE && m_element->renderStyle()->display() != NONE)
                || (m_previousVisibility == HIDDEN && m_element->renderStyle()->visibility() != HIDDEN)
                || (m_previousImplicitVisibility == HIDDEN && m_element->implicitVisibility() != HIDDEN))
                m_client->notifyVisibilityChange(ChromeClient::VisibilityChange);
        }
...
#if PLATFORM(BLACKBERRY)
    CheckForVisibilityChangeOnRecalcStyle checkForVisibilityChange(this, currentStyle.get(), document()->page()->chrome()->client());
#endif
