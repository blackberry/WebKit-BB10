...
PassRefPtr<MouseEvent> MouseEvent::create(const AtomicString& eventType, PassRefPtr<AbstractView> view, const PlatformMouseEvent& event, int detail, PassRefPtr<Node> relatedTarget)
{
    ...
    // To support websites that rely on mousemove events, we should be able to preventDefault such an event.
    // This way, we would allow the client to understand that no scrolling should take place, and mouse move events
    // should get forward to the Web content instead.
    if (!isCancelable && event.inputMethod() == TouchScreen)
        isCancelable = true;
#endif
