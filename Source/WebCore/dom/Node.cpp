...
static void setHasMouseOrTouchMoveListenerRecursively(Node* node, bool flag)
{
    node->setSelfOrAncestorHasMouseOrTouchMoveListener(flag);
    node = node->firstChild();

    while (node) {
        if (node->selfOrAncestorHasMouseOrTouchMoveListener() == flag)
            return;
...
static inline bool tryAddEventListener(Node* targetNode, const AtomicString& eventType, PassRefPtr<EventListener> listener, bool useCapture)
{
    ...
    if (Document* document = targetNode->document()) {
        ...
        if (eventType == eventNames().touchmoveEvent
            ...
            setHasMouseOrTouchMoveListenerRecursively(targetNode, true);
    }
...
static inline bool tryRemoveEventListener(Node* targetNode, const AtomicString& eventType, EventListener* listener, bool useCapture)
{
    ...
    if (Document* document = targetNode->document()) {
        ...
        if (eventType == eventNames().touchmoveEvent
            || eventType == eventNames().mousemoveEvent)
            setHasMouseOrTouchMoveListenerRecursively(targetNode, false);
    }
