...
// FIXME: Temporary solution for PR# 283989 until https://bugs.webkit.org/show_bug.cgi?id=107954 is done.
static inline bool deferEventSendingIfNecessary(ImageLoader* loader, Document* document, ImageEventSender& eventSender)
{
    if (!document->activeDOMObjectsAreSuspended())
        return false;

    // Make sure the timer will still fire.
    // FIXME: Wait at least 500ms to avoid burning CPU.
    eventSender.dispatchEventSoon(loader, 0.5);
    return true;
}
#endif
...
void ImageLoader::dispatchPendingBeforeLoadEvent()
{
    ...
    if (!m_element->document()->attached())
        return;
#if PLATFORM(BLACKBERRY)
    if (deferEventSendingIfNecessary(this, m_element->document(), beforeLoadEventSender()))
        return;
#endif
...
#if PLATFORM(BLACKBERRY)
    if (deferEventSendingIfNecessary(this, m_element->document(), loadEventSender()))
        return;
#endif
...
#if PLATFORM(BLACKBERRY)
    if (deferEventSendingIfNecessary(this, m_element->document(), errorEventSender()))
        return;
#endif
