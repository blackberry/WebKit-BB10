...
void ImageLoader::dispatchPendingBeforeLoadEvent()
{
    ...
    if (!document()->attached())
        return;
#if PLATFORM(BLACKBERRY)
    if (deferEventSendingIfNecessary(this, document(), beforeLoadEventSender()))
        return;
#endif
...
#if PLATFORM(BLACKBERRY)
    if (deferEventSendingIfNecessary(this, document(), loadEventSender()))
        return;
#endif
...
#if PLATFORM(BLACKBERRY)
    if (deferEventSendingIfNecessary(this, document(), errorEventSender()))
        return;
#endif
