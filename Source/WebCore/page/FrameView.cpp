...
void FrameView::setContentsSize(const IntSize& size)
{
    ...
    if (!m_frame->loader()->isComplete())
        size_ = size.expandedTo(m_contentsSizeFromHistory);
#endif

    if (size_ == contentsSize())
        return;

    m_deferSetNeedsLayouts++;

    ScrollView::setContentsSize(size_);
...
void FrameView::checkRelayoutForContentsSizeFromHistory()
{
    ...
    // schedule a relayout
    if (!m_contentsSizeFromHistory.isEmpty() && LayoutSize(m_contentsSizeFromHistory) != root->maxLayoutOverflow())
        scheduleRelayout();
}
#endif
...
void FrameView::adjustViewSize()
{
    ...
    bool overflowExceedsContentsSize = root->rightAbsoluteVisibleOverflow() > root->layoutOverflowRect().maxX() ||
                                       root->bottomAbsoluteVisibleOverflow() > root->layoutOverflowRect().maxY();
...
{
// FIXME: The BlackBerry port just ignores static backgrounds for now, but the
...
#if PLATFORM(BLACKBERRY)
    // We let fixed be absolute in the horizontal dimension
    // to work better with our pinch zoom feature
    int x = 0;
    // FIXME remove the setting of y value once we clean up WebPage::viewportSize() vs WebPagePrivate::viewportSize() (PR 198761)
    int y = scrollPosition.y();
    if (y > maximumScrollPosition().y())
        y = maximumScrollPosition().y();
    if (y < 0)
        y = 0;
#else
...
void FrameView::addScrollableArea(ScrollableArea* scrollableArea)
{
    ...
    page->chrome()->scrollableAreasDidChange();
}
