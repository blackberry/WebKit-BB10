...
void FrameView::setContentsSize(const IntSize& size)
{
    ...
    if (RenderView* root = rootRenderer(this)) {
        if (root->usesCompositing())
            root->compositor()->frameViewContentsSizeDidChange();
    }
#endif
#endif
...
{
    // See https://bugs.webkit.org/show_bug.cgi?id=59204 for a discussion why the BlackBerry port differs
    // here. The upstream code doesn't make sense and this seems like the right thing to do. See also
    // ScrollView::minimumScrollPosition() and maximumScrollPosition().
    if (scrollPosition > maxValue - scrollOrigin)
        scrollPosition = maxValue - scrollOrigin;
    if (scrollPosition < -scrollOrigin)
        scrollPosition = -scrollOrigin;

    // The dragFactor can be 0 because the caller doesn't take scrollOrigin into account
...
IntSize FrameView::scrollOffsetForFixedPosition() const
{
    ...
    if (!fixedReportedSize().isEmpty())
        visibleContentRect.setSize(fixedReportedSize());
#endif
