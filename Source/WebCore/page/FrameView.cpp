...
void FrameView::adjustViewSize()
{
    ...
    // Note: This is required for BlackBerry's zoom on load algorithm, but I don't anticipate this
    // will be upstreamed anytime soon...
    bool overflowExceedsContentsSize = renderView->rightAbsoluteVisibleOverflow() > renderView->layoutOverflowRect().maxX() ||
                                       renderView->bottomAbsoluteVisibleOverflow() > renderView->layoutOverflowRect().maxY();
...
{
// The BlackBerry port now supports accelerated compositing for fixed background images
// by disabling FAST_MOBILE_SCROLLING and avoiding slow repaints
#if !PLATFORM(BLACKBERRY)
...
#if PLATFORM(BLACKBERRY)
void FrameView::scrollContentsSlowPath(const IntSize& delta, const IntRect& updateRect)
#else
...
#if PLATFORM(BLACKBERRY)
    ScrollView::scrollContentsSlowPath(delta, updateRect);
#else
...
#if USE(ACCELERATED_COMPOSITING) && !PLATFORM(BLACKBERRY)
...
#if ENABLE(FIXED_REPORTED_SIZE)
        // If there's a fixed reported size set, the frame is not considered to
        // have changed size when the layout size or zoom factor changes, but
        // instead resize events have to be injected by the caller of
        // ScrollView::setFixedReportedSize() when appropriate.
        if (!fixedReportedSize().isEmpty())
            resized = false;
#endif
        if (resized) {
...
#if PLATFORM(BLACKBERRY) && USE(ACCELERATED_COMPOSITING) && !ENABLE(FAST_MOBILE_SCROLLING)
    RenderObject* rootObject = renderView->document()->documentElement() ? renderView->document()->documentElement()->renderer() : 0;
    // Force compositing re-evaluation in case document element or its renderer do not exist yet
    if (!rootObject)
        renderView->compositor()->setShouldReevaluateCompositingAfterLayout();
    else {
        // Don't paint anything here if root background is fixed. The root layer
        // will be painted on its own backing. Exception is snapshot painting when
        // FlattenCompositingLayers flag is present
        if (renderView->rootBackgroundIsEntirelyFixed() && !(m_paintBehavior & PaintBehaviorFlattenCompositingLayers))
            return;
    }
#endif
...
bool FrameView::addScrollableArea(ScrollableArea* scrollableArea)
{
    ...
    bool isNewEntry = m_scrollableAreas->add(scrollableArea).isNewEntry;
...
bool FrameView::flowTextToFit(Node* node) const
{
    ...
    if (settings) {
        ...
        if (node && node->renderer()) {
            ...
            if (node->renderer()->isOutOfFlowPositioned())
                return false;
            ...
            while (obj) {
                ...
                // The table cell's declared line height less than the height of font metrics can potentially
                // cause text overlap if reflowed. Using lineSpacing() can still cause overlap in reality.
                if ((obj->style()->height().isFixed()
                    && !obj->isTable() && !obj->isTableCell() && !obj->isRenderTableCol()
                    && !obj->isTableRow() && !obj->isTableSection())
                    || (obj->isTableCell() && obj->style()->computedLineHeight() < obj->style()->fontMetrics().height())) {
                    doReflow = false;
                    break;
                }
