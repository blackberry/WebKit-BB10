...
const float scaleFactorThreshold = 1.75;
...
const float xPositionRatio = 0.25;
...
static bool isReadOnlySliderThumb(RenderObject* object)
{
    RenderSlider* slider = determineRenderSlider(object);
    if (slider && slider->node())
        return toElement(slider->node())->hasAttribute(HTMLNames::readonlyAttr);
    return false;
}
...
static float determineFullScreenMultiplier(Element* element)
{
    ...
    if (element && element->document()->webkitIsFullScreen() && element->document()->webkitCurrentFullScreenElement() == toParentMediaElement(element)) {
        if (Page* page = element->document()->page()) {
            ...
            float scaleFactor = element->document()->view()->hostWindow()->platformPageClient()->currentScale();
...
void RenderThemeBlackBerry::adjustSearchFieldCancelButtonStyle(StyleResolver*, RenderStyle* style, Element* e) const
{
    static const float defaultControlFontPixelSize = 10;
    static const float defaultCancelButtonSize = 13;
    static const float minCancelButtonSize = 5;

    DEFINE_STATIC_LOCAL(QualifiedName, bbShowCancel, (nullAtom, "data-blackberry-hide-cancel", nullAtom));

    // if we have data-blackberry-hide-cancel active, make the 'X' take no space
    if (e && e->shadowHost() && e->shadowHost()->fastGetAttribute(bbShowCancel) == "true") {
        Length length(0, Fixed);
...
bool RenderThemeBlackBerry::paintSearchFieldCancelButton(RenderObject* cancelButtonObject, const PaintInfo& paintInfo, const IntRect& r)
{
    ...
    // if we have data-blackberry-hide-cancel active, don't draw the 'X'
    if (cancelButtonObject->style()->visibility() == HIDDEN)
        return false;
...
void RenderThemeBlackBerry::adjustMenuListButtonStyle(StyleResolver*, RenderStyle* style, Element*) const
{
    ...
    style->setPaddingRight(Length(minHeight / 3 + paddingRight, Fixed));
...
static IntRect computeMenuListArrowButtonRect(const IntRect& rect)
{
    ...
    return IntRect(IntPoint(rect.maxX() - rect.height() / 2, rect.y()), IntSize(rect.height() / 3, rect.height()));
}
...
bool RenderThemeBlackBerry::paintMenuList(RenderObject* object, const PaintInfo& info, const IntRect& rect)
{
    ...
    float width = arrowButtonRectangle.width();
...
void RenderThemeBlackBerry::adjustMediaControlStyle(StyleResolver*, RenderStyle* style, Element* element) const
{
    ...
    float trackListFontSize = mediaControlsHeight / 2 * fullScreenMultiplier;
    ...
    case MediaToggleClosedCaptionsButtonPart:
        ...
        style->setHeight(controlsHeight);
        break;
    case MediaClosedCaptionsTrackListPart:
        style->setHeight(controlsHeight);
        style->setFontSize(static_cast<int>(trackListFontSize));
        break;
...
// We hi-jack the Overlay Play Button code to use it for the PlayOn Button
bool RenderThemeBlackBerry::paintMediaOverlayPlayButton(RenderObject* object, const PaintInfo& paintInfo, const IntRect& rect)
{
#if ENABLE(VIDEO)
    ...
    MediaPlayerPrivate* platformPlayer = static_cast<MediaPlayerPrivate*>(player->implementation());

    // Reject null pointers AND the null media player.
    if (!platformPlayer || platformPlayer->platformMedia().type != PlatformMedia::QNXMediaPlayerType)
        return false;

    static Image* playOnRemote = Image::loadPlatformResource("play_on").leakRef();
    static Image* playOnLocal = Image::loadPlatformResource("play_local").leakRef();
    static Image* playOnSelected = Image::loadPlatformResource("play_on_selected").leakRef();
    static Image* playOnDisabled = Image::loadPlatformResource("play_on_disabled").leakRef();
    static Image* playLocalSelected = Image::loadPlatformResource("play_local_selected").leakRef();
    Image* imageToUse;

    bool hasRemoteDisplays = platformPlayer->hasRemoteDisplays();
    if (!platformPlayer->isPresentationModeEnabled() || !hasRemoteDisplays) {
        // We are not in presentation mode. Show the Play On button image.
        if (!hasRemoteDisplays && !platformPlayer->isMiracastSupported())
            imageToUse = playOnDisabled;
        else if (isPressed(object) || isFocused(object))
            imageToUse = playOnSelected;
        else
            imageToUse = playOnRemote;
    } else {
        // We are in presentation mode. Show the Eject button image.
        if (isPressed(object) || isFocused(object))
            imageToUse = playLocalSelected;
        else
            imageToUse = playOnLocal;
    }
    return paintMediaButton(paintInfo.context, rect, imageToUse);
#else
    UNUSED_PARAM(object);
...
bool RenderThemeBlackBerry::paintMediaToggleClosedCaptionsButton(RenderObject* object, const PaintInfo& paintInfo, const IntRect& rect)
{
#if ENABLE(VIDEO)
    HTMLMediaElement* mediaElement = toParentMediaElement(object);
    ...
    static Image* ccIcon = Image::loadPlatformResource("ic_cc").leakRef();
    static Image* ccIconDisabled = Image::loadPlatformResource("ic_cc_disabled").leakRef();
    Image* imageToUse;

    if (isEnabled(object))
        imageToUse = ccIcon;
    else
        imageToUse = ccIconDisabled;
...
bool RenderThemeBlackBerry::paintMediaSliderThumb(RenderObject* object, const PaintInfo& paintInfo, const IntRect& rect)
{
    ...
    if (!isReadOnlySliderThumb(object) && (isPressed(object) || isHovered(object) || isFocused(object)))
...
{
    return Color(0, 168, 223, 127);
}

Color RenderThemeBlackBerry::platformInactiveSelectionBackgroundColor() const
{
    return Color(211, 211, 211, 127);
}
