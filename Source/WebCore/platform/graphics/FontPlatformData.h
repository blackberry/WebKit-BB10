...
#if (PLATFORM(CHROMIUM) && OS(DARWIN)) || (PLATFORM(BLACKBERRY) && USE(HARFBUZZ))
class HarfBuzzFace;
#endif
...
public:
...
#endif
#if PLATFORM(BLACKBERRY)
    FontPlatformData(char* name, float size, bool syntheticBold, bool syntheticOblique, FontOrientation orientation = Horizontal,
...
#if PLATFORM(BLACKBERRY)
    const ITypeState& font() const;

    void setFakeBold(bool);
    void setFakeItalic(bool);
#endif
    ...
    const void* platformFontHandle() const;
#if !USE(HARFBUZZ)
    ...
    unsigned hash() const
    {
        ...
        if (!m_hash && m_font.isValid()) {
            // The font size is not worked into the hash because the platform uses
            // distance fields that can be rendered beautifully at any scale.
            uintptr_t hashCodes[3] = { CaseFoldingHash::hash(m_name, strlen(m_name)), m_widthVariant, static_cast<uintptr_t>(m_orientation << 2 | m_syntheticBold << 1 | m_syntheticOblique) };
            m_hash = StringHasher::hashMemory<sizeof(hashCodes)>(hashCodes);
        }
        return m_hash;
#endif
    }
...
#if PLATFORM(BLACKBERRY)
    static ITypeState hashTableDeletedFontValue() { return ITypeState::deletedValue(); }
#endif
...
#elif PLATFORM(BLACKBERRY)
    bool applyState(const ITypeState& font, float scale = 1.0) const;
    ITypeState m_font; // a font at a particular ppem size
    char* m_name;
    mutable float m_scale;
    mutable unsigned m_hash;
#if !USE(HARFBUZZ)
    RefPtr<WorldTypeFace> m_worldtypeFace;
#endif
#endif
