...
#if (PLATFORM(CHROMIUM) && OS(DARWIN)) || (PLATFORM(BLACKBERRY) && USE(HARFBUZZ_NG))
...
public:
...
#if PLATFORM(BLACKBERRY)
    FS_STATE* font() const { return m_font; }
    FS_STATE* scaledFont(float scale) const;
    ...
    unsigned hash() const
    {
        ...
        return m_font ? PtrHash<const void*>::hash(platformFontHandle()) : 0;
#endif
    }
...
#if PLATFORM(BLACKBERRY)
    static FS_STATE* hashTableDeletedFontValue() { return reinterpret_cast<FS_STATE*>(-1); }
#endif
...
#elif PLATFORM(BLACKBERRY)
    bool applyState(FS_STATE* font, float scale = 1.0) const;
    FS_STATE* m_font; // a font at a particular ppem size
    char* m_name;
    mutable FS_STATE* m_scaledFont; // a cache for m_font with a particular zoom scale
