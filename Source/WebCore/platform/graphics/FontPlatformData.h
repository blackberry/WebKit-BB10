...
#if (PLATFORM(CHROMIUM) && !OS(DARWIN)) || (PLATFORM(BLACKBERRY) && USE(SKIA))
...
class MonotypeFont : public RefCounted<MonotypeFont> {
public:
    static PassRefPtr<MonotypeFont> create(TsFont* font, const TsFontParam& param)
    {
        return adoptRef(new MonotypeFont(font, param));
    }

    TsFontStyle* scaledFont(float scale)
    {
        if (m_scale == scale) return m_scaledFont;

        TsFixed size = m_param.size;
        m_param.size = static_cast<TsFixed>(size * scale);
        m_scaledFont = TsFontStyleMgr_getFontStyle(BlackBerry::Platform::Graphics::getFontStyleMgr(), m_font, &m_param);
        m_scale = scale;
        m_param.size = size;
        return m_scaledFont;
    }

    TsFontStyle* unscaledFont()
    {
        return m_unscaledFont;
    }

    TsFont* font() { return m_font; }
    const TsFontParam& param() { return m_param; }

private:
    MonotypeFont(TsFont* font, TsFontParam param)
    : m_font(font), m_param(param)
    {
        m_unscaledFont = TsFontStyleMgr_getFontStyle(BlackBerry::Platform::Graphics::getFontStyleMgr(), m_font, &m_param);
    }

    TsFont* m_font;
    TsFontParam m_param;
    TsFontStyle* m_unscaledFont;
    float m_scale;
    TsFontStyle* m_scaledFont;
};
#endif
...
public:
...
#endif
#if PLATFORM(BLACKBERRY)
    FontPlatformData(PassRefPtr<MonotypeFont> font, float size, bool syntheticBold = false, bool syntheticOblique = false, FontOrientation orientation = Horizontal,
...
#if PLATFORM(BLACKBERRY)
    RefPtr<MonotypeFont> font() const { return m_font; }
#endif
    ...
    unsigned hash() const
    {
        ...
        return PtrHash<RefPtr<MonotypeFont> >::hash(m_font);
#endif
    }
