...
class CanvasLayerWebKitThread
#if USE(SKIA)
: public EGLImageLayerWebKitThread
#else
: public LayerWebKitThread
#endif
{
#if USE(SKIA)
...
public:
    static PassRefPtr<CanvasLayerWebKitThread> create(BlackBerry::Platform::Graphics::Buffer* buffer)
    {
        return adoptRef(new CanvasLayerWebKitThread(buffer));
    }

    static void clearBuffer(CanvasLayerWebKitThread*);

private:
    CanvasLayerWebKitThread(BlackBerry::Platform::Graphics::Buffer*);
