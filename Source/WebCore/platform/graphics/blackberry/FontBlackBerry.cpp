...
#else
    WorldTypeShaper shaper(this, run);
#endif
                      ...
                      int from, int numGlyphs, const FloatPoint& point) const
{
    ...
    // FIXME: we should push patterns here as soon as the text system can handle it
    context->platformContext()->addGlyphs(glyphBuffer.glyphs(from),
                                          reinterpret_cast<const BlackBerry::Platform::FloatSize*>(glyphBuffer.advances(from)),
                                          numGlyphs, adjustedPoint, iType,
                                          context->fillGradient() ? context->fillGradient()->platformGradient() : 0,
                                          context->strokeGradient() ? context->strokeGradient()->platformGradient() : 0);
