...
FontPlatformData* FontCache::createFontPlatformData(const FontDescription& fontDescription, const AtomicString& family)
{
    ...
    fprintf(stderr, "Looking for %s found %s\n", String(family).latin1().data(), fontFileName);
    FcPatternDestroy(resultPattern);

    TsResult result;
    TsFontSpec fontSpec;
    result = TsResource_initPath(&fontSpec.resource, reinterpret_cast<TsChar*>(fontFileName));
    fprintf(stderr, "initPath %d\n", result);
    TsFont* font = TsFontMgr_getFont(BlackBerry::Platform::Graphics::getFontMgr(), fontSpec);
    fprintf(stderr, "font %x\n", font);

    TsFontParam fontParam;
    TsFontParam_defaults(&fontParam);
    fontParam.render = TS_RENDER_GRAYSCALE8;
    fontParam.size = static_cast<TsFixed>(fontDescription.computedSize() * 65536);
    fontParam.renderFlags = TS_DISABLE_HINTS | TS_DISABLE_AUTOHINT;

    return new FontPlatformData(MonotypeFont::create(font, fontParam), fontDescription.computedSize());
}
