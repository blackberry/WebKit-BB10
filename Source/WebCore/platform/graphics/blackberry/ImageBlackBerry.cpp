...
NativeImagePtr ImageFrame::allocateNativeImagePixelData(const IntSize& size)
{
    return BlackBerry::Platform::Graphics::TiledImage::allocateNativeImagePixelData(size, BlackBerry::Platform::Graphics::TiledImage::Software);
}

ImageFrame::PixelData* ImageFrame::getNativeImageDataForRow(unsigned y)
{
    ASSERT(m_nativeImage);
    return static_cast<PixelData*>(m_nativeImage->getPixelDataForRow(y));
}

void ImageFrame::copyNativeImageIntoBuffer(PixelData *buffer, NativeImagePtr nativeImageToCopy)
{
    nativeImageToCopy->copyNativeImageIntoBuffer(buffer);
}

NativeImagePtr ImageFrame::copyNativeImage(NativeImagePtr nativeImageToCopy)
{
    return nativeImageToCopy->copyNativeImage(BlackBerry::Platform::Graphics::TiledImage::Software);
}

void ImageFrame::deleteNativeImage()
{
    delete m_nativeImage;
    m_nativeImage = 0;
    delete m_backingStoreCachedNativeImage;
    m_backingStoreCachedNativeImage = 0;
}

NativeImagePtr ImageFrame::asNewNativeImage()
{
    if (m_nativeImage) {
        // the screen pixmap was allocated (dataisBRGA,dataIsPremultiplied) and the pixels were decoded directly into it,
        // there was no opportunity to update the alpha attribute during the decode so we do it here
        return BlackBerry::Platform::Graphics::TiledImage::asNewNativeImage(*m_nativeImage, false);
    }
    if (m_backingStoreCachedNativeImage) {
        // multiple invocations require us to update the native image with the data in the backingStore
        // (this happens automatically when we decode into the native image but we have to explicitly copy otherwise)
        m_backingStoreCachedNativeImage->updateNativeImage(m_bytes);
    } else {
        // allocateNativeImagePixelData must match this call, type == Software
        m_backingStoreCachedNativeImage = new BlackBerry::Platform::Graphics::TiledImage(m_size, m_bytes);
    }
    // return a copy of our native image
    return BlackBerry::Platform::Graphics::TiledImage::asNewNativeImage(*m_backingStoreCachedNativeImage, true);
}
