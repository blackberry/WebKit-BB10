                        ...
                        CompositeOperator op, const FloatRect& dst)
{
    ...
    // corresponds to a texture drawn with srcRect starting at (0, 0).
    // Thus, dstRect starts at phase + patternTransformation.mapPoint(src.location()).
    AffineTransform srcTransform;
    srcTransform.translate(phase.x(), phase.y());
    srcTransform.multiply(patternTransformation);
    srcTransform.translate(src.x(), src.y());
    ...
    CompositeOperator oldOperator = context->compositeOperation();
    context->setCompositeOperation(op);
    context->platformContext()->addPattern(dst, src, srcTransformArray, image);
    context->setCompositeOperation(oldOperator);
}
