...
static bool getImageDataInternal(GraphicsContext* context, const IntRect& rect, unsigned char* result, bool unmultiply, HostWindow* window)
{
    ...
    context->platformContext()->readPixels(rect, result, unmultiply);
    return true;
}

void ImageBufferData::getImageData(GraphicsContext* context, const IntRect& rect, unsigned char* result, bool unmultiply) const
{
    if (!isCompositingThread()) {
        ...
        dispatchSyncCompositingMessage(BlackBerry::Platform::createFunctionCallMessage(
            &getImageDataInternal, context, rect, result, unmultiply, m_window));
        return;
    }
...
static bool flushAndDraw(const ImageBufferData* object, GraphicsContext* context, ColorSpace styleColorSpace, const FloatRect& destRect, const FloatRect& srcRect,
...
void ImageBufferData::draw(GraphicsContext* thisContext, GraphicsContext* otherContext,
...
{
    if (thisContext->platformContext()->isEmpty()) {
        CompositeOperator oldOperator = otherContext->compositeOperation();
        otherContext->setCompositeOperation(op);
        otherContext->platformContext()->drawBuffer(m_buffer, destRect, srcRect);
        otherContext->setCompositeOperation(oldOperator);
    }
    else {
        dispatchSyncCompositingMessage(BlackBerry::Platform::createFunctionCallMessage(
            &flushAndDraw, this, otherContext, styleColorSpace, destRect, srcRect, op, useLowQualityScale));
    }
}
    ...
    , m_resolutionScale(1)
{
    ...
    if (maxTileSize <= size.width() || maxTileSize <= size.height()) {
        ...
        // FIXME: We need to implmenet a tiled buffer.
        m_size = m_size.shrunkTo(IntSize(maxTileSize, maxTileSize));
        m_logicalSize = m_size;
    }

    m_data.m_buffer = BlackBerry::Platform::Graphics::createBuffer(m_size, BlackBerry::Platform::Graphics::AlwaysBacked);
    m_data.m_platformLayer = CanvasLayerWebKitThread::create(m_data.m_buffer);
...
PassRefPtr<Image> ImageBuffer::copyImage(BackingStoreCopy copyBehavior) const
{
    ...
    m_data.getImageData(m_context.get(), IntRect(IntPoint(0, 0), m_size), (unsigned char*)imageData, false /* unmultiply */);
...
PassRefPtr<Uint8ClampedArray> ImageBuffer::getUnmultipliedImageData(const IntRect& rect, CoordinateSystem) const
{
    ...
    m_data.getImageData(m_context.get(), rect, result->data(), true);
    return result;
}
...
{
    RefPtr<Uint8ClampedArray> result = Uint8ClampedArray::createUninitialized(rect.width() * rect.height() * 4);
    m_data.getImageData(m_context.get(), rect, result->data(), false);
    return result;
}
...
void ImageBuffer::putByteArray(Multiply multiplied, Uint8ClampedArray* source, const IntSize& sourceSize, const IntRect& sourceRect, const IntPoint& destPoint, CoordinateSystem)
{
    ...
    m_context->platformContext()->addImage(FloatRect(destPoint, sourceRect.size()), FloatRect(sourceRect), &image);
