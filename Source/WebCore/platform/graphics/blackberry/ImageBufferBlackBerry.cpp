...
static bool getImageDataInternal(GraphicsContext* context, const IntRect& rect, const IntRect& size, unsigned char* result, bool unmultiply, HostWindow* window)
{
    ...
    if (subrect.maxX() > maxTileSize || subrect.maxY() > maxTileSize) {
        // we do not support GL_PACK_ROW_LENGTH, do it manually
        IntRect imageRect = IntRect(0, 0, std::min(size.width(), maxTileSize), std::min(size.height(), maxTileSize));
        int srcstride = imageRect.width() * 4;
        int dststride = rect.width() * 4;
        unsigned char* temp = new unsigned char[srcstride * imageRect.height()];
        context->platformContext()->readPixels(imageRect, temp, unmultiply);
        subrect.intersect(imageRect);
        for (int i = 0; i < subrect.height(); ++i) {
            memcpy(result + subrect.x() * 4 + i * dststride, temp + subrect.x() * 4 + i * srcstride, subrect.width() * 4);
            memset(result + subrect.width() * 4 + i * dststride, 0, (rect.width() - subrect.width()) * 4);
        }

        delete [] temp;
    } else {
        if (!subrect.isEmpty())
            context->platformContext()->readPixels(subrect, result, unmultiply);
    }
    return true;
}
...
ImageBuffer::ImageBuffer(const IntSize& size, float scale, ColorSpace, RenderingMode renderingMode, DeferralMode, HostWindow* window, bool& success)
    : m_size(expandedIntSize(size * scale))
    , m_logicalSize(size)
...
void ImageBuffer::draw(GraphicsContext* context, ColorSpace styleColorSpace, const FloatRect& destRect, const FloatRect& srcRect, CompositeOperator op, BlendMode, bool useLowQualityScale)
{
    ...
    m_data.draw(m_context.get(), context, styleColorSpace, destRect, scaledSrcRect, op, useLowQualityScale);
}
...
void ImageBuffer::drawPattern(GraphicsContext* context, const FloatRect& srcRect, const AffineTransform& patternTransform, const FloatPoint& phase, ColorSpace styleColorSpace, CompositeOperator op, const FloatRect& destRect)
{
    ...
    image->drawPattern(context, scaledSrcRect, patternTransform, phase, styleColorSpace, op, destRect);
}
...
PassRefPtr<Uint8ClampedArray> ImageBuffer::getUnmultipliedImageData(const IntRect& rect, CoordinateSystem coordinateSystem) const
{
    ...
    RefPtr<Uint8ClampedArray> result = Uint8ClampedArray::createUninitialized(scaledRect.width() * scaledRect.height() * 4);
    if (result)
        m_data.getImageData(m_context.get(), scaledRect, IntRect(IntPoint(0, 0), m_size), result->data(), true);
    return result;
}
...
PassRefPtr<Uint8ClampedArray> ImageBuffer::getPremultipliedImageData(const IntRect& rect, CoordinateSystem coordinateSystem) const
{
    ...
    RefPtr<Uint8ClampedArray> result = Uint8ClampedArray::createUninitialized(scaledRect.width() * scaledRect.height() * 4);
    if (result)
        m_data.getImageData(m_context.get(), scaledRect, IntRect(IntPoint(0, 0), m_size), result->data(), false);
    return result;
}
...
void ImageBuffer::putByteArray(Multiply multiplied, Uint8ClampedArray* source, const IntSize& sourceSize, const IntRect& sourceRect, const IntPoint& destPoint, CoordinateSystem coordinateSystem)
{
    ...
    FloatRect dstRect(destPoint + sourceRect.location(), sourceRect.size());
    if (coordinateSystem == LogicalCoordinateSystem)
        dstRect.scale(m_resolutionScale);
    ...
    m_context->platformContext()->addImage(dstRect, FloatRect(sourceRect), &image);
