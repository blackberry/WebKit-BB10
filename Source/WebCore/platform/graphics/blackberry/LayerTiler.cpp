    ...
    HashSet<TileIndex> renderJobs;
    {
        ...
        for (TileMap::iterator it = m_tiles.begin(); it != m_tiles.end(); ++it) {
            ...
            if (tile->needsRender()) {
                ...
                // visible. Workaround this by always rendering them (a very fast operation with the GL renderer,
                // since it will just result in ref'ing the mask image).
...
#else
        if (BlackBerry::Platform::Graphics::Buffer* buffer = m_layer->paintContents(contentsRect, scale, m_needsBacking))
...
#else
            if (BlackBerry::Platform::Graphics::Buffer* buffer = m_layer->paintContents(tileRect, scale, m_needsBacking))
...
bool LayerTiler::drawTile(LayerCompositingThread* layer, double scale, const TileIndex& index, const FloatRect& dst, const GLES2Program& program)
{
    ...
    glVertexAttribPointer(program.positionLocation(), 4, GL_FLOAT, GL_FALSE, 0, vertices);
...
#if USE(SKIA)
            if (texture->isOpaque() && globalAlpha == 255 && !layer->maskLayer())
                glDisable(GL_BLEND);
            else {
                glEnable(GL_BLEND);
    ...
    if (layer->contentsResolutionIndependent()) {
        ...
        for (src.setX(0); src.x() < m_requiredTextureSize.width(); src.setX(src.x() + src.width())) {
            for (src.setY(0); src.y() < m_requiredTextureSize.height(); src.setY(src.y() + src.height())) {
                ...
                FloatRect dst(x - bounds.width() / 2.0,
                              y - bounds.height() / 2.0,
                              min(bounds.width() - x, src.width() * sx),
                              min(bounds.height() - y, src.height() * sy));
