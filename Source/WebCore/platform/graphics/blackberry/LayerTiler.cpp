...
void LayerTiler::updateTextureContentsIfNeeded(double scale)
{
    ...
    if (!renderJobs.isEmpty()) {
        if (Image* image = m_layer->contents()) {
            ...
            if (NativeImagePtr nativeImage = image->nativeImageForCurrentFrame()) {
                ...
                addTextureJob(TextureJob::setContents(bitmap, contentsRect, isOpaque));
            }
#else
            if (BlackBerry::Platform::Graphics::Buffer* buffer = m_layer->paintContents(contentsRect, scale))
                addTextureJob(TextureJob::setContents(buffer, contentsRect, m_layer->isOpaque()));
#endif
        } else {
...
#else
                if (BlackBerry::Platform::Graphics::Buffer* buffer = m_layer->paintContents(tileRect, scale))
    ...
    if (Image* image = m_layer->contents()) {
        ...
        if (NativeImagePtr nativeImage = m_layer->contents()->nativeImageForCurrentFrame()) {
            SkBitmap bitmap = SkBitmap(nativeImage->bitmap());
            addTextureJob(TextureJob::setContents(bitmap, contentsRect, isOpaque));
        }
#else
...
#else
                    TransformationMatrix dt = drawTransform;
                    dt.scale(layer->contentsResolutionIndependent() ? 1.0 : 1.0 / layer->contentsScale());
                    dt.scale(layer->sizeIsScaleInvariant() ? 1.0 / scale : 1.0);
                    blitToBuffer(0, texture->textureId(), reinterpret_cast<BlackBerry::Platform::TransformationMatrix&>(dt),
...
#else
    lockAndBindBufferGLTexture(tile->texture()->textureId(), GL_TEXTURE_2D);
#endif
}
