...
MediaPlayerPrivate::MediaPlayerPrivate(MediaPlayer* player)
    ...
    , m_platformPlayerIsCamera(false)
{
}
...
void MediaPlayerPrivate::load(const WTF::String& url)
{
    ...
    if (modifiedUrl.startsWith("local://")) {
        ...
        tempPath.append(decodeURLEscapeSequences(kurl.path())); // setPath will re-encode
    ...
    bool isMediaDocument = m_webCorePlayer->mediaPlayerClient()->mediaPlayerOwningDocument()->isMediaDocument();

    deleteGuardedObject(m_platformPlayer);
#if USE(ACCELERATED_COMPOSITING)
    m_platformPlayer = PlatformPlayer::create(this, tabId, isVideo, true, isMediaDocument, modifiedUrl);
#else
    m_platformPlayer = PlatformPlayer::create(this, tabId, isVideo, false, isMediaDocument, modifiedUrl);
#endif
    m_platformPlayer->setPrivateBrowsingMode(m_privateBrowsing);

    std::vector<BlackBerry::Platform::String> cookies;
    if (!url.isEmpty()) {
        Vector<RefPtr<ParsedCookie> > rawCookies;
        cookieManager().getRawCookies(rawCookies, KURL(ParsedURLString, url), WithHttpOnlyCookies);
        WTF::StringBuilder cookieStringBuilder;
        cookieStringBuilder.reserveCapacity(256);
        for (size_t i = 0; i < rawCookies.size(); i++) {
            RefPtr<ParsedCookie> rawCookie = rawCookies[i];
            cookieStringBuilder.clear();
            cookieStringBuilder.append("Set-Cookie: ");
            cookieStringBuilder.append(rawCookie->toNameValuePair());
            if (!rawCookie->domain().isEmpty()) {
                cookieStringBuilder.append("; domain=");
                cookieStringBuilder.append(rawCookie->domain());
            }
            if (!rawCookie->path().isEmpty()) {
                cookieStringBuilder.append("; path=");
                cookieStringBuilder.append(rawCookie->path());
            }
            cookies.push_back(cookieStringBuilder.toString());
        }
    }
    m_platformPlayer->load(playerID, modifiedUrl, m_webCorePlayer->userAgent(), cookies, m_webCorePlayer->contentMIMEType());
}
...
float MediaPlayerPrivate::currentTime() const
{
    ...
    return m_userDrivenSeekTimer.isActive() && !shortMedia ? m_lastSeekTime: m_platformPlayer->currentMediaTime();
}
...
void MediaPlayerPrivate::seek(float time)
{
    ...
    bool timerActive = m_userDrivenSeekTimer.isActive();
    // If we just did (or are about to do) a seek to a time within a hundredth
    // of a second of the requested time, do nothing. This gets rid of some
    // redundant seeks.
    if (timerActive && fabsf(m_lastSeekTime - time) < 0.01f)
        return;
    m_lastSeekTime = time;
...
void MediaPlayerPrivate::setPrivateBrowsingMode(bool mode)
{
    // m_platformPlayer may not be created yet.
    m_privateBrowsing = mode;
    if (m_platformPlayer)
        m_platformPlayer->setPrivateBrowsingMode(mode);
}
...
bool MediaPlayerPrivate::hasRemoteDisplays()
{
    if (m_platformPlayer)
        return m_platformPlayer->hasRemoteDisplays();
    return false;
}

bool MediaPlayerPrivate::isPresentationModeEnabled()
{
    if (m_platformPlayer)
        return m_platformPlayer->isPresentationModeEnabled();
    return false;
}

bool MediaPlayerPrivate::isMiracastSupported()
{
    if (m_platformPlayer)
        return m_platformPlayer->isMiracastSupported();
    return false;
}

bool MediaPlayerPrivate::isAccessibilityCaptionsEnabled()
{
    if (m_platformPlayer)
        return m_platformPlayer->isAccessibilityCaptionsEnabled();
    return false;
}

void MediaPlayerPrivate::setPresentationModeEnabled(bool modeOn)
{
    if (m_platformPlayer)
        m_platformPlayer->setPresentationModeEnabled(modeOn);
}
...
void MediaPlayerPrivate::onMuteChanged(bool mute)
{
    m_webCorePlayer->muteChanged(mute);
}
...
static const int popupDialogInterval = 15;
...
void MediaPlayerPrivate::onWaitMetadataNotified(bool hasFinished, int timeWaited)
{
    if (!hasFinished) {
        if (!m_waitMetadataTimer.isActive()) {
            // Popup dialog "popupDialogInterval" seconds after metadata starts
            // to load, then increasing times after that. This function should
            // be called only once the first time the user presses play.
            m_waitMetadataPopDialogCounter = static_cast<int>(timeWaited / checkMetadataReadyInterval);
            m_hitTimeToPopupDialog = static_cast<int>(popupDialogInterval / checkMetadataReadyInterval);
...
void MediaPlayerPrivate::waitMetadataTimerFired(Timer<MediaPlayerPrivate>*)
{
    ...
    if (m_waitMetadataPopDialogCounter < m_hitTimeToPopupDialog) {
        m_waitMetadataTimer.startOneShot(checkMetadataReadyInterval);
        return;
    }
    m_waitMetadataPopDialogCounter = 0;
...
void MediaPlayerPrivate::onAuthenticationNeeded(MMRAuthChallenge& authChallenge)
{
    ...
    AuthenticationChallengeManager::instance()->authenticationChallenge(url, protectionSpace, this, m_webCorePlayer->mediaPlayerClient()->mediaPlayerHostWindow()->platformPageClient(), true);
}
...
void MediaPlayerPrivate::notifyChallengeResult(const KURL& url, const ProtectionSpace&, AuthenticationChallengeResult result, const Credential& credential)
{
    ...
    // FIXME: this treats AuthenticationChallengeRetryWithoutNegotiate the same as
    // AuthenticationChallengeCancelled
    if (result != AuthenticationChallengeSuccess || !url.isValid())
        return;

    m_platformPlayer->reloadWithCredential(credential.user());
}
...
void MediaPlayerPrivate::onAuthenticationAccepted(const MMRAuthChallenge& authChallenge) const
{
    ...
    if (savedCredential.isEmpty())
        CredentialStorage::set(Credential(authChallenge.credential().c_str(), WTF::emptyString(), CredentialPersistenceForSession), protectionSpace, url);
}
...
int MediaPlayerPrivate::onShowErrorDialog(PlatformPlayer::Error type)
{
    ...
    case PlatformPlayer::MediaInvalidError:
        ...
        break;
    case PlatformPlayer::MediaTrackLoadError:
        atype = WebPageClient::MediaTrackLoadError;
        break;
    default:
...
bool MediaPlayerPrivate::isElementFullscreen() const
{
    return m_webCorePlayer->mediaPlayerClient()->mediaPlayerIsFullscreen();
}
...
void MediaPlayerPrivate::onSetHolePunchRectEnabled(bool enabled)
{
#if USE(ACCELERATED_COMPOSITING)
    if (m_platformLayer)
        static_cast<VideoLayerWebKitThread*>(m_platformLayer.get())->setHolePunchRectEnabled(enabled);
#endif
}
...
void MediaPlayerPrivate::onUpdatePlatformLayer()
{
#if USE(ACCELERATED_COMPOSITING)
    if (m_platformLayer)
        m_platformLayer->setNeedsDisplay();
