...
#if USE(SKIA)
    BlackBerry::Platform::Graphics::Drawable* dst = context->platformContext()->canvas();
#else
...
void MediaPlayerPrivate::onAuthenticationNeeded(MMRAuthChallenge& authChallenge)
{
    ...
    m_webCorePlayer->mediaPlayerClient()->mediaPlayerHostWindow()->platformPageClient()->authenticationChallenge(url, protectionSpace, credential, this);
}
...
static void loadBufferingImageData()
{
    ...
    if (!loaded) {
        ...
        memcpy(s_bufferingImageData, bufferingIcon->data()->data(), bufSize);
        bufferingIcon->deref();
    }
}
