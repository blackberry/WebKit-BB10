...
void GraphicsContext::fillPath(const Path& path)
{
    ...
    if (!pp->isEmpty()) {
        ...
        platformContext()->addFillPath(*pp, static_cast<BlackBerry::Platform::Graphics::WindRule>(m_state.fillRule), platformGradient, platformPattern);
    }
}
...
{
    BlackBerry::Platform::Graphics::Path* pp = path.platformPath();
    pp->applyAsClip(platformContext(), static_cast<BlackBerry::Platform::Graphics::WindRule>(fillRule));
}

void GraphicsContext::clipPath(const Path& path, WindRule fillRule)
{
