...
void SimpleFontData::platformInit()
{
    ...
    if (m_platformData.size() > 0) {
        // FIXME: hack! FS_get_ascender_descender_leading() returns ERR_NO_CURRENT_SFNT when size is 0, even though we called FS_set_scale and m_platformData.font()->cur_sfnt is not NULL
...
float SimpleFontData::platformWidthForGlyph(Glyph glyph) const
{
    FS_SHORT i_dx, i_dy;
    FS_FIXED dx, dy;
    if (FS_get_advance(m_platformData.font(), glyph, FS_MAP_DISTANCEFIELD | FS_MAP_GRAYMAP8, &i_dx, &i_dy, &dx, &dy) != SUCCESS)
        return 0;
