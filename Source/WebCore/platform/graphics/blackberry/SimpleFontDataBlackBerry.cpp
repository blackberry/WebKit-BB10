...
{
    FS_FIXED iTypeAscender = 0;
    FS_FIXED iTypeDescender = 0;
    FS_FIXED iTypeLeading = 0;
    FsAscDescLeadSource source = FsAscDescLeadSource_IGNORE_VDMX_TABLE;
    FS_LONG result;
    if (m_platformData.size() > 0) {
        ...
        result = FS_get_ascender_descender_leading(m_platformData.font(), &iTypeAscender, &iTypeDescender, &iTypeLeading, &source);
        ASSERT_UNUSED(result, result == SUCCESS);
    }

    float ascent = FS_ROUND(iTypeAscender);
    float descent = FS_ROUND(iTypeDescender);
    float linegap = iTypeFixedToFloat(iTypeLeading);

    // If the descent is rounded down, the descent part of the glyph may be truncated when displayed in a 'overflow: hidden' container.
    // To avoid that, borrow 1 unit from the ascent when possible.
    if (descent < iTypeFixedToFloat(iTypeDescender) && ascent >= 1) {
        ++descent;
        --ascent;
    }
    ...
    m_fontMetrics.setLineSpacing(ascent + descent + lroundf(m_fontMetrics.lineGap()));
    ...
    if (FS_get_table_structure(m_platformData.font(), TAG_OS2, &os2) == SUCCESS && os2.sxHeight && os2.xAvgCharWidth) {
        FS_USHORT yppem = m_platformData.font().state()->lpm;
...
PassRefPtr<SimpleFontData> SimpleFontData::createScaledFontData(const FontDescription& fontDescription, float scaleFactor) const
{
    ...
    return adoptRef(new SimpleFontData(
        FontPlatformData(m_platformData.font().state()->cur_lfnt->name,
            scaledSize,
