...
Texture::GpuHandle TextureCacheCompositingThread::allocateTextureId()
{
#if USE(SKIA)
    unsigned texid;
    ...
    return texid;
#else
    return createBuffer(IntSize(0,0), BlackBerry::Platform::Graphics::TileBuffer);
#endif
}
...
bool TextureCacheCompositingThread::install(Texture* texture)
{
    ...
    if (!texture->hasTexture()) {
        Texture::GpuHandle textureId = allocateTextureId();
        if (!textureId)
            return false;
