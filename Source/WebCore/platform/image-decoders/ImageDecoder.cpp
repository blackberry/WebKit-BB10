...
void ImageFrame::zeroFillPixelData()
{
    m_hasAlpha = true;
#if PLATFORM(BLACKBERRY)
    ASSERT(!m_backingStoreCachedNativeImage);
    if (m_nativeImage) {
        ASSERT(!m_bytes);
        const unsigned numRows = m_size.height();
        const unsigned numBytes = m_size.width() * sizeof(PixelData);
        for (unsigned r = 0; r < numRows; ++r)
            memset(getNativeImageDataForRow(r), 0, numBytes);
        return;
    }
#endif
...
bool ImageFrame::copyBitmapData(const ImageFrame& other)
{
    ...
    bool ret = true;
    bool backingStoreCopied = false;
#if PLATFORM(BLACKBERRY)
    deleteNativeImage(); // prevent memory leak if we are being called by op=
    ASSERT(!m_nativeImage);
    ASSERT(!m_backingStoreCachedNativeImage);
    if (other.m_nativeImage) {
        m_nativeImage = copyNativeImage(other.m_nativeImage);
        if (!m_nativeImage) {
            // we couldn't allocate a native image object to copy into!
            // action: if possible, switch this ImageFrame over to m_backingStore
            size_t backingStoreSize = other.m_size.width() * other.m_size.height();
            if (m_backingStore.tryReserveCapacity(backingStoreSize)) {
                m_backingStore.resize(backingStoreSize);
                copyNativeImageIntoBuffer(m_backingStore.data(), other.m_nativeImage);
                backingStoreCopied = true;
            } else
                ret = false;
        }
    }
#endif
    if (!backingStoreCopied)
...
{
    ASSERT((!width() && !height()) || m_status == FrameEmpty);
#if PLATFORM(BLACKBERRY)
    m_nativeImage = allocateNativeImagePixelData(IntSize(newWidth, newHeight));
    if (m_nativeImage) {
        ASSERT(!m_bytes);
...
#if PLATFORM(BLACKBERRY)
    if (m_nativeImage) {
        // doesn't include stride but close enough
        const size_t pixmapSize = m_size.height() * m_size.width() * sizeof(PixelData);
        info.addPrivateBuffer(pixmapSize, 0, "screen_pixmap", "nativeImage");
    }
#endif
