...
BlobStream::BlobStream(const ResourceRequest& request, ResourceHandle* handle)
    : FilterStream()
    , m_request(request)
{
    registerStreamId();
...
void BlobStream::didReceiveResponse(ResourceHandle*, const ResourceResponse& response)
{
    m_response = response;
}
...
{
    if (m_response.url().string().length())
        return m_response.url().string();
    return m_request.url().string();
}
