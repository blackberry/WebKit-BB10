...
public:
    ...
    virtual void notifyAuthReceived(BlackBerry::Platform::NetworkRequest::AuthType,
        ...
        const char* realm,
        AuthResult);
    virtual void notifyClientCertRequested();
    ...
    virtual void notifyWillUpdateDiskCache();
    virtual void notifyDidLoadFromDiskCache();
...
private:
    ...
    void notifyChallengeFailed(const ProtectionSpace&);
    ...
    void sendRequestWithCredentials(ProtectionSpaceServerType, ProtectionSpaceAuthenticationScheme, const String& realm = WTF::emptyString(), bool requireCredentials = true);

    // storeCredentials() are only storing credentials in memory, not on disk.
    // Saving credential to disk (or not) are handled by CredMgr.
