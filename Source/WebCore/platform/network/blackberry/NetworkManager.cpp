...
static void setAuthCredentials(NetworkRequest& platformRequest, const AuthenticationChallenge& challenge)
{
    ...
    if (authType != NetworkRequest::AuthTypeNone && authProtocol != NetworkRequest::AuthProtocolNone && authScheme != NetworkRequest::AuthSchemeNone) {
        // The credential handle is in the user field. The password field is unused.
        platformRequest.setCredential(authType, authProtocol, authScheme, credential.user());
    }

    platformRequest.setAllowNegotiateAuth(challenge.shouldAllowNegotiate());
}
...
int NetworkManager::startJob(int playerId, const String& pageGroupName, PassRefPtr<ResourceHandle> job, const ResourceRequest& request, BlackBerry::Platform::NetworkStreamFactory* streamFactory, Frame* frame, int deferLoadingCount, int redirectCount, bool rereadCookies)
{
    ...
    BlackBerry::Platform::NetworkRequest platformRequest(url.string());
    request.initializePlatformRequest(platformRequest, frame->loader() && frame->loader()->client() && static_cast<FrameLoaderClientBlackBerry*>(frame->loader()->client())->cookiesEnabled(), rereadCookies);
    ...
    if (settings && settings->privateBrowsingEnabled())
        platformRequest.setPrivateClientId(playerId);
