...
static inline NetworkRequest::TargetType platformTargetTypeForRequest(const ResourceRequest& request)
{
    ...
    case ResourceRequest::TargetIsTextTrack:
        return NetworkRequest::TargetIsSubresource;
    case ResourceRequest::TargetIsDownload:
...
void ResourceRequest::initializePlatformRequest(NetworkRequest& platformRequest, bool cookiesEnabled, bool rereadCookies) const
{
    platformRequest.setMethod(httpMethod());
    platformRequest.setCachePolicy(platformCachePolicyForRequest(*this));
    platformRequest.setTargetType(platformTargetTypeForRequest(*this));
    platformRequest.setTimeout(timeoutInterval());
    platformRequest.setConditional(isConditional());
    platformRequest.setSuggestedSaveName(suggestedSaveName());
    platformRequest.setClientCertHandle(clientCertificateHandle());
...
PassOwnPtr<CrossThreadResourceRequestData> ResourceRequest::doPlatformCopyData(PassOwnPtr<CrossThreadResourceRequestData> data) const
{
    ...
    data->m_clientCertificateHandle = m_clientCertificateHandle;
...
void ResourceRequest::doPlatformAdopt(PassOwnPtr<CrossThreadResourceRequestData> data)
{
    ...
    m_clientCertificateHandle = data->m_clientCertificateHandle;
...
void ResourceRequest::clearHTTPConditionalHeaders()
{
    updateResourceRequest();
