...
void RSSFilterStream::notifyHeadersReceived(const NetworkRequest::HeaderList& headers)
{
    ...
    // just ignore the MIMEType from "Content-Type", we will sniff  in notifyDataReceived method later
...
{
    // here we will sniff the MIMEType from the content
    if (!m_sniffed) {
        MIMESniffer sniffer = MIMESniffer(m_contentType.data(), MIMETypeRegistry::isSupportedImageResourceMIMEType(m_contentType));

        if (const char* type = sniffer.sniff(BlackBerry::Platform::networkBufferData(buffer), std::min(BlackBerry::Platform::networkBufferDataLength(buffer), sniffer.dataSize())))
            m_sniffedMimeType = BlackBerry::Platform::String::fromLatin1(type);

        // if the content-type both from http header and the sniffed one doesn't include "xml", it's not a RSS content
        if (m_contentType.find("xml") == std::string::npos && m_sniffedMimeType.find("xml") == std::string::npos)
            m_resourceType = TypeNotRSS;
...
void RSSFilterStream::handleRSSContent()
{
    ...
    // after the content is handled as html, the MIME type will be changed, so we sniff it again
    if (const char* type = sniffer.sniff(html.c_str(), std::min(html.size(), sniffer.dataSize())))
        m_sniffedMimeType = BlackBerry::Platform::String::fromLatin1(type);
    else
        m_sniffedMimeType = BlackBerry::Platform::String::emptyString();
...
BlackBerry::Platform::String RSSFilterStream::sniffedMimeType() const
{
    return m_sniffedMimeType;
}
