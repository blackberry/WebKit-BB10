...
void EllipsisBox::paint(PaintInfo& paintInfo, const LayoutPoint& paintOffset, LayoutUnit lineTop, LayoutUnit lineBottom)
{
    ...
    if (style->textOverflow() == TextOverflowFade) {
        int scrollAmount = 0;
        if (Node* checkNode = m_renderer->containingBlock()->node())
            scrollAmount = static_cast<Element*>(checkNode)->scrollLeft();
        FloatPoint fp1 = FloatPoint(x() + scrollAmount + paintOffset.x(), y() + paintOffset.y() + style->fontMetrics().ascent());
        FloatPoint fp2 = fp1;
        fp2.move(width(), 0);
        RefPtr<Gradient> gradient = Gradient::create(fp1, fp2);
        bool ltr = style->direction() == LTR;
        gradient->addColorStop(0, Color(textColor.red(), textColor.green(), textColor.blue(), ltr ? 255 : 30));
        gradient->addColorStop(1, Color(textColor.red(), textColor.green(), textColor.blue(), ltr ? 30 : 255));
