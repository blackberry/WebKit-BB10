...
void InlineTextBox::paintAttributeText(GraphicsContext* context, const FloatPoint& boxOrigin, DocumentMarker* marker, RenderStyle* style, const Font& font)
{
    ...
    IntRect markerRect = enclosingIntRect(font.selectionRectForText(run, IntPoint(x(), y()), heightOfSelectionRect, startPos, endPos));
    markerRect = renderer()->localToAbsoluteQuad(FloatRect(markerRect)).enclosingBoundingBox();
    toRenderedDocumentMarker(marker)->setRenderedRect(markerRect);
    ...
    if (backgroundColor.isValid()) {
        ...
        context->clip(IntRect(tx + x(), ty + topOfSelectionRect, m_logicalWidth, heightOfSelectionRect));
        context->drawHighlightForText(font, run, IntPoint(x() + tx, topOfSelectionRect + ty), heightOfSelectionRect, backgroundColor, style->colorSpace(), startPos, endPos);
        context->restore();
    }
    ...
    if (hasAttributeTextFont || backgroundColor.isValid()) {
        ...
        IntPoint textOrigin(x() + tx, y() + ty + renderer()->style(isFirstLineStyle())->font().fontMetrics().ascent());
        FloatRect boxRect(boxOrigin, LayoutSize(logicalWidth(), logicalHeight()));
