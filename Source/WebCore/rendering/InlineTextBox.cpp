...
float InlineTextBox::placeEllipsisBox(bool flowIsLTR, float visibleLeftEdge, float visibleRightEdge, float ellipsisWidth, float &truncatedWidth, bool& foundBox)
{
    ...
    if (ltrEllipsisWithinBox || rtlEllipsisWithinBox) {
        ...
        if (renderer()->style()->textOverflow() == TextOverflowFade) {
            if (root()->ellipsisBox())
                root()->ellipsisBox()->m_renderer = renderer();
        }
#endif
    ...
    if (!paintSelectedTextOnly) {
        ...
        if (renderer()->style()->textOverflow() == TextOverflowFade) {
            if (root()->ellipsisBox()) {
                 ...
                 String s(characters, maximumLength);
                 bool ltr = renderer()->containingBlock()->style()->isLeftToRightDirection();
                 if (ltr)
                     root()->ellipsisBox()->setText(s.substring(m_truncation));
                 else
                     root()->ellipsisBox()->setText(ellipsisStr);
             }
        }
#endif
