...
void InlineTextBox::addFadeEffectIfNeeded(GraphicsContext* context, const LayoutPoint& paintOffset, const Font& font)
{
    if (renderer()->style()->textOverflow() == TextOverflowFade) {
        const int fadeAmount = 50; // 50px fade

        const LayoutUnit containerWidth = renderer()->containingBlock()->contentWidth();
        int scrollingAmount = 0;
        if (Node* checkNode = renderer()->containingBlock()->node())
            scrollingAmount = static_cast<Element*>(checkNode)->scrollLeft();
        bool fadeOnLeft = (left() - scrollingAmount) < 0;
        bool fadeOnRight = (right() - (scrollingAmount + left())) > containerWidth;
        if (!fadeOnLeft && !fadeOnRight)
            return;
        FloatPoint fp1 = FloatPoint(left() + scrollingAmount + paintOffset.x(), y() + paintOffset.y());
        FloatPoint fp2 = fp1;
        if (fadeOnRight) {
            if (!fadeOnLeft)
                fp1.move(containerWidth - fadeAmount, 0);
            fp2.move(containerWidth, 0);
        } else
            fp1.move(fadeAmount, 0);
        RefPtr<Gradient> gradient = Gradient::create(fp1, fp2);
        Color textColor = renderer()->style()->visitedDependentColor(CSSPropertyWebkitTextFillColor);
        if (fadeOnLeft && fadeOnRight) {
            const float fadeFraction = float(fadeAmount) / containerWidth;
            const float fadeLeft = std::min(0.5f, (std::min(1.0f, float(scrollingAmount) / float(fadeAmount)) * fadeFraction));
            const float fadeRight = std::min(0.5f, (std::min(1.0f, float((right() - (scrollingAmount + left())) - containerWidth) / float(fadeAmount)) * fadeFraction));
            gradient->addColorStop(0, Color(textColor.red(), textColor.green(), textColor.blue(), 30));
            gradient->addColorStop(fadeLeft, Color(textColor.red(), textColor.green(), textColor.blue(), 255));
            gradient->addColorStop(1.0f - fadeRight, Color(textColor.red(), textColor.green(), textColor.blue(), 255));
            gradient->addColorStop(1, Color(textColor.red(), textColor.green(), textColor.blue(), 30));
        } else {
            gradient->addColorStop(0, Color(textColor.red(), textColor.green(), textColor.blue(), 255));
            gradient->addColorStop(1, Color(textColor.red(), textColor.green(), textColor.blue(), 30));
        }
...
#if PLATFORM(BLACKBERRY)
        addFadeEffectIfNeeded(context, paintOffset, font);
#endif
...
#if PLATFORM(BLACKBERRY)
        // We want the rendered rects for spelling markers as well so we can hit test them later.
        if (true) {
#else
...
void InlineTextBox::paintAttributeText(GraphicsContext* context, const FloatPoint& boxOrigin, DocumentMarker* marker, RenderStyle* style, const Font& font)
{
    // TODO when preparing this code for upstreaming, this code can be shared with paintTextMatchMarker and
    ...
    IntRect markerRect = enclosingIntRect(font.selectionRectForText(run, IntPoint(x(), topOfSelectionRect), heightOfSelectionRect, startPos, endPos));
    markerRect = renderer()->localToAbsoluteQuad(FloatRect(markerRect)).enclosingBoundingBox();
    toRenderedDocumentMarker(marker)->setRenderedRect(markerRect);
    ...
    if (backgroundColor.isValid()) {
        ...
        context->clip(IntRect(tx, ty - deltaY, m_logicalWidth, heightOfSelectionRect));
        context->drawHighlightForText(font, run, IntPoint(tx, ty - deltaY), heightOfSelectionRect, backgroundColor, style->colorSpace(), startPos, endPos);
        context->restore();
    }
    ...
    if (hasAttributeTextFont || backgroundColor.isValid()) {
        ...
        IntPoint textOrigin(tx, ty + renderer()->style(isFirstLineStyle())->font().fontMetrics().ascent());
        FloatRect boxRect(boxOrigin, LayoutSize(logicalWidth(), logicalHeight()));
        ...
        paintTextWithShadows(context, hasAttributeTextFont ? attributeTextFont : font, run, nullAtom, 0, startPos, endPos, m_len, textOrigin, boxRect, 0, textStrokeWidth > 0, isHorizontal());
        if (textStrokeWidth > 0)
            context->restore();
    }
    ...
    if (attributeTextStyle.underline() != AttributeTextStyle::UnderlineNotProvided)
        paintCompositionUnderline(context, boxOrigin, CompositionUnderline(marker->startOffset(), marker->endOffset(), textColor, false, toStrokeStyle(attributeTextStyle.underline())));
    if (attributeTextStyle.strikethrough() == AttributeTextStyle::AttributeApplied)
        paintCompositionLineThrough(context, boxOrigin, CompositionLineThrough(marker->startOffset(), marker->endOffset(), textColor));
}
...
void InlineTextBox::paintDocumentMarkers(GraphicsContext* pt, const FloatPoint& boxOrigin, RenderStyle* style, const Font& font, bool background)
{
    ...
    for ( ; markerIt != markers.end(); markerIt++) {
        ...
        switch (marker->type()) {
            case DocumentMarker::AttributeText:
                paintAttributeText(pt, boxOrigin, marker, style, font);
                break;
            case DocumentMarker::Spelling:
            case DocumentMarker::CorrectionIndicator:
            case DocumentMarker::DictationAlternatives:
...
{
    start = 0;                 // start of line to draw, relative to tx
    width = m_logicalWidth;           // how much line to draw
...
void InlineTextBox::paintCompositionUnderline(GraphicsContext* ctx, const FloatPoint& boxOrigin, const CompositionUnderline& underline)
{
    ...
    unsigned start = 0; // start of line to draw, relative to tx
    unsigned width = m_logicalWidth; // how much line to draw
...
void InlineTextBox::paintCompositionLineThrough(GraphicsContext* ctx, const FloatPoint& boxOrigin, const CompositionLineThrough& lineThrough)
{
    int tx = boxOrigin.x();
    int ty = boxOrigin.y();
    tx += x();
    ...
    int baseline = renderer()->style(isFirstLineStyle())->font().fontMetrics().ascent();
