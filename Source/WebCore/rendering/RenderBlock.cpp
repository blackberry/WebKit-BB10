...
void RenderBlock::layoutBlock(bool relayoutChildren, LayoutUnit pageLogicalHeight)
{
    ...
    FrameView* frameView = this->document()->view();
    if (!frameView->fixedReportedSizeChanged() && !relayoutChildren && simplifiedLayout())
        return;
#else
...
#if ENABLE(FIXED_REPORTED_SIZE)
    // Positioned objects with height or width specified as a percentage need a layout using the new
    // fixed reported size to calculate the actual size they should now be
    if (frameView->fixedReportedSizeChanged())
        relayoutChildren = true;
#endif
...
void RenderBlock::paint(PaintInfo& paintInfo, const LayoutPoint& paintOffset)
{
    ...
    if (!isRoot() && !(hasOverflowClip() && hasLayer() && layer()->usesCompositedScrolling())) {
