...
public:
    ...
    void fitBelowFloats();
#if ENABLE(VIEWPORT_REFLOW)
    bool shouldReflow() const { return m_shouldReflow; }
    void setShouldReflow(bool shouldReflow) { m_shouldReflow = shouldReflow; }
    void shrinkAvailableWidthForReflowIfNeeded(float reflowWidth)
    {
        if (m_availableWidth > reflowWidth)
            m_availableWidth = reflowWidth;
    }
#endif
...
#if ENABLE(VIEWPORT_REFLOW)
        if (m_shouldReflow) {
            int reflowWidth = m_block->view()->frameView()->reflowWidth();
            reflowWidth -= 2 * 4; // Fixed margin
            if (reflowWidth && (newLineWidth + m_overhangWidth) > reflowWidth)
                newLineWidth = reflowWidth - m_overhangWidth;
        }
#endif
...
void RenderBlock::layoutInlineChildren(bool relayoutChildren, LayoutUnit& repaintLogicalTop, LayoutUnit& repaintLogicalBottom)
{
    ...
    bool hasTextOverflow = (style()->textOverflow() == TextOverflowEllipsis && hasOverflowClip())
        || (isAnonymousBlock() && parent() && parent()->isRenderBlock() && parent()->style()->textOverflow() == TextOverflowEllipsis && parent()->hasOverflowClip());
...
#if ENABLE(VIEWPORT_REFLOW)
    if (m_block->document()->settings() && m_block->document()->settings()->isTextReflowEnabled()) {
        FrameView* frameView = m_block->view()->frameView();

        // Use container node for anonymous RenderBlock, otherwise the block which has style "text-align:justify"
        // can be reflowed and the text will space out.
        Node* node = m_block->isAnonymous() ? m_block->container()->node() : m_block->node();

        // Cache shouldReflow for fitBelowFloats which can avoid calling flowTextToFit twice.
        width.setShouldReflow(frameView->flowTextToFit(node));
        if (width.shouldReflow()) {
            int reflowWidth = frameView->reflowWidth();
            reflowWidth -= 2 * 4; // Fixed margin
            if (reflowWidth)
                width.shrinkAvailableWidthForReflowIfNeeded(static_cast<float>(reflowWidth));
        }
    }
#endif
...
void RenderBlock::addOverflowFromInlineChildren()
{
    ...
    for (RootInlineBox* curr = firstRootBox(); curr; curr = curr->nextRootBox()) {
        addAbsoluteLayoutOverflow(curr->layoutOverflowRect(curr->lineTop(), curr->lineBottom()));
        addAbsoluteVisualOverflow(curr->visualOverflowRect(curr->lineTop(), curr->lineBottom()));
...
void RenderBlock::checkLinesForTextOverflow()
{
    ...
    String ellipsis = ellipsisStr;
    const Font& firstLineFont = firstLineStyle()->font();
    int firstLineEllipsisWidth = firstLineFont.width(constructTextRun(this, firstLineFont, ellipsis, firstLineStyle()));
    int ellipsisWidth = (font == firstLineFont) ? firstLineEllipsisWidth : font.width(constructTextRun(this, font, ellipsis, style()));
