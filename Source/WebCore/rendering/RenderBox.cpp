...
void RenderBox::styleDidChange(StyleDifference diff, const RenderStyle* oldStyle)
{
    ...
    if (isRootRenderer || isBodyRenderer) {
        ...
        // Need both rebuild and reevaluate flags set here since updateCompositingLayers() does not force
        // a rebuild when a RL does not change its compositing state. Needed in case of iframe since its root layer
        // stays composited regardless of fixed background.
        bool oldStyleHasFixedBackground = oldStyle && oldStyle->hasEntirelyFixedBackground();
        bool newStyleHasFixedBackground = style() && style()->hasEntirelyFixedBackground();
        if (oldStyleHasFixedBackground != newStyleHasFixedBackground) {
            RenderView * viewRenderer = view();
            if (viewRenderer) {
                viewRenderer->compositor()->setShouldReevaluateCompositingAfterLayout();
                viewRenderer->compositor()->setCompositingLayersNeedRebuild(true);
            }
        }
#endif
    ...
    LayoutUnit offsetFromLogicalTopOfFirstPage, bool checkForPerpendicularWritingMode) const
{
    ...
    if (frameView && !frameView->fixedReportedSize().isEmpty() && style()->position() == FixedPosition) {
        LayoutUnit widthResult = frameView->reportedWidth();
        widthResult -= (containingBlock->borderBefore() + containingBlock->borderAfter());
        return widthResult;
    }
#endif
...
LayoutUnit RenderBox::containingBlockLogicalHeightForPositioned(const RenderBoxModelObject* containingBlock, bool checkForPerpendicularWritingMode) const
{
    ...
    if (frameView && !frameView->fixedReportedSize().isEmpty() && style()->position() == FixedPosition) {
        LayoutUnit heightResult = frameView->reportedHeight();
...
void RenderBox::addVisualEffectOverflow()
{
    ...
    addAbsoluteVisualOverflow(IntRect(overflowMinX, overflowMinY, overflowMaxX - overflowMinX, overflowMaxY - overflowMinY));
...
void RenderBox::addOverflowFromChild(RenderBox* child, const LayoutSize& delta)
{
    ...
    LayoutRect absoluteChildLayoutOverflowRect = child->absoluteLayoutOverflowRect();
    ...
    LayoutRect absoluteChildVisualOverflowRect = child->absoluteVisualOverflowRect();
...
void RenderBox::addAbsoluteLayoutOverflow(const LayoutRect& rect)
{
    LayoutRect borderBox = borderBoxRect();
    ...
    if (!m_absoluteOverflow)
        m_absoluteOverflow = adoptPtr(new RenderOverflow(borderBox, borderBox));

    m_absoluteOverflow->addLayoutOverflow(rect);
}

void RenderBox::addAbsoluteVisualOverflow(const LayoutRect& rect)
{
    LayoutRect borderBox = borderBoxRect();
    if (borderBox.contains(rect))
        return;
