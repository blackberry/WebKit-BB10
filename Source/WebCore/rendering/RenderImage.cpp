...
bool RenderImage::setImageSizeForAltText(CachedImage* newImage /* = 0 */)
{
    ...
    if (!m_altText.isEmpty()) {
        ...
        // A 'theoretical' fix for email: if we're showing the alt text, it needs padding
        textSize.expand(paddingWidth, paddingHeight);
#endif
    ...
    if (cWidth > 0 && cHeight > 0) {
        ...
        if (shouldDrawBorderWhileLoading(this)) {
            ...
            drawBorder = (!m_imageResource->hasImage() || m_imageResource->errorOccurred()
                            ...
                            || !img || (img->isNull() && document()->settings() && !document()->settings()->loadsImagesAutomatically())
#endif
                         );
        }
    }
