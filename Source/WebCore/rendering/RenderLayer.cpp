...
// FIXME_agomes: Extend it to inner frames case?
...
void RenderLayer::updateLayerPositionsAfterScroll(UpdateLayerPositionsAfterScrollFlags flags)
{
    ...
    if ((flags & HasSeenViewportConstrainedAncestor) || renderer()->style()->hasViewportConstrainedPosition()) {
        ...
        // With fixed reported size, we may not just have moved, but also changed size, if
        // the fixed reported size changed. Schedule a layout, but only for the root of the
        // fixed positioned tree.
...
{
#if PLATFORM(BLACKBERRY)
    // FIXME_agomes: We differ from upstream here due to the following:
    // By the time the compositor checks if this overflow:scroll layer needs
    // to be composited, neither 'scrollbars' have not been created yet (so
    // 'allowsScrolling' fails) nor the layer has not yet been added to
    // FrameView's scrollable area set.
    // Later on, we crashes when we try to scroll since this method returns
    // true, and we try to use a LayerWebKitThread that was not created.
    // See https://bugs.webkit.org/show_bug.cgi?id=95494 .
    if (!scrollsOverflow())
#else
    ...
    return renderer()->style()->overflowScrolling() != OSAuto;
#else
    return false;
#endif
}
...
void RenderLayer::computeScrollDimensions()
{
    ...
    bool hasHorizontalOverflow = scrollWidth() > box->pixelSnappedClientWidth();
    bool hasVerticalOverflow = scrollHeight() > box->pixelSnappedClientHeight();
...
#if PLATFORM(BLACKBERRY)
    if (request.touchEvent() && !request.release() && !request.move()) {
        Node* newActiveNode = result.innerNode();
        if (newActiveNode && newActiveNode->isElementNode()) {
            Element* element = static_cast<Element*>(newActiveNode->shadowAncestorNode());
            if (!element)
                return;
            bool hasTouchHandler = false;
            while (!hasTouchHandler && newActiveNode) {
                hasTouchHandler = newActiveNode->willRespondToTouchEvents();
                newActiveNode = newActiveNode->parentNode();
            }

            if (!element->isFormControlElement() && !hasTouchHandler)
                return;
        } else
            return;
    }
...
#if USE(ACCELERATED_COMPOSITING)
#if PLATFORM(BLACKBERRY)
    // FIXME_agomes: Maybe this can be a setting?
    if (!hasOverflow)
        hasOverflow = usesCompositedScrolling();
#endif
#endif
...
#if USE(ACCELERATED_COMPOSITING)
#if PLATFORM(BLACKBERRY)
bool RenderLayer::allowsCompositingForFixedPosition() const
{
    return hasFixedContainer()
        || (renderer()->isOutOfFlowPositioned() && renderer()->style()->position() == FixedPosition);
}
#endif
#endif
