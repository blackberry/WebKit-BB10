...
void RenderLayer::updateNeedsCompositedScrolling()
{
    ...
    if (!frameView || !frameView->containsScrollableArea(this))
        ...
        m_needsCompositedScrolling = forceUseCompositedScrolling || renderer()->style()->overflowScrolling();
#else
...
#if USE(ACCELERATED_COMPOSITING)
        || renderer()->isRenderIFrame()
        || compositor()->needsFixedRootBackgroundLayer(this);
#else
        || renderer()->isRenderIFrame();
#endif
...
#if USE(ACCELERATED_COMPOSITING)
#if PLATFORM(BLACKBERRY) && ENABLE(ACCELERATED_OVERFLOW_SCROLLING)
    // FIXME_agomes: Maybe this can be a setting?
    if (!hasOverflow)
        hasOverflow = scrollsOverflow() && renderer()->style()->overflowScrolling();
#endif
#endif
