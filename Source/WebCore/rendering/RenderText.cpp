...
void RenderText::containerQuadsForRange(Vector<FloatQuad>& quads, RenderLayerModelObject* container, unsigned start, unsigned end, bool useSelectionHeight, bool* wasFixed)
{
    // Work around signed/unsigned issues. This function takes unsigneds, and is often passed UINT_MAX
    // to mean "all the way to the end". InlineTextBox coordinates are unsigneds, so changing this 
    // function to take ints causes various internal mismatches. But selectionRect takes ints, and 
    ...
    for (InlineTextBox* box = firstTextBox(); box; box = box->nextTextBox()) {
        ...
        if (start <= box->start() && box->end() < end) {
            ...
            quads.append(localToContainerQuad(r, container, 0, wasFixed));
        } else {
            FloatRect rect = localQuadForTextBox(box, start, end, useSelectionHeight);
            if (!rect.isZero())
                quads.append(localToContainerQuad(rect, container, 0, wasFixed));
        }
    }
}
