...
void RenderTextControlSingleLine::styleDidChange(StyleDifference diff, const RenderStyle* oldStyle)
{
    ...
    if (HTMLElement* placeholder = inputElement()->placeholderElement()) {
        int textOverflow = CSSValueClip;
        if (textShouldBeTruncated()) {
            if (style()->textOverflow() == TextOverflowEllipsis)
                textOverflow = CSSValueEllipsis;
            else
                textOverflow = CSSValueBlackberryFade;
        }
        placeholder->setInlineStyleProperty(CSSPropertyTextOverflow, textOverflow);
    }
...
LayoutUnit RenderTextControlSingleLine::computeControlHeight(LayoutUnit lineHeight, LayoutUnit nonContentHeight) const
{
    ...
    return max(lineHeight + nonContentHeight, innerTextElement()->renderBox()->height());
#else
...
PassRefPtr<RenderStyle> RenderTextControlSingleLine::createInnerTextStyle(const RenderStyle* startStyle) const
{
    ...
    textBlockStyle->setTextOverflow(textShouldBeTruncated() ? style()->textOverflow() : TextOverflowClip);
