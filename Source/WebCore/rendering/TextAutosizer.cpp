...
bool TextAutosizer::processSubtree(RenderObject* layoutRoot)
{
    ...
    if (windowInfo.windowSize.isEmpty()) {
        ...
        // In the BlackBerry port WebCore does not know about our device density
        // and so we must use screenRect to match up with the above comment.
        windowInfo.windowSize = flooredIntSize(screenRect(mainFrame->view()).size());
#else
...
#if PLATFORM(BLACKBERRY)
    // In the BlackBerry port, the web page can always be zoomed out to fit the content width
    // (if a viewport meta tag would prevent such zooming, we declare the viewport broken and
    // ignore it). For vertical writing mode, we want to use the viewport height.
    windowInfo.minLayoutSize.setWidth(std::min(mainFrame->view()->contentsWidth(), 1024));
#endif
#if ENABLE(VIEWPORT_REFLOW)
    if (m_document->settings()->isTextReflowEnabled())
        windowInfo.minLayoutSize.setWidth(mainFrame->view()->reflowWidth());
#endif
    for (Frame* frame = m_document->frame(); frame && frame != mainFrame; frame = frame->tree()->parent()) {
