...
#if USE(SKIA)
        // On the GPU we always want to clear the tile as it's free and allows for optimizations
...
#if !USE(SKIA)
    if (!previousContentsRects.empty()) {
        BlackBerry::Platform::Graphics::blitToBuffer(
            tile->backBuffer()->nativeBuffer(), tile->backBuffer()->rect(),
            tile->frontBuffer()->nativeBuffer(), tile->frontBuffer()->rect());
    }
#endif
}
