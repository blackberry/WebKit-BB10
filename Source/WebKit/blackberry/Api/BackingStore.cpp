...
void BackingStorePrivate::resumeScreenUpdates(BackingStore::ResumeUpdateOperation op)
{
    ...
    if (!m_client->contentsSize().isEmpty())
        m_webPage->client()->notifyPixelContentRendered(dstRect);
}
...
void BackingStorePrivate::repaint(const Platform::IntRect& windowRect, bool contentChanged, bool immediate)
{
    ...
    if (contentChanged && !windowRect.isEmpty()) {
        ...
        // Invalidate tiles in the repaint rect
        invalidateRect(rect);
...
void BackingStorePrivate::scrollingStartedHelper(const Platform::IntSize& delta)
{
    ...
    // The tile indexes might change after scrollBackingStore(), but the corresponding
    // render regions will not change. So we can save the regions of scroll jobs and
    // add them back to the render queue after scrollBackingStore().
    m_renderQueue->saveAndClearScrollRenderJobs();

    // Scroll the actual backingstore.
    scrollBackingStore(delta.width(), delta.height());

    // Add the saved render regions back to the scroll render jobs.
    m_renderQueue->restoreSavedScrollRenderJobs();
...
void BackingStorePrivate::resetTiles()
{
    ...
    m_invalidatedRegion = Platform::IntRectRegion();
...
void BackingStorePrivate::updateTilesForScrollOrNotRenderedRegion(bool checkLoading)
{
    ...
    for (TileMap::const_iterator it = currentMap.begin(); it != end; ++it) {
        ...
        if (geometry->isTileCorrespondingToBuffer(index, tileBuffer)) {
            ...
            if (m_invalidatedRegion.isRectInRegion(rect) != Platform::IntRectRegion::NotInRegion) {
                updateTile(tileOrigin, false /*immediate*/);
                continue;
            }
        }
        if ((isVisible || updateNonVisibleTiles)
            && (!tileBuffer || !tileBuffer->isRendered(tileVisibleContentsRect(index, geometry), geometry->scale()))
            && !isCurrentVisibleJob(index, geometry))
            updateTile(tileOrigin, false /*immediate*/);
    }
    m_invalidatedRegion = Platform::IntRectRegion::subtractRegions(m_invalidatedRegion, geometry->backingStoreRect());
}
...
void BackingStorePrivate::invalidateRect(const Platform::IntRect& rect)
{
    m_invalidatedRegion = Platform::IntRectRegion::unionRegions(m_invalidatedRegion, rect);
}
