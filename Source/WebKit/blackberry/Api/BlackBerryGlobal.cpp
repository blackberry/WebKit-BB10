...
void globalInitialize()
{
    ...
    int rc = credmgr_ClientInitialize(CREDMGR_CLIENT_LOG_TARGET_SLOG2_REUSE_BUFFER);
    if (rc != CREDMGR_SUCCESS)
        logAlways(LogLevelWarn, "credmgr_ClientInitialize failed: error 0x%04X", rc);
    else
        LegacyCredentialBackingStore::instance()->migrateCredentials();
}
