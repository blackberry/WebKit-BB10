...
void WebOverlayPrivate::drawContents(Platform::Graphics::Drawable* dst)
{
    if (!client)
        return;

    client->drawOverlayContents(q, dst);
}
...
void WebOverlayLayerCompositingThreadClient::setContents(const WebCore::Texture::HostType& contents)
{
    m_contents = contents;
    m_color = Color();
    m_texture.clear();
...
void WebOverlayLayerCompositingThreadClient::uploadTexturesIfNeeded(LayerCompositingThread*)
{
...
#else
    if (!m_contents && !m_color.isValid() && !m_drawsContent)
        return;
    ...
    if (m_drawsContent) {
        ...
        if (!m_contents) {
            m_contents = BlackBerry::Platform::Graphics::createBuffer(IntSize(0,0), BlackBerry::Platform::Graphics::TileBuffer);
        }

        clearBuffer(m_contents, 0, 0, 0, 0);
        PlatformGraphicsContext* pgc = lockBufferDrawable(m_contents);
        double transform[] = {
            1, 0,
            0, 1,
            -m_layerCompositingThread->bounds().width()/2.0, -m_layerCompositingThread->bounds().height()/2.0
        };
        pgc->setTransform(transform);
        m_client->drawOverlayContents(m_owner, pgc);
        releaseBufferDrawable(m_contents);
    }

    m_texture = Texture::create();
    IntRect bitmapRect(0, 0, m_layerCompositingThread->bounds().width(), m_layerCompositingThread->bounds().height());
    m_uploadedContents = m_contents;
    m_texture->updateContents(m_uploadedContents, bitmapRect, bitmapRect, false);
    m_contents = 0;
#endif
...
void WebOverlayLayerCompositingThreadClient::drawTextures(LayerCompositingThread* layer, double scale, int positionLocation, int texCoordLocation)
{
    ...
    dt.scale(layer->sizeIsScaleInvariant() ? 1.0 / scale : 1.0);
    blitToBuffer(0, m_texture->textureId(), reinterpret_cast<BlackBerry::Platform::TransformationMatrix&>(dt),
                 BlackBerry::Platform::Graphics::SourceOver, static_cast<unsigned char>(m_layerCompositingThread->drawOpacity() * 255));
#endif
}
...
#else
    BlackBerry::Platform::Graphics::Buffer* buffer = createBuffer(IntSize(0,0), BlackBerry::Platform::Graphics::TileBuffer);
    if (!buffer)
        return;
    PlatformGraphicsContext* platformContext = lockBufferDrawable(buffer);
    BlackBerry::Platform::Graphics::TiledImage image(imageSize, (unsigned*)data);
    IntSize bounds = m_layerCompositingThread->bounds();
    FloatRect dst(-bounds.width()/2.0, -bounds.height()/2.0, bounds.width(), bounds.height());
    FloatRect src(0, 0, imageSize.width(), imageSize.height());
    platformContext->addImage(dst, src, &image);
    releaseBufferDrawable(buffer);

    m_layerCompositingThreadClient->setContents(buffer);
#endif
