...
void WebOverlayLayerCompositingThreadClient::setContentsToImage(const ImageType& image)
{
    m_image = image;
    m_color = Color();
    m_texture.clear();
...
void WebOverlayLayerCompositingThreadClient::uploadTexturesIfNeeded(LayerCompositingThread* layer)
{
    ...
    if (m_drawsContent) {
...
#if USE(SKIA)
        textureContents.setConfig(SkBitmap::kARGB_8888_Config, layer->bounds().width(), layer->bounds().height());
        textureContents.allocPixels();

        SkDevice device(textureContents);
        ...
        PlatformGraphicsContext* pgc = lockBufferDrawable(textureContents);
        double transform[] = {
            1, 0,
            0, 1,
...
#if USE(SKIA)
    m_texture->protect(textureSize);
#else
    // For GL renderer, we don't want to force backing
...
void WebOverlayLayerCompositingThreadClient::drawTextures(LayerCompositingThread* layer, double scale, const GLES2Program& program)
{
    ...
    if (!m_image.isNull()) {
        ...
        dt.scaleNonUniform(static_cast<double>(layer->bounds().width()) / m_image.width(), static_cast<double>(layer->bounds().height()) / m_image.height());
    }
...
void WebOverlayPrivateCompositingThread::setContentsToImage(const unsigned char* data, const IntSize& imageSize, WebOverlay::ImageDataAdoptionType adoptionType)
{
    ...
    WebOverlayLayerCompositingThreadClient::ImageType image;

#if USE(SKIA)
    const SkBitmap& oldImage = m_layerCompositingThreadClient->image();
    if (!oldImage.isNull()) {
        SkAutoLockPixels lock(oldImage);
        if (data == oldImage.getPixels())
            return;
    }
    ...
    case WebOverlay::ReferenceImageData:
        image.setPixels(const_cast<unsigned char*>(data));
        break;
    case WebOverlay::CopyImageData:
        if (image.allocPixels()) {
            ...
            memcpy(image.getPixels(), data, bytes);
            image.unlockPixels();
        }
        break;
    default:
    ...
    image = BlackBerry::Platform::Graphics::TiledImage(imageSize, reinterpret_cast_ptr<const unsigned*>(data), true, BlackBerry::Platform::Graphics::TiledImage::Hardware);
#endif
