...
public:
    // ImageType needs to be something we can discard when backgrounded and then reupload it to the GPU when returning to foreground.
#if USE(SKIA)
    typedef SkBitmap ImageType;
#else
    typedef BlackBerry::Platform::Graphics::TiledImage ImageType;
#endif
    ...
    void setContentsToImage(const ImageType&);
    void setContentsToColor(const WebCore::Color&);

    const ImageType& image() const { return m_image; }
