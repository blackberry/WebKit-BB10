...
void WebPagePrivate::download(Frame* frame, const Platform::NetworkRequest& platformRequest, PreDownloadHandlingPolicy policy, bool needReferer)
{
    ...
    download(frame, request, policy);
}

void WebPagePrivate::download(Frame* frame, const ResourceRequest& request, PreDownloadHandlingPolicy policy)
{
    NetworkingContext* context = frame->loader()->networkingContext();
    OwnPtr<WebPagePreDownloadClient> preclient = adoptPtr(new WebPagePreDownloadClient(predownloadManager()));
    ...
    ASSERT(job->mutableRequest());
    if (job->mutableRequest())
        job->mutableRequest()->setTargetType(Platform::NetworkRequest::TargetIsDownload);
    else
        Platform::logAlways(Platform::LogLevelCritical, "WebPagePrivate::download(): no mutable NetworkRequest for %s.", request.url().string().utf8().data());

    preclient->setResourceHandle(handle);
    if (policy == DownloadDirectly)
        m_client->downloadRequested(job->wrappedStream(), Platform::String(request.suggestedSaveName()));
...
void WebPagePrivate::load(const Platform::NetworkRequest& netReq, bool needReferer)
{
    ...
    if (Platform::NetworkRequest::TargetIsDownload == netReq.getTargetType()) {
        download(m_mainFrame, netReq, DownloadWithRedirectAndAuth, needReferer);
        return;
    }
...
void WebPagePrivate::notifyAdjustedZoom(const Platform::FloatPoint& newPos, double newZoom)
{
    m_client->notifyAdjustedZoom(newPos, newZoom);
}
...
void WebPagePrivate::authenticationChallenge(const KURL& url, const ProtectionSpace& protectionSpace, const Credential& inputCredential)
{
    ...
    credentialManager().autofillAuthenticationChallenge(this, protectionSpace, username, password);
#endif
...
Platform::WebContext WebPagePrivate::webContext(TargetDetectionStrategy strategy)
{
    ...
    if (node->isElementNode()) {
        ...
        if (DOMSupport::isTextBasedContentEditableElement(element)
            || (!element->isEnabledFormControl() && (m_currentContextNode->hasTagName(HTMLNames::textareaTag) || (m_currentContextNode->hasTagName(HTMLNames::inputTag) && m_currentContextNode->toInputElement()->isTextField())))) {
...
void WebPage::adjustDocumentZoomPosition(const Platform::FloatPoint& documentScrollPosition, double zoomLevel)
{
   notImplemented();
}

void WebPage::notifyAdjustedZoom(const Platform::FloatPoint& newPos, double newZoom)
{
    d->notifyAdjustedZoom(newPos, newZoom);
}
