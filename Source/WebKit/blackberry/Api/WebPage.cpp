...
#if ENABLE(ACCELERATED_2D_CANVAS) && USE(SKIA)
#include "GrContext.h"
...
#if ENABLE(REGISTER_PROTOCOL_HANDLER) || ENABLE(CUSTOM_SCHEME_HANDLER)
   WebCore::provideRegisterProtocolHandlerTo(m_page, new RegisterProtocolHandlerClientBlackBerry(this));
#endif
    ...
    if (!localFileSystemInitialized) {
        ...
        // Make sure the path passed to this call exist except the deepest component.
        // FIXME: set the path value to the desired one. Maybe a setting is needed.
        WebCore::LocalFileSystem::initializeLocalFileSystem(String::fromUTF8((BlackBerry::Platform::Settings::instance()->applicationDataDirectory() + "/webfs").c_str()));
    }
    m_page->settings()->setFileSystemAPISandboxed(m_webSettings->fileSystemAPISandboxed());
#endif
...
void WebPagePrivate::loadString(const char* string, size_t len, const char* baseURL, const char* contentType, const char* failingURL)
{
    KURL kurl = parseUrl(baseURL);
    ResourceRequest request(kurl);
    WTF::RefPtr<SharedBuffer> buffer
        = SharedBuffer::create(string, len);
...
void WebPage::loadString(const char* string, size_t len, const char* baseURL, const char* mimeType, const char* failingURL)
{
    d->loadString(string, len, baseURL, mimeType, failingURL);
}
...
void WebPagePrivate::authenticationChallenge(const KURL& url, const ProtectionSpace& protectionSpace, const Credential& inputCredential, AuthenticationChallengeClient* client)
{
    ...
    bool isConfirmed = m_client->authenticationChallenge(protectionSpace.realm().characters(), protectionSpace.realm().length(), username, password, protectionSpace.isProxy());
...
static IntRect getNodeWindowRect(Node* node)
{
    if (Frame* frame = frameForNode(node)) {
        if (FrameView* view = frame->view())
            return view->contentsToWindow(IntRect(node->getRect()));
    }
    ASSERT_NOT_REACHED();
    return IntRect();
}
...
IntRect WebPagePrivate::getRecursiveVisibleWindowRect(ScrollView* view, bool noClipOfMainFrame)
{
    ...
    IntRect visibleWindowRect(view->contentsToWindow(IntRect(view->visibleContentRect(false))));
...
void WebPage::selectAtPoint(const Platform::IntPoint& location, SelectionExpansionType selectionExpansionType)
{
    if (d->m_page->defersLoading())
        return;
    ...
    d->m_selectionHandler->selectAtPoint(selectionLocation, selectionExpansionType);
}
...
void WebPage::setExtraPluginDirectory(const std::string& path)
{
    PluginDatabase* database = PluginDatabase::installedPlugins(true);
...
#if ENABLE(FILE_SYSTEM)
    coreSettings->setFileSystemAPISandboxed(webSettings->fileSystemAPISandboxed());
#endif
