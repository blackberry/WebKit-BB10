    ...
    if (node->isHTMLElement()) {
        ...
        if (imageElement && imageElement->renderer()) {
            ...
            if (CachedImage* cachedImage = imageElement->cachedImage()) {
                if (cachedImage->isLoaded() && cachedImage->CachedResource::data()) {
...
bool WebPagePrivate::handleMouseEvent(PlatformMouseEvent& mouseEvent)
{
    ...
    if (!node) {
        ...
        HitTestResult result = eventHandler->hitTestResultAtPoint(documentContentsPoint, false /*allowShadowContent*/);
...
void WebPage::setOverlayExpansionPixelHeight(const int dy)
{
    d->setOverlayExpansionPixelHeight(dy);
}

void WebPagePrivate::setOverlayExpansionPixelHeight(const int dy)
{
