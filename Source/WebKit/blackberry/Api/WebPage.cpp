...
#include <BlackBerryPlatformRawTouchEvent.h>
...
#if DEBUG_ZOOM
#define ZoomLog(...) Platform::logAlways(Platform::LogLevelCritical, "ZoomLog: " __VA_ARGS__)
#define BlockZoomLog(...) Platform::logAlways(Platform::LogLevelCritical, "ZoomLog:     " __VA_ARGS__)
namespace BlackBerry {
namespace WebKit {
static WTF::String toString(WebCore::Node*);
}
}
#else
#define ZoomLog(...)
...
STATIC_GLOBAL_STRING(s_get, "GET");
...
const int blockZoomMargin = 3; // Add 3 pixel margin on each side.
...
const double minimumScaleRatio = 0.1; // A relative change lower than this is not considered actionable
...
const double baseFontSize = 8; // The system default font size, in the same units as Platform::FontInfo uses (pt)
#endif

static double relativeChange(double value, double reference)
{
...
WebPagePrivate::WebPagePrivate(WebPage* webPage, WebPageClient* client, const IntRect& rect)
    ...
    , m_zoomViewportAccessor(new ZoomViewportAccessor(this))
    ...
    , m_pendingBlockZoom(false)
    , m_zoomFactorAtViewportChange(1.0)
    , m_fullscreenNode(0)
...
#endif
    , m_accessibilityFunctionHandler(new WebAccessibilityFunctionHandler(this))
    , m_redrawSpellCheckDialogTimer(this, &WebPagePrivate::redrawSpellCheckDialogTimerFired)
    , m_suspendAnimationCauses(SuspendByHidingPage)
{
...
#if ENABLE(TEXT_AUTOSIZING)
    Platform::FontInfo::instance()->addListener(this);
#endif
}
...
{
#if ENABLE(TEXT_AUTOSIZING)
    Platform::FontInfo::instance()->removeListener(this);
#endif
    ...
    delete m_accessibilityFunctionHandler;
    m_accessibilityFunctionHandler = 0;
    ...
    delete m_zoomViewportAccessor;
    m_zoomViewportAccessor = 0;
...
void WebPagePrivate::init(const BlackBerry::Platform::String& pageGroupName)
{
...
#if ENABLE(TEXT_AUTOSIZING)
    m_page->settings()->setTextAutosizingFontScaleFactor(Platform::FontInfo::instance()->fontSize() / baseFontSize);
#endif
    ...
    m_page->settings()->setShouldDisplaySubtitles(true);
    m_page->settings()->setShouldDisplayCaptions(true);
    m_page->settings()->setShouldDisplayTextDescriptions(true);
#endif
...
WebPagePreDownloadManager* WebPagePrivate::predownloadManager()
{
    if (!m_predownloadManager)
        m_predownloadManager = adoptPtr(new WebPagePreDownloadManager(client()));

    return m_predownloadManager.get();
}

WebAccessibilityFunctionHandler* WebPage::accessibilityFunctionHandler()
{
    return d->m_accessibilityFunctionHandler;
}
...
void initializeResourceRequestFromPlatformRequest(ResourceRequest& request, Frame* frame, const Platform::NetworkRequest& platformRequest, bool needReferrer)
{
    request.setHTTPMethod(platformRequest.getMethodRef());
    request.setCachePolicy(toWebCoreCachePolicy(platformRequest.getCachePolicy()));
    if (!platformRequest.getOverrideContentType().empty())
        request.setOverrideContentType(platformRequest.getOverrideContentType());

    const Platform::NetworkRequest::HeaderList& list = platformRequest.getHeaderListRef();
    for (unsigned i = 0; i < list.size(); ++i)
        request.addHTTPHeaderField(list[i].first.c_str(), list[i].second.c_str());

    if (needReferrer && frame && frame->document())
        request.addHTTPHeaderField("Referer", frame->document()->url().string().utf8().data());

    if (!platformRequest.getSuggestedSaveName().empty())
        request.setSuggestedSaveName(platformRequest.getSuggestedSaveName());
}

unsigned WebPagePrivate::download(Frame* frame, const Platform::NetworkRequest& platformRequest, PreDownloadHandlingPolicy policy, unsigned parentDownloadId, WebPagePreDownloadListener* listener, bool needReferer, bool confirmed)
{
    String urlString(platformRequest.getUrlRef());
    KURL kurl = parseUrl(urlString);
    ResourceRequest request(kurl);
    initializeResourceRequestFromPlatformRequest(request, frame, platformRequest, needReferer);
    request.setTargetType(ResourceRequest::TargetIsDownload);
    return download(frame, request, policy, parentDownloadId, listener, confirmed);
}

unsigned WebPagePrivate::download(Frame* frame, const ResourceRequest& request, PreDownloadHandlingPolicy policy, unsigned parentDownloadId, WebPagePreDownloadListener* listener, bool confirmed)
{
    BLACKBERRY_ASSERT(request.targetType() == ResourceRequest::TargetIsDownload);

    Platform::FilterStream* stream;
    NetworkingContext* context = frame->loader()->networkingContext();
    OwnPtr<WebPagePreDownloadClient> preclient = adoptPtr(new WebPagePreDownloadClient(predownloadManager(), listener, parentDownloadId, confirmed));
    RefPtr<ResourceHandle> handle = ResourceHandle::create(context, request, preclient.get(), false /*defersLoading*/, false /*shouldContentSniff*/);
    NetworkJob* job = NetworkManager::instance()->findJobForHandle(handle);
    if (!job && request.url().protocol() != "blob") {
        Platform::logAlways(Platform::LogLevelCritical, "WebPagePrivate::download(): failed to create download job: %s.", request.url().string().utf8().data());
        return 0;
    }

    if (job)
        stream = job->wrappedStream();
    else
        stream = new BlobStream(request, handle.get());
    preclient->setResourceHandle(handle);

    unsigned id = 0;
    if (policy == DownloadDirectly || request.url().protocol() == "blob")
        id = m_client->downloadRequested(stream, Platform::String(request.suggestedSaveName()), parentDownloadId, confirmed);

    predownloadManager()->addPreDownloadClient(preclient.leakPtr());
    return id;
}
...
void WebPagePrivate::load(const Platform::NetworkRequest& netReq, bool needReferer)
{
...
#if ENABLE(VIDEO)
    m_page->settings()->setMediaEnabled(BlackBerry::Platform::Settings::instance()->isMediaEnabled());
#endif

    if (Platform::NetworkRequest::TargetIsDownload == netReq.getTargetType()) {
        download(m_mainFrame, netReq, DownloadWithRedirectAndAuth, 0 /* parentDownloadId */, 0 /* WebPagePreDownloadListener* */, needReferer, false /* confirmed */);
        return;
    }
    ...
    ResourceRequest request(kurl);
    initializeResourceRequestFromPlatformRequest(request, m_mainFrame, netReq, needReferer);
...
void WebPage::loadFile(const BlackBerry::Platform::String& path, const BlackBerry::Platform::String& overrideContentType)
{
    ...
    Platform::NetworkRequest netRequest(fileUrl);
...
{
#if 0 // FIXME: https://bugzilla.qnx.com/bugzilla/show_bug.cgi?id=225419
...
void WebPagePrivate::notifyAdjustedScaleAndScrollPosition(const Platform::FloatPoint& newDocumentScrollPosition, double newScale)
{
    m_client->notifyAdjustedScaleAndScrollPosition(newDocumentScrollPosition, newScale);
}
...
{
    if (!isRestoringZoomLevel && !m_pendingBlockZoom) {
    ...
    // Now just because we got here doesn't mean we need to reflow text (i.e. setNeedsLayout).
    // Relayout is only needed if we toggled reflow, or reflow is enabled and the actual visible width changed.
    // The latter case is not handled here - the actual visible width changes when the scale changes or when
    // setViewportSize is called.
    // Scale changes further down in this method.
#if ENABLE(VIEWPORT_REFLOW)
    if (toggleTextReflowIfEnabledForBlockZoomOnly(m_shouldReflowBlock))
        setNeedsLayout();
#endif
...
#if ENABLE(VIEWPORT_REFLOW)
    // Relayout is only needed if we toggled reflow, or reflow is enabled and the actual visible width changed.
    // Here we know that the actual visible width changed due to a scale change.
    if (m_page->settings()->isTextReflowEnabled())
        setNeedsLayout();
#endif
    ...
    *m_transformationMatrix = zoom;
    ...
    if (!updateScrollPositionAfterReflow(anchor, inverseScale)) {
        setScrollPosition(IntPoint(max(0, static_cast<int>(roundf(anchor.x() - anchorOffset.x() / inverseScale))),
...
IntPoint WebPagePrivate::calculateReflowedScrollPosition(const FloatPoint& anchorOffset, double inverseScale)
{
    ...
    ZoomLog("WebPagePrivate::calculateReflowedScrollPosition(anchorOffset = %s, inverseScale = %f)", Platform::FloatPoint(anchorOffset).toString().c_str(), inverseScale);

    ZoomLog("WebPagePrivate::calculateReflowedScrollPosition: m_currentPinchZoomNode = %s", toString(m_currentPinchZoomNode.get()).latin1().data());
    ZoomLog("WebPagePrivate::calculateReflowedScrollPosition: m_anchorInNodeRectRatio = %s", Platform::FloatPoint(m_anchorInNodeRectRatio).toString().c_str());
    ...
    if (m_currentPinchZoomNode->renderer() && m_anchorInNodeRectRatio.y() >= 0) {
        ...
        if (m_currentPinchZoomNode->renderer()->isImage() && m_anchorInNodeRectRatio.x() > 0)
            ...
            offsetX = -blockZoomMargin;
    }

    IntRect reflowedRect = adjustRectOffsetForFrameOffset(nodeRect, m_currentPinchZoomNode.get());

    ZoomLog("WebPagePrivate::calculateReflowedScrollPosition: reflowedRect.location = %s, offset = (%d, %d)", Platform::IntPoint(reflowedRect.location()).toString().c_str(), offsetX, offsetY);
...
void WebPagePrivate::layoutFinished()
{
    ...
    m_inputHandler->spellingHandler()->stopPendingSpellCheckingRequests(true /* isRestartRequired */);
...
double WebPagePrivate::zoomToFitScale() const
{
    ...
    double zoomToFitScale = contentWidth > 0.0 ? static_cast<double>(m_zoomViewportAccessor->preferredSize().width()) / contentWidth : 1.0;
...
{
    m_backingStoreClient->notifyTransformedScrollChanged();
}

void WebPagePrivate::scrollChanged()
{
...
bool WebPagePrivate::setViewMode(ViewMode mode)
{
    ...
    if (m_nestedLayoutFinishedCount) {
        double widthChange = relativeChange(newSize.width(), currentSize.width());
        double heightChange = relativeChange(newSize.height(), currentSize.height());
...
PageClientBlackBerry::AuthDialogResult WebPagePrivate::authenticationChallenge(const KURL& url, const ProtectionSpace& protectionSpace, Credential& credential)
{
...
#if !defined(PUBLIC_BUILD) || !PUBLIC_BUILD
    if (m_dumpRenderTree)
        return m_dumpRenderTree->didReceiveAuthenticationChallenge(credential) ? AuthDialogConfirmed : AuthDialogCancelled;
#endif

    WebPageClient::AuthDialogResult result = m_client->authenticationChallenge(protectionSpace.realm().characters(), protectionSpace.realm().length(), username, password, requestURL, protectionSpace.isProxy());
    if (result != WebPageClient::AuthDialogNotShown)
        credential = Credential(username, password, CredentialPersistenceForSession);
    switch (result) {
    case WebPageClient::AuthDialogNotShown:
        return PageClientBlackBerry::AuthDialogNotShown;
    case WebPageClient::AuthDialogConfirmed:
        return PageClientBlackBerry::AuthDialogConfirmed;
    case WebPageClient::AuthDialogCancelled:
        return PageClientBlackBerry::AuthDialogCancelled;
    }
    BLACKBERRY_ASSERT_NOT_REACHED();
    return PageClientBlackBerry::AuthDialogNotShown;
}

PageClientBlackBerry::SaveCredentialDialogResult WebPagePrivate::notifyShouldSaveCredential(bool isNew)
{
    WebPageClient::SaveCredentialDialogResult result = m_client->notifyShouldSaveCredential(isNew);
    switch (result) {
    case WebPageClient::SaveCredentialDialogNotShown:
        return PageClientBlackBerry::SaveCredentialDialogNotShown;
    case WebPageClient::SaveCredentialDialogNeverForThisSite:
        return PageClientBlackBerry::SaveCredentialDialogNeverForThisSite;
    case WebPageClient::SaveCredentialDialogNotNow:
        return PageClientBlackBerry::SaveCredentialDialogNotNow;
    case WebPageClient::SaveCredentialDialogYes:
        return PageClientBlackBerry::SaveCredentialDialogYes;
    }
    BLACKBERRY_ASSERT_NOT_REACHED();
    return PageClientBlackBerry::SaveCredentialDialogNotShown;
}

bool WebPagePrivate::isCredentialAutofillEnabled() const
{
    return m_webSettings->isCredentialAutofillEnabled();
}

bool WebPagePrivate::isFormAutofillEnabled() const
{
    return m_webSettings->isFormAutofillEnabled();
}

bool WebPagePrivate::isPrivateBrowsingEnabled() const
{
    return m_webSettings->isPrivateBrowsingEnabled();
}
...
Platform::WebContext WebPagePrivate::webContext(TargetDetectionStrategy strategy)
{
    ...
    RefPtr<Node> node = contextNode(strategy, isContentEditable);
    ...
    if (!m_currentContextNode->dispatchMouseEvent(mouseEvent, eventNames().contextmenuEvent, 0)
        && !m_currentContextNode->hasTagName(HTMLNames::textareaTag)
        && !(m_currentContextNode->hasTagName(HTMLNames::inputTag) && m_currentContextNode->toInputElement()->isTextField())) {
    ...
    if (node->isHTMLElement()) {
        ...
        if (imageElement && imageElement->renderer()) {
            ...
            if (CachedImage* cachedImage = imageElement->cachedImage()) {
                if (cachedImage->isLoaded() && cachedImage->CachedResource::resourceBuffer()) {
    ...
    bool isUserSelectNone = DOMSupport::isUserSelectNone(node.get());
    bool canStartSelection = node->canStartSelection() && !isUserSelectNone;

    if (node->isElementNode()) {
        Element* element = toElement(node->deprecatedShadowAncestorNode());

        bool isInput = DOMSupport::isTextBasedContentEditableElement(element);
        bool isDisabledOrReadOnlyInput = false;
        if (!isInput) {
            HTMLTextFormControlElement* textFormControlElement = toHTMLTextFormControlElement(element);
            if (textFormControlElement)
                isDisabledOrReadOnlyInput = textFormControlElement->isDisabledOrReadOnly();
        }
        if (isInput || isDisabledOrReadOnlyInput) {
            if (!canStartSelection) {
                ...
                if (nodeUnderFinger)
                    canStartSelection = nodeUnderFinger->canStartSelection() && !isUserSelectNone;
            }
            if (isInput)
                context.setFlag(Platform::WebContext::IsInput);
            else
                context.setFlag(Platform::WebContext::IsDisabledOrReadOnlyInput);
            ...
            if (!elementText.stripWhiteSpace().isEmpty() && canStartSelection)
...
PassRefPtr<Node> WebPagePrivate::contextNode(TargetDetectionStrategy strategy, bool &isContentEditable)
{
...
#if DEBUG_ZOOM
static HTMLElement* ancestorElement(Node* node)
{
    if (!node)
        return 0;
...
static String toString(Node* node)
{
    ...
    if (node && node->isHTMLElement())
        s = toHTMLElement(node)->outerHTML();
    else {
        if (HTMLElement* element = ancestorElement(node))
            s = element->innerHTML();
    }
...
Node* WebPagePrivate::bestNodeForZoomUnderPoint(const IntPoint& documentPoint)
{
    ...
    node = node ? adjustedBlockZoomNodeForZoomAndExpandingRatioLimits(node) : adjustedBlockZoomNodeForZoomAndExpandingRatioLimits(originalNode);
    BlockZoomLog("WebPagePrivate::bestNodeForZoomUnderPoint returns %s", toString(node).latin1().data());
    return node;
}
...
Node* WebPagePrivate::adjustedBlockZoomNodeForZoomAndExpandingRatioLimits(Node* node)
{
    ...
    RenderObject* renderer = node->renderer();
    bool acceptableNodeSize = newScaleForBlockZoomRect(rectForNode(node), 0) < maximumScale();
    double unzoomableWidth = m_zoomViewportAccessor->preferredSize().width() / m_zoomViewportAccessor->scale();
    ...
    while (!renderer || !acceptableNodeSize) {
        ...
        if (!node || relativeChange(nodeRect.width(), unzoomableWidth) < minimumExpandingRatio)
            return initialNode;

        renderer = node->renderer();
        acceptableNodeSize = newScaleForBlockZoomRect(rectForNode(node), 0) < maximumScale();
    }
...
double WebPagePrivate::newScaleForBlockZoomRect(const IntRect& rect, double margin)
{
    if (rect.isEmpty())
    ...
    BlockZoomLog("WebPagePrivate::newScaleForBlockZoomRect preferred width %d, rect.width %d, margin %f", m_zoomViewportAccessor->preferredSize().width(), rect.width(), margin);
    double newScale = static_cast<double>(m_zoomViewportAccessor->preferredSize().width()) / (rect.width() + margin);
    BlockZoomLog("WebPagePrivate::newScaleForBlockZoomRect returns %f", newScale);
    return newScale;
}
...
IntRect WebPagePrivate::rectForNode(Node* node)
{
    ...
    if (m_webPage->settings()->textReflowMode() == WebSettings::TextReflowEnabled && renderer->isText()) {
        ...
        int xOffset = 0;
        int yOffset = 0;
        while (renderBlock && !renderBlock->isRoot()) {
        ...
        reflowedBlockRect = IntRect(xOffset + linesBox.x(), yOffset + linesBox.y(), linesBox.width(), linesBox.height());

        ZoomLog("WebPagePrivate::rectForNode: reflowedBlockRect = %s", Platform::IntRect(reflowedBlockRect).toString().c_str());
    }

    blockRect = IntRect(renderer->absoluteClippedOverflowRect());

    if (renderer->isText()) {
        if (RenderBlock* rb = renderer->containingBlock()) {
    ...
    ZoomLog("WebPagePrivate::rectForNode: blockRect = %s", Platform::IntRect(blockRect).toString().c_str());

    if (reflowedBlockRect.isEmpty())
        return blockRect;

    // A heuristic to avoid random line width differences cause reflow when double tapping sequential text columns
    return relativeChange(reflowedBlockRect.width(), blockRect.width()) > minimumScaleRatio ? reflowedBlockRect : blockRect;
}
...
IntRect WebPagePrivate::blockZoomRectForNode(Node* node)
{
    ...
    ZoomLog("WebPagePrivate::blockZoomRectForNode: m_currentBlockZoomAdjustedNode = %s", toString(m_currentBlockZoomAdjustedNode.get()).latin1().data());
    ...
    if (!tnode->hasTagName(HTMLNames::imgTag) && !tnode->hasTagName(HTMLNames::inputTag) && !tnode->hasTagName(HTMLNames::textareaTag)) {
        while ((tnode = tnode->parentNode())) {
            ...
            IntSize actualSize = m_zoomViewportAccessor->documentViewportSize();
    ...
    const ZoomViewportAccessor* viewportAccessor = m_zoomViewportAccessor;

    blockRect = adjustRectOffsetForFrameOffset(blockRect, node);
    blockRect.intersect(viewportAccessor->documentContentsRect());
...
void WebPagePrivate::zoomAnimationFinished(double finalAnimationScale, const WebCore::FloatPoint& finalAnimationDocumentScrollPosition, bool shouldConstrainScrollingToContentEdge)
{
    ...
    TransformationMatrix zoom;
    double inverseScale = finalAnimationScale / m_transformationMatrix->m11();
...
#if ENABLE(VIEWPORT_REFLOW)
    // Relayout is only needed if we toggled reflow, or reflow is enabled and the actual visible width changed. In this
    // case we know that the actual visible width changes, so go ahead and relayout if text reflow is enabled as well.
    if (toggleTextReflowIfEnabledForBlockZoomOnly(m_shouldReflowBlock) || m_page->settings()->isTextReflowEnabled()) {
        setNeedsLayout();
        layoutIfNeeded();
    }
#endif
...
bool WebPagePrivate::updateScrollPositionAfterReflow(const FloatPoint& anchor, double inverseScale)
{
#if ENABLE(VIEWPORT_REFLOW)
    if (!m_pendingBlockZoom) {
        bool didScroll = false;

        // Even if there was no block zoom, we try to preserve what the user was looking at when reflow is always-on
        if (m_webPage->settings()->textReflowMode() == WebSettings::TextReflowEnabled) {
            ...
            if (m_currentPinchZoomNode) {
                ZoomLog("WebPagePrivate::updateScrollPositionAfterReflow: scrollPosition = %s", Platform::IntPoint(scrollPosition()).toString().c_str());
                // Calling code would setNeedsLayout() if appropriate, now we need to layout for the calculations to
                // use up-to-date metrics.
                layoutIfNeeded();
                FloatPoint anchorOffset = FloatPoint() + (anchor - scrollPosition());
                IntPoint scrollPosition = calculateReflowedScrollPosition(anchorOffset, m_zoomViewportAccessor->scale() == minimumScale() ? 1.0 : inverseScale);
                ZoomLog("WebPagePrivate::updateScrollPositionAfterReflow: reflowed scrollPosition = %s", Platform::IntPoint(scrollPosition).toString().c_str());
    ...
    if (!m_currentBlockZoomAdjustedNode)
        return false;

    if (m_webPage->settings()->textReflowMode() == WebSettings::TextReflowDisabled)
        return false;

    ZoomLog("WebPagePrivate::updateScrollPositionAfterReflow: willUseTextReflow = %d, m_shouldReflowBlock = %d", true, m_shouldReflowBlock);
    if (m_shouldReflowBlock) {
        ...
        if (renderer && renderer->isText()) {
            ...
            ZoomLog("WebPagePrivate::updateScrollPositionAfterReflow: scrollPosition = %s @line %d", Platform::IntPoint(textAnchor).toString().c_str(), __LINE__);
            setScrollPosition(textAnchor);
        } else {
            IntPoint scrollPosition = renderer->style()->isLeftToRightDirection()
                ? topLeftPoint
                : IntPoint(reflowedRect.x() + reflowedRect.width() - actualVisibleSize().width(), topLeftPoint.y());
            ZoomLog("WebPagePrivate::updateScrollPositionAfterReflow: scrollPosition = %s @line %d", Platform::IntPoint(scrollPosition).toString().c_str(), __LINE__);
            setScrollPosition(scrollPosition);
        }
        scrollChanged();
    } else {
        ZoomLog("WebPagePrivate::updateScrollPositionAfterReflow: m_currentBlockZoomAdjustedNode = %s", toString(m_currentBlockZoomAdjustedNode.get()).latin1().data());
        ...
        IntPoint scrollPosition = IntPoint(0, finalRect.y() + m_finalAnimationDocumentScrollPositionReflowOffset.y());
        ZoomLog("WebPagePrivate::updateScrollPositionAfterReflow: scrollPosition = %s @line %d", Platform::IntPoint(scrollPosition).toString().c_str(), __LINE__);
        setScrollPosition(scrollPosition);
        scrollChanged();
        resetBlockZoom();
...
void WebPagePrivate::resetBlockZoom()
{
    m_currentBlockZoomNode = 0;
    m_currentBlockZoomAdjustedNode = 0;
    ZoomLog("WebPagePrivate::resetBlockZoom: m_currentBlockZoomAdjustedNode = (null)");
    m_shouldReflowBlock = false;
}
...
void WebPage::destroy()
{
    ...
    // Ensure that localstorage has synced to disk
...
unsigned WebPage::savePage(const Platform::String& filename)
{
    return WebPageSaver::saveCurrentPage(d, filename);
}
...
{
    d->suspendAnimations(WebPagePrivate::SuspendBySystem);
}

void WebPagePrivate::suspendAnimations(enum SuspendAnimationCause cause)
{
    // Ensure there is a single cause. Use Wegner's bit-counting method.
    ASSERT(cause && !(cause & (cause - 1)));

    // Ensure that suspends and resumes are paired correctly.
    // The given cause must not exist in the bitset upon entry.
    ASSERT(!(m_suspendAnimationCauses & cause));

    // First check the bitset, and suspend if there aren't any
    // previously-stored causes in it; set the cause bit *afterwards*.
    // This specific order of operations makes the code failsafe,
    // even in the event of failing the previous assertion.
    if (!m_suspendAnimationCauses) {
        if (m_mainFrame)
            m_mainFrame->animation()->suspendAnimations();
        if (!m_page->scriptedAnimationsSuspended())
            m_page->suspendScriptedAnimations();
    }
    m_suspendAnimationCauses |= cause;
}

void WebPage::resumeAnimations()
{
    d->resumeAnimations(WebPagePrivate::SuspendBySystem);
}

void WebPagePrivate::resumeAnimations(enum SuspendAnimationCause cause)
{
    // Ensure there is a single cause. Use Wegner's bit-counting method.
    ASSERT(cause && !(cause & (cause - 1)));

    // Ensure that suspends and resumes are paired correctly.
    // The given cause must exist in the bitset upon entry.
    ASSERT(m_suspendAnimationCauses & cause);

    // First check the bitset, and resume if the only previously-stored
    // cause in it is the given cause; clear the cause bit *afterwards*.
    // This specific order of operations makes the code failsafe,
    // even in the event of failing the previous assertion.
    if (m_suspendAnimationCauses == cause) {
        if (m_mainFrame)
...
void WebPagePrivate::setVisible(bool visible)
{
    if (visible != m_visible) {
        if (visible) {
            resumeAnimations(SuspendByHidingPage);
        } else {
            suspendAnimations(SuspendByHidingPage);
            closePagePopup();
        }
...
void WebPage::setVisible(bool visible)
{
    ...
    if (!visible) {
        ...
        if (foundIndex != WTF::notFound) {
            ...
            ASSERT(visibleWebPages()->find(this) == WTF::notFound);
        }
    ...
    if (!visibleWebPages()->isEmpty()) {
        if (visibleWebPages()->last() != this) {
            size_t foundIndex = visibleWebPages()->find(this);
            if (foundIndex != WTF::notFound) {
                visibleWebPages()->remove(foundIndex);
                ASSERT(visibleWebPages()->find(this) == WTF::notFound);
        ...
        // else do nothing since *this* is already at the top.
    } else
        visibleWebPages()->append(this);
...
void WebPagePrivate::updateDelegatedOverlays(bool duringCommit, bool dispatched)
{
    ...
    if (Platform::webKitThreadMessageClient()->isCurrentThread()) {
        ...
        if (m_selectionHandler->isSelectionActive()) {
            // When doing a commit we need to redraw the selection overlay.
            m_selectionHandler->selectionPositionChanged(duringCommit /*forceUpdateWithoutChange*/, duringCommit /*updateSelectionOverlay*/);
        }
        if (m_inspectorOverlay)
            m_inspectorOverlay->update();
    } else if (m_inspectorOverlay) {
        // Don't bother dispatching to webkit thread if inspector overlay is not active.
        m_updateDelegatedOverlaysDispatched = true;
        Platform::webKitThreadMessageClient()->dispatchMessage(Platform::createMethodCallMessage(&WebPagePrivate::updateDelegatedOverlays, this, duringCommit, true /*dispatched*/));
    }
}
...
bool WebPage::getInputSelection(int* start, int* end)
{
    if (d->m_page->defersLoading())
        return false;
    return d->m_inputHandler->getSelection(start, end);
}

bool WebPage::isTextSelected()
{
    return d->m_inputHandler->isTextSelected();
}

bool WebPage::isAllTextSelected()
{
    return d->m_inputHandler->isAllTextSelected();
}
...
{
    ZoomLog("WebPagePrivate::setScreenOrientation(%d)", orientation);
...
{
    ZoomLog("WebPage::setScreenOrientation(%d)", orientation);
...
void WebPage::applyPendingOrientationIfNeeded()
{
    ...
    if (d->m_redrawSpellCheckDialogTimer.isActive())
        d->m_redrawSpellCheckDialogTimer.stop();
    d->m_redrawSpellCheckDialogTimer.startOneShot(0.5);
}

void WebPagePrivate::redrawSpellCheckDialogTimerFired(WebCore::Timer<BlackBerry::WebKit::WebPagePrivate>*)
{
    m_inputHandler->redrawSpellCheckDialogIfRequired(false /* shouldMoveDialog */);
}
...
bool WebPagePrivate::setViewportSize(const IntSize& transformedActualVisibleSize, const IntSize& defaultLayoutSize, bool ensureFocusElementVisible)
{
    ...
    ZoomLog("WebPagePrivate::setViewportSize(%s, %s) m_pendingOrientation = %d", Platform::IntSize(transformedActualVisibleSize).toString().c_str(), Platform::IntSize(defaultLayoutSize).toString().c_str(), m_pendingOrientation);
    ...
    // The compositor client asks us to "setViewportSize", not "setScrollPosition"! Don't scroll! Unless a screen
    // orientation change is pending.
    // FIXME: Not sure why we ever scroll here if it's not an orientation change, but limit the fix to compositors with
    //        clients for now.
    bool shouldSuppressScrollNotification = m_compositor && m_compositor->client() && !hasPendingOrientation;
    ScrollNotificationSuppressor suppressor(m_backingStoreClient, shouldSuppressScrollNotification);
    ...
    // Detect viewport width change which affects text reflow
    if (m_webPage->settings()->textReflowMode() == WebSettings::TextReflowEnabled
        && transformedActualVisibleSize.width() != m_actualVisibleWidth) {
        // When the viewport width changes we will need layout if reflow is always-on.
        needsLayout = true;

        if (!m_pendingBlockZoom && relativeChange(m_zoomFactor, m_zoomFactorAtViewportChange) > minimumScaleRatio) {
            // Store a node the user is likely looking at before we relayout, so we can scroll to it later
            IntRect documentViewport(scrollPosition(),
                m_webkitThreadViewportAccessor->roundToDocumentContents(FloatRect(FloatPoint(), transformedActualVisibleSize), currentScale()).size());
            setTextReflowAnchorPoint(documentViewport);
        }

        m_zoomFactorAtViewportChange = m_zoomFactor;
    }
    ...
    // Unless there's a compositor client instead of a backing store,
    // in which case it will take care of scrolling and clamping so
    // avoid the scroll position of WebKit and compositor client getting
    // out of sync.
    if (!shouldSuppressScrollNotification) {
    ...
    if (zoomAboutPoint(scale, anchor, false /*enforceScaleClamping*/, true /*forceRendering*/)) {
        ...
        if (!updateScrollPositionAfterReflow(anchor, static_cast<double>(viewportSizeBefore.width()) / viewportSizeAfter.width())) {
        ...
        // Even if the scale didn't change, min/max scale might have changed after screen rotation or default layout size change for example
        if (m_compositor && m_compositor->client())
            m_client->scaleChanged();
...
{
    m_defaultLayoutSize = size.expandedTo(minimumLayoutSize);
}
...
void WebPage::setDefaultLayoutSize(const Platform::IntSize& platformSize)
{
    ...
    // The scale didn't change, but min/max scale might have changed after default layout size change.
    if (d->compositor() && d->compositor()->client())
        client()->scaleChanged();
}
...
bool WebPage::mouseEvent(const Platform::MouseEvent& mouseEvent, bool* wheelDeltaAccepted)
{
    ...
    PlatformMouseEvent platformMouseEvent(mouseEvent.documentViewportPosition(), mouseEvent.screenPosition(),
        toWebCoreMouseEventType(mouseEvent.type()), mouseEvent.clickCount(), buttonType,
...
bool WebPagePrivate::handleMouseEvent(PlatformMouseEvent& mouseEvent)
{
    ...
    if (mouseEvent.inputMethod() == TouchScreen) {
        ...
        // Don't update the state for MouseReleased, it will be reset at the bottom of the function.
        if (mouseEvent.type() != WebCore::PlatformEvent::MouseReleased)
            m_touchEventHandler->setUserTriggeredTouchPressed(mouseEvent.type() == WebCore::PlatformEvent::MousePressed && lastFatFingersResult.isTextInput());
    } else
        m_touchEventHandler->setUserTriggeredTouchPressed(false); // See comment above.
    ...
    Node* nodeUnderMouse = result.innerNode();
    ...
    if (mouseEvent.type() == WebCore::PlatformEvent::MousePressed) {
        ...
        } else if (!m_inputHandler->willOpenPopupForNode(nodeUnderMouse)) // Double check target node when convert from touch to mouse.
    ...
    if (mouseEvent.inputMethod() == TouchScreen && mouseEvent.type() == WebCore::PlatformEvent::MouseReleased)
        m_touchEventHandler->setUserTriggeredTouchPressed(false);
    return true;
}
...
const char* WebPage::doFatFingersTest(Platform::TouchPoint touchPoint, const int targetType)
{
    return d->doFatFingersTestPrivate(touchPoint, targetType);
}

const char* WebPagePrivate::doFatFingersTestPrivate(Platform::TouchPoint touchPoint, const int targetType)
{
    FatFingersResult result = FatFingers(this, touchPoint.documentContentPosition(), static_cast<BlackBerry::WebKit::FatFingers::TargetType>(targetType)).findBestPoint();
    WebCore::Element* bestElement = result.nodeAsElementIfApplicable();

    return bestElement->getIdAttribute().string().utf8().data();
}

bool WebPage::touchEvent(const Platform::TouchEvent& event, bool resetFatFingersResultOnTouchReleased)
{
    ...
    if (event.isSingleTap()
        || (event.m_type == Platform::TouchEvent::TouchEnd && !event.touchHoldCount() && event.neverHadMultiTouch()))
        d->m_pluginMayOpenNewTab = true;
    ...
    if (d->m_preventDefaultOnTouchStart) {
        if (tEvent.m_type == Platform::TouchEvent::TouchEnd || tEvent.m_type == Platform::TouchEvent::TouchCancel) {
            ...
            if (tEvent.touchHoldCount())
                d->m_touchEventHandler->resetLastFatFingersResult();
        }
        return true;
    }
    ...
    if (handled) {
        ...
        if ((tEvent.m_type == Platform::TouchEvent::TouchEnd || tEvent.m_type == Platform::TouchEvent::TouchCancel) && tEvent.touchHoldCount())
            d->m_touchEventHandler->resetLastFatFingersResult();
        return true;
    }
#endif
    if (resetFatFingersResultOnTouchReleased && tEvent.touchHoldCount() && (tEvent.m_type == Platform::TouchEvent::TouchEnd || tEvent.m_type == Platform::TouchEvent::TouchCancel))
        d->m_touchEventHandler->resetLastFatFingersResult();
    return false;
}
...
void WebPage::touchPointAsMouseEvent(const Platform::TouchPoint& point)
{
    ...
    d->m_touchEventHandler->handleTouchPoint(point);
}
...
void WebPage::inputSessionDisconnected()
{
    d->m_inputHandler->inputSessionDisconnected();
}
...
void WebPage::setSpellCheckingEnabled(bool enabled)
{
    ...
    d->m_inputHandler->spellingHandler()->setSystemSpellCheckStatus(enabled);

    if (!enabled)
        d->m_inputHandler->spellingHandler()->stopPendingSpellCheckingRequests();
}

void WebPage::spellCheckingRequestProcessed(int32_t transactionId, spannable_string_t* spannableString)
{
    d->m_inputHandler->spellingHandler()->spellCheckingRequestProcessed(transactionId, spannableString);
}
...
void WebPage::cut(bool isTouchTriggered)
{
    if (d->m_page->defersLoading())
        return;

    if (isTouchTriggered && !d->m_touchEventHandler->isUserTriggeredTouchPressed()) {
        // If cut is init by menu, we need to treat it as user triggered
        d->m_touchEventHandler->setUserTriggeredTouchPressed(true);
        d->m_inputHandler->cut();
        d->m_touchEventHandler->setUserTriggeredTouchPressed(false);
        return;
    }
...
void WebPage::setDocumentSelection(const Platform::IntPoint& documentStartPoint, const Platform::IntPoint& documentEndPoint, bool isTouch, const bool isBaseHandleGrabbed, const bool limitedToBoundingBox)
{
    if (d->m_page->defersLoading())
        return;

    d->m_touchEventHandler->setUserTriggeredTouchPressed(isTouch);
    d->m_selectionHandler->setSelection(documentStartPoint, documentEndPoint, isBaseHandleGrabbed, limitedToBoundingBox);
}

void WebPage::ensureExtentOfSelectionVisible()
{
    if (d->m_page->defersLoading() || !d->focusedOrMainFrame())
        return;

    if (FrameSelection* selection = d->focusedOrMainFrame()->selection())
        selection->revealSelection(ScrollAlignment::alignToEdgeIfNeeded, RevealExtent);
}
...
void WebPagePrivate::setTextReflowAnchorPoint(const Platform::IntRect& documentViewport)
{
    // Should only be invoked when text reflow is enabled.
    ASSERT(m_webPage->settings()->textReflowMode() == WebSettings::TextReflowEnabled);

    FatFingersResult result = FatFingers(this, documentViewport.center(), FatFingers::Text, documentViewport).findBestPoint();
    m_currentPinchZoomNode = result.node(FatFingersResult::ShadowContentNotAllowed);
    ZoomLog("WebPagePrivate::setTextReflowAnchorPoint: m_currentPinchZoomNode = %s", toString(m_currentPinchZoomNode.get()).latin1().data());
    if (!m_currentPinchZoomNode)
        return;

    IntRect nodeRect = rectForNode(m_currentPinchZoomNode.get());
    m_anchorInNodeRectRatio.set(
        static_cast<float>(documentViewport.center().x() - nodeRect.x()) / nodeRect.width(),
        static_cast<float>(documentViewport.center().y() - nodeRect.y()) / nodeRect.height());
}
...
bool WebPage::pinchZoomAboutPoint(double scale, const Platform::FloatPoint& documentFocalPoint)
{
    ...
    if (d->m_webPage->settings()->textReflowMode() == WebSettings::TextReflowEnabled) {
        Platform::ViewportAccessor* viewportAccessor = webkitThreadViewportAccessor();
        IntRect documentViewport = viewportAccessor->roundToDocumentContents(viewportAccessor->pixelViewportRect(), scale);
        documentViewport.setLocation(viewportAccessor->roundedDocumentContents(FloatPoint(documentFocalPoint) - FloatSize(documentViewport.size()) * 0.5f));
        d->setTextReflowAnchorPoint(documentViewport);
    ...
    if (!d->zoomAboutPoint(scale, documentFocalPoint)) {
        d->m_currentPinchZoomNode = 0;
        d->m_anchorInNodeRectRatio = FloatPoint(-1, -1);
        return false;
    }
...
double WebPage::zoomFactor() const
{
    return d->m_zoomFactor;
}

void WebPage::setZoomFactor(double zoomFactor)
{
    d->m_zoomFactor = zoomFactor;
}

#if ENABLE(VIEWPORT_REFLOW)
bool WebPagePrivate::toggleTextReflowIfEnabledForBlockZoomOnly(bool shouldEnableTextReflow)
{
    if (m_webPage->settings()->textReflowMode() == WebSettings::TextReflowEnabledOnlyForBlockZoom) {
        bool wasEnabled = m_page->settings()->isTextReflowEnabled();
        m_page->settings()->setTextReflowEnabled(shouldEnableTextReflow);
        return wasEnabled != shouldEnableTextReflow;
    }
...
bool WebPage::blockZoom(const Platform::IntPoint& documentTargetPoint)
{
    ...
    ZoomLog("WebPage::blockZoom(documentTargetPoint = %s)", documentTargetPoint.toString().c_str());

    Node* node = d->bestNodeForZoomUnderPoint(documentTargetPoint);
    if (!node)
        return false;

    const ZoomViewportAccessor* viewportAccessor = d->m_zoomViewportAccessor;
    const Platform::IntSize documentContentsSize = viewportAccessor->documentContentsSize();
    ...
    const double oldScale = viewportAccessor->scale();
    double newScale = 0;
    const double margin = endOfBlockZoomMode ? 0 : blockZoomMargin * 2;
    bool isFirstZoom = false;

    if (endOfBlockZoomMode) {
        // End of block zoom mode
        BlockZoomLog("end of block zoom mode");
        const IntRect rect = d->blockZoomRectForNode(node);
        blockRect = IntRect(0, rect.y(), documentContentsSize.width(), documentContentsSize.height() - rect.y());
        d->m_shouldReflowBlock = false;
    } else {
        // Start/continue block zoom mode
        BlockZoomLog("start/continue block zoom mode");
        ...
        if (!node->hasTagName(HTMLNames::imgTag)) {
            int blockArea = blockRect.width() * blockRect.height();
            int pageArea = documentContentsSize.width() * documentContentsSize.height();
            ...
            if (blockToPageRatio < minimumExpandingRatio) {
                ...
                BlockZoomLog("d->m_currentBlockZoomAdjustedNode = %s", toString(d->m_currentBlockZoomAdjustedNode.get()).latin1().data());
                BlockZoomLog("early return line %d", __LINE__);
                return false;
            }
        }
    ...
    BlockZoomLog("blockRect %s @line %d", Platform::IntRect(blockRect).toString().c_str(), __LINE__);
    newScale = std::min(d->newScaleForBlockZoomRect(blockRect, margin), d->maxBlockZoomScale());
    newScale = std::max(newScale, minimumScale());
    BlockZoomLog("newScale: %f @line %d", newScale, __LINE__);
    ...
    if (d->m_currentBlockZoomNode && d->m_shouldReflowBlock && settings()->textReflowMode() != WebSettings::TextReflowDisabled) {
        ...
        if (renderer && renderer->isText()) {
            // Target a minimum readable font size depending on this device's screen resolution.
            double minimumFontSize = d->m_webSettings->defaultFontSize() * d->m_webSettings->devicePixelRatio();
            double newFontSize = renderer->style()->fontSize() * newScale;
            BlockZoomLog("newFontSize: %f defaultFontSize %d devicePixelRatio %f @line %d", newFontSize, d->m_webSettings->defaultFontSize(), d->m_webSettings->devicePixelRatio(), __LINE__);
            if (newFontSize < minimumFontSize) {
                newScale = std::min(minimumFontSize / renderer->style()->fontSize(), d->maxBlockZoomScale());
                newScale = std::max(newScale, minimumScale());
                BlockZoomLog("newScale: %f @line %d", newScale, __LINE__);
            }
            // We know that text will never be wider than the actual visible size when reflow is on, but it could be narrower.
            blockRect.setWidth(ceil(viewportAccessor->preferredSize().width() / newScale));
            BlockZoomLog("blockRect %s @line %d", Platform::IntRect(blockRect).toString().c_str(), __LINE__);
    ...
    // Instead of messing with old scale, new scale and document viewport size, we can just use preferredsize divided
    // by newScale.
    // FIXME: Cascades uses a somewhat unreliable value for viewport size in UIB, this is a bit of a workaround until
    //        those calculations can be improved up in Cascades.
    float scaledViewportWidth = viewportAccessor->preferredSize().width() / newScale;
    // FIXME: However, the preferredSize.height is unreliable in Cascades (it tends to be a very low value to allow the
    //        web content to grow to it's inherent size without being influenced by outside factors).
    float scaledViewportHeight = viewportAccessor->documentViewportSize().height() * oldScale / newScale;
    BlockZoomLog("scaledViewport: %f, %f", scaledViewportWidth, scaledViewportHeight);
    // Typically, dx = (blockRect.width + 2 * blockZoomMargin) - blockRect.width()) / 2 = blockZoomMargin
    float dx = std::max(0.0f, (scaledViewportWidth - blockRect.width()) / 2.0f);
    float dy = std::max(0.0f, (scaledViewportHeight - blockRect.height()) / 2.0f);
    BlockZoomLog("dx: %f, dy: %f", dx, dy);

    const RenderObject* renderer = d->m_currentBlockZoomAdjustedNode->renderer();
    const FloatPoint topLeftPoint = blockRect.location();
    FloatPoint anchor;
    ...
    BlockZoomLog("anchor: %s", Platform::FloatPoint(anchor).toString().c_str());

    WebCore::FloatPoint finalAnimationDocumentScrollPosition;

    if (blockRect.height() <= scaledViewportHeight) {
        // The block fits in the viewport so center it.
        finalAnimationDocumentScrollPosition = FloatPoint(anchor.x() - dx, anchor.y() - dy);
    } else {
        // The block is longer than the viewport so top align it and add blockZoomMargin pixel margin.
        finalAnimationDocumentScrollPosition = FloatPoint(anchor.x() - dx, anchor.y() - blockZoomMargin);
    }

    BlockZoomLog("finalAnimationDocumentScrollPosition %s @line %d", Platform::FloatPoint(finalAnimationDocumentScrollPosition).toString().c_str(), __LINE__);
    ...
    if (settings()->textReflowMode() != WebSettings::TextReflowDisabled) {
        finalAnimationDocumentScrollPosition = FloatPoint(anchor.x() - dx, anchor.y() - blockZoomMargin);
        d->m_finalAnimationDocumentScrollPositionReflowOffset = FloatPoint(-dx, -blockZoomMargin);
    }
#endif
    ...
    if (!endOfBlockZoomMode && abs(newScale - oldScale) / oldScale < minimumExpandingRatio) {
        double unzoomableWidth = viewportAccessor->preferredSize().width() / viewportAccessor->scale();
        const double minimumDisplacement = minimumExpandingRatio * unzoomableWidth;
        ...
        BlockZoomLog("Checking whether to zoom:");
        BlockZoomLog("    oldScale = %f, minimumScale = %f", oldScale, d->minimumScale());
        BlockZoomLog("    scrollPositionDisplacement = %f, minimumDisplacement = %f", (double)scrollPositionDisplacement, minimumDisplacement);
        BlockZoomLog("    newScale = %f, oldScale = %f, ratio = %f", newScale, oldScale, relativeChange(newScale, oldScale));

        if (oldScale == d->minimumScale() || (scrollPositionDisplacement < minimumDisplacement && relativeChange(newScale, oldScale) < minimumScaleRatio)) {
            // If it's the first zoom and we're already close to the zoom to fit scale, return.
            if (isFirstZoom && relativeChange(d->zoomToFitScale(), oldScale) < minimumScaleRatio) {
                d->resetBlockZoom();
                BlockZoomLog("early return line %d", __LINE__);

                d->m_client->animateDeniedBlockZoom();

                return false;
            }
            // Zoom out of block zoom.
            BlockZoomLog("zooming out of block zoom line %d", __LINE__);
    ...
    d->m_userPerformedManualZoom = true;
    d->m_userPerformedManualScroll = true;
    BlockZoomLog("origScale: %f, documentScrollPosition: %s", oldScale, viewportAccessor->documentScrollPosition().toString().c_str());
    BlockZoomLog("newScale: %f, finalAnimationDocumentScrollPosition: %s", newScale, Platform::FloatPoint(finalAnimationDocumentScrollPosition).toString().c_str());
    d->m_client->animateToScaleAndDocumentScrollPosition(newScale, finalAnimationDocumentScrollPosition, true);

    // The animation request can either terminate with a "zoomAnimationFinished" callback, or a new viewport,
    // depending on the integration (webplatform or compositor client, respectively). Keep track of this pending
    // block zoom so we can act on it depending on which event comes first.
    d->m_pendingBlockZoom = true;
...
{
    ZoomLog("WebPage::isAtInitialZoom: m_zoomViewportAccessor->scale = %f, initialScale = %f", d->m_zoomViewportAccessor->scale(), d->initialScale());
    return fabs(d->m_zoomViewportAccessor->scale() - d->initialScale()) <= zoomFactorEpsilon || !d->isUserScalable();
}
...
void WebPage::setFocused(bool focused)
{
    ...
    if (d->m_inputHandler) {
        if (focused)
            d->m_inputHandler->sendInputStateNotifications(true /* sendFormStateNotification */, true /* sendOnlyIfSessionDisconnected */);
        else if (d->m_inputHandler->isPasswordType())
            d->assignFocus(Platform::FocusDirectionNone);
    }
}
...
void WebPage::clearCredentials()
{
    ...
    if (d->m_webSettings->isCredentialAutofillEnabled())
        AuthenticationChallengeManager::instance()->clearAllCredentials();
    LegacyCredentialBackingStore::instance()->deleteCredentials();
#endif
}
...
{
    // Do nothing: "never remember" is now handled internally by CredMgr and cleared in
    // clearCredentials.
}
...
void WebPage::clearBackForwardList(bool keepCurrentPage) const
{
    ...
    if (keepCurrentPage && currentItem)
...
void WebPage::adjustScaleAndDocumentScrollPosition(const Platform::FloatPoint& documentScrollPosition, double currentScale)
{
    notImplemented();
}

void WebPage::notifyAdjustedScaleAndScrollPosition(const Platform::FloatPoint& newDocumentScrollPosition, double newScale)
{
    d->notifyAdjustedScaleAndScrollPosition(newDocumentScrollPosition, newScale);
}
...
void WebPagePrivate::setCompositorDrawsRootLayer(bool compositorDrawsRootLayer)
{
    ...
    // FIXME: rebase. Consider using updateCompositingLayersAfterStyleChange();
...
void WebPagePrivate::commitRootLayer(const IntRect& layoutRect, const IntRect& documentRect, bool drawsRootLayer)
{
    ...
    m_compositor->setRootLayer(rootLayer ? rootLayer->layerCompositingThread() : 0);
    RenderView* renderView = m_mainFrame ? m_mainFrame->contentRenderer() : 0;
    GraphicsLayer* scrollLayer = renderView ? renderView->compositor()->scrollLayer() : 0;
    m_compositor->setScrollLayer(scrollLayer ? scrollLayer->platformLayer()->layerCompositingThread() : 0);

    // Overlay layers
    LayerWebKitThread* overlayLayer = 0;
    if (m_overlayLayer)
        overlayLayer = m_overlayLayer->platformLayer();
    m_compositor->setOverlayLayer(overlayLayer ? overlayLayer->layerCompositingThread() : 0);
    ...
    if (m_compositor->context() && m_compositor->context()->makeCurrent()) {
        BlackBerry::Platform::Graphics::GLES2ContextState::AppStateSaver appStateSaver;

        LayerTiler::willCommit();
        if (rootLayer)
...
bool WebPagePrivate::commitRootLayerIfNeeded()
{
    ...
    updateDelegatedOverlays(true /* duringCommit */);
    ...
    IntRect layoutRect(scrollPosition(), view->visibleSize());
...
{
    bool shouldSuspend = !m_visible || m_activationState != ActivationActive || m_fullScreenPluginView;
...
void WebPagePrivate::didChangeSettings(WebSettings* webSettings)
{
    ...
    if (coreSettings->privateBrowsingEnabled() != webSettings->isPrivateBrowsingEnabled()) {
        ...
        AuthenticationChallengeManager::instance()->setPrivateMode(webSettings->isPrivateBrowsingEnabled());
    }
...
void WebPage::setStandalone(bool enabled)
{
    d->m_page->settings()->setStandalone(enabled);
}

bool WebPage::standalone() const
{
    return d->m_page->settings()->standalone();
}
...
const HitTestResult& WebPagePrivate::hitTestResult(const IntPoint& contentPos, HitTestRequest::HitTestRequestType extraFlags /* = 0 */)
{
    if (m_cachedHitTestContentPos != contentPos) {
        m_cachedHitTestContentPos = contentPos;
        m_cachedRectHitTestResults.clear();
        m_cachedHitTestRequestType.clear();
        m_cachedHitTestResult = m_mainFrame->eventHandler()->hitTestResultAtPoint(m_cachedHitTestContentPos, HitTestRequest::ReadOnly | HitTestRequest::Active | HitTestRequest::AllowShadowContent | extraFlags);
    }
...
{
    if (destinationScrollPosition == scrollPosition() && destinationZoomScale == m_zoomViewportAccessor->scale())
        return;
...
void WebPagePrivate::invokeCard(const BlackBerry::Platform::String& mimeType)
{
    m_client->invokeCard(mimeType);
}

#if ENABLE(TEXT_AUTOSIZING)
void WebPagePrivate::onFontInfoChange(const Platform::String&, int fontSize)
{
    m_page->settings()->setTextAutosizingFontScaleFactor(fontSize / baseFontSize);
}
#endif
...
WebAccessibilityFunctionHandlerPrivate* WebPagePrivate::accessibilityFunctionHandlerPrivate()
{
    return m_accessibilityFunctionHandler->d;
}

void WebPagePrivate::requestSpellCheckingOptions(imf_sp_text_t& spellCheckingOptionRequest, WebCore::IntRect& caretRect, bool shouldMoveDialog)
{
    ASSERT(Platform::webKitThreadMessageClient()->isCurrentThread());
    // Dispatch to UI thread.
    Platform::userInterfaceThreadMessageClient()->dispatchMessage(
        Platform::createMethodCallMessage(
            &WebPagePrivate::requestSpellCheckingOptionsUserInterfaceThread,
            this,
...
void WebPagePrivate::requestSpellCheckingOptionsUserInterfaceThread(imf_sp_text_t& spellCheckingOptionRequest, WebCore::IntRect& caretRect, bool shouldMoveDialog, bool isCascades)
{
    ASSERT(Platform::userInterfaceThreadMessageClient()->isCurrentThread());

    if (isCascades)
        convertToWindowCoordinates(caretRect);

    // Dispatch back to WebKit thread.
    Platform::webKitThreadMessageClient()->dispatchMessage(
        Platform::createMethodCallMessage(
            &WebPagePrivate::requestSpellCheckingOptionsWebKitThread,
            this,
            spellCheckingOptionRequest,
            caretRect,
            shouldMoveDialog));
...
void WebPagePrivate::requestSpellCheckingOptionsWebKitThread(imf_sp_text_t& spellCheckingOptionRequest, WebCore::IntRect& caretRect, bool shouldMoveDialog)
{
    ASSERT(Platform::webKitThreadMessageClient()->isCurrentThread());
    Platform::IntRect platformCaretRect(caretRect);
    m_client->requestSpellCheckingOptions(spellCheckingOptionRequest, platformCaretRect, shouldMoveDialog);
}

void WebPagePrivate::convertToWindowCoordinates(WebCore::IntRect& documentRect)
{
    if (compositor() && compositor()->getLayerRenderer())
        documentRect = compositor()->getLayerRenderer()->toLastCompositedWindowCoordinates(documentRect);
}

void WebPage::convertToWindowCoordinates(Platform::IntRect& platformDocumentRect)
{
    WebCore::IntRect webCoreDocumentRect = platformDocumentRect;
    d->convertToWindowCoordinates(webCoreDocumentRect);
    platformDocumentRect = webCoreDocumentRect;
}
...
void WebPage::handleAccessibilityTouchEvent(const Platform::RawTouchEvent& event)
{
    if (!AXObjectCache::accessibilityEnabled())
        d->m_client->onUnhandledAccessibilityTouchEvent(event);

    Frame* mainFrame = d->mainFrame();
    if (!mainFrame)
        d->m_client->onUnhandledAccessibilityTouchEvent(event);
    ...
    AXObjectCache* axObjCache = doc->axObjectCache();
    if (!axObjCache)
        d->m_client->onUnhandledAccessibilityTouchEvent(event);

    AccessibilityObject* rootAXObject = axObjCache->rootObject();
    if (!rootAXObject)
        d->m_client->onUnhandledAccessibilityTouchEvent(event);

    Platform::ViewportAccessor* viewportAccessor = webkitThreadViewportAccessor();
    AccessibilityObject* objectAtPoint = 0;
    std::map<int, BlackBerry::Platform::RawTouchPoint> points = event.m_points;

    for (std::map<int, BlackBerry::Platform::RawTouchPoint>::iterator it = points.begin(); it != points.end(); ++it) {
        BlackBerry::Platform::RawTouchPoint rawPoint = it->second;

        screen_event_t screenEvent = 0;
        int result = screen_create_event(&screenEvent);
        if (result == -1)
            d->m_client->onUnhandledAccessibilityTouchEvent(event);

        int values[2];
        values[0] = event.m_type;
        screen_set_event_property_iv(screenEvent, SCREEN_PROPERTY_TYPE, values);
        values[0] = rawPoint.contactId();
        screen_set_event_property_iv(screenEvent, SCREEN_PROPERTY_TOUCH_ID, values);

        values[0] = rawPoint.screenPos().x();
        values[1] = rawPoint.screenPos().y();
        screen_set_event_property_iv(screenEvent, SCREEN_PROPERTY_POSITION, values);
        values[0] = rawPoint.pixelPos().x();
        values[1] = rawPoint.pixelPos().y();
        screen_set_event_property_iv(screenEvent, SCREEN_PROPERTY_SOURCE_POSITION, values);

        values[0] = rawPoint.orientation();
        screen_set_event_property_iv(screenEvent, SCREEN_PROPERTY_TOUCH_ORIENTATION, values);

        values[0] = rawPoint.pressure();
        screen_set_event_property_iv(screenEvent, SCREEN_PROPERTY_TOUCH_PRESSURE, values);

        values[0] = 1;
        values[1] = 1;
        screen_set_event_property_iv(screenEvent, SCREEN_PROPERTY_SIZE, values);

        // Transform the viewport pixel positions to document viewport coordinates.
        IntPoint documentViewportTouchPos = viewportAccessor->roundToDocumentFromPixelContents(rawPoint.pixelPos());
        IntRect documentViewportRect = viewportAccessor->roundToDocumentFromPixelContents(viewportAccessor->pixelViewportRect());
        documentViewportRect.setLocation(IntPoint(0, 0));
        documentViewportTouchPos = Platform::pointClampedToRect(documentViewportTouchPos, documentViewportRect);
        IntPoint point = frameView->windowToContents(documentViewportTouchPos);

        // Determine the item the finger is touching on.
        objectAtPoint = rootAXObject->accessibilityHitTest(point);
        objectAtPoint = objectAtPoint ? objectAtPoint : rootAXObject;

        // When the touched node is contenteditable, we want to return
        // the child node where the caret will be placed on instead.
        Node* node = objectAtPoint->node();
        if (node && node->isContentEditable()) {
            RenderObject* renderer = node->renderer();
            if (renderer) {
                VisiblePosition visiblePos = renderer->positionForPoint(point);
                node = visiblePos.deepEquivalent().anchorNode();
                if (node)
                    objectAtPoint = axObjCache->getOrCreate(node);
            }
        }

        if (!d->accessibilityFunctionHandlerPrivate()->emitInputEvent(objectAtPoint, screenEvent))
            d->m_client->onUnhandledAccessibilityTouchEvent(event);
    }
}
