...
enum SelectionExpansionType { Word = 0, Sentence, Paragraph };
...
public:
    ...
    void loadString(const char* string, size_t len, const char* baseURL, const char* contentType = "text/html", const char* failingURL = 0);
    ...
    void selectAtPoint(const Platform::IntPoint&, SelectionExpansionType = Word);
    ...
    void setExtraPluginDirectory(const std::string& path);
