...
public:
    ...
    bool touchEvent(const Platform::TouchEvent& event, bool resetFatFingersResultOnTouchReleased = false);
    ...
    void cut(bool isTouchTriggered = false);
    void copy();
    void paste();
    void selectAll();
    ...
    // The zoom factor is an additional scale applied by the compositor beyond
    // the scale defined by viewport arguments or device pixel ratio.
    ...
    bool getInputSelection(int* start, int* end);
    ...
    void setDocumentSelection(const Platform::IntPoint& documentStartPoint, const Platform::IntPoint& documentEndPoint, bool isTouch, const bool isBaseHandleGrabbed = false, const bool limitedToBoundingBox = false);
    void ensureExtentOfSelectionVisible();
    ...
    void adjustScaleAndDocumentScrollPosition(const Platform::FloatPoint& documentScrollPosition, double currentScale);
    ...
    void animateDeniedBlockZoom();

    void setShowDebugBorders(bool);
    unsigned savePage(const Platform::String& filename);

    WebAccessibilityFunctionHandler* accessibilityFunctionHandler();

    void convertToWindowCoordinates(Platform::IntRect& platformDocumentRect);

    void enableAccessibility();
    void handleAccessibilityTouchEvent(const BlackBerry::Platform::RawTouchEvent&);

    const char* doFatFingersTest(Platform::TouchPoint, const int);
