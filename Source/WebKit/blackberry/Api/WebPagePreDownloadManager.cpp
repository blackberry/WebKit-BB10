...
public:
    ...
    WebPagePreDownloadListener* m_listener;
...
WebPagePreDownloadClient::WebPagePreDownloadClient(WebPagePreDownloadManager* manager, WebPagePreDownloadListener* listener, unsigned parentDownloadId, bool confirmed)
    : d(new WebPagePreDownloadClientPrivate)
{
    d->m_manager = manager;
    d->m_listener = listener;
    d->m_parentId = parentDownloadId;
    d->m_confirmed = confirmed;
}
...
void WebPagePreDownloadClient::didReceiveResponse(ResourceHandle* handle, const ResourceResponse& response)
{
    ...
    Platform::String filename = stream->request().getSuggestedSaveName();
    if (filename.empty())
        filename = Platform::String(response.suggestedFilename());

    unsigned id = d->m_manager->pageClient()->downloadRequested(stream, filename, d->m_parentId, d->m_confirmed);

    if (d->m_listener)
        d->m_listener->notifyDownloadCreated(id);
...
void WebPagePreDownloadClient::didFail(ResourceHandle* handle, const ResourceError& error)
{
    ...
    Platform::FilterStream* stream = job->wrappedStream();
    if (stream) {
        // We tell WebPageClient it is confirmed so it will not ask user for
        // filename for a failed download.
        Platform::String filename = stream->request().getSuggestedSaveName();
        unsigned jobId = d->m_manager->pageClient()->downloadRequested(stream, filename, d->m_parentId, true /* confirmed */);

        if (d->m_listener)
            d->m_listener->notifyDownloadFailed(jobId, error.errorCode());
