...
class WebAccessibilityFunctionHandler;
...
#endif
#if ENABLE(TEXT_AUTOSIZING)
    , public Platform::FontInfoListener
#endif
...
public:
    ...
    enum SuspendAnimationCause {
        SuspendBySystem = 1 << 0,
        SuspendByHidingPage = 1 << 1
    };
    ...
    WebPagePreDownloadManager* predownloadManager();
    ...
    enum PreDownloadHandlingPolicy {
        DownloadDirectly,
        DownloadWithRedirectAndAuth
    };

    unsigned download(WebCore::Frame*, const Platform::NetworkRequest&, PreDownloadHandlingPolicy, unsigned parentDownloadId, WebPagePreDownloadListener*, bool needReferer, bool confirmed);
    unsigned download(WebCore::Frame*, const WebCore::ResourceRequest&, PreDownloadHandlingPolicy, unsigned parentDownloadId, WebPagePreDownloadListener*, bool confirmed);
    ...
    void setTextReflowAnchorPoint(const Platform::IntRect& documentViewport);
    ...
    void notifyAdjustedScaleAndScrollPosition(const Platform::FloatPoint& newDocumentScrollPosition, double newScale);
    ...
    virtual double currentScale() const { return m_transformationMatrix->m11(); }
    virtual int showAlertDialog(WebPageClient::AlertType atype);
    virtual bool isActive() const;
    virtual bool isVisible() const { return m_visible; }
    virtual AuthDialogResult authenticationChallenge(const WebCore::KURL&, const WebCore::ProtectionSpace&, WebCore::Credential&);
    virtual SaveCredentialDialogResult notifyShouldSaveCredential(bool isNew);
    virtual bool isCredentialAutofillEnabled() const;
    virtual bool isFormAutofillEnabled() const;
    ...
    double newScaleForBlockZoomRect(const WebCore::IntRect&, double margin);
    double maxBlockZoomScale() const;

    // Context Methods.
    Platform::WebContext webContext(TargetDetectionStrategy);
    PassRefPtr<WebCore::Node> contextNode(TargetDetectionStrategy, bool &isContentEditable);

#if ENABLE(VIEWPORT_REFLOW)
    // Returns true if toggled. After calling this method, you need relayout if it toggled.
    bool toggleTextReflowIfEnabledForBlockZoomOnly(bool shouldEnableTextReflow = false);
#endif
    ...
    void updateDelegatedOverlays(bool duringCommit = false, bool dispatched = false);
    ...
    bool updateScrollPositionAfterReflow(const WebCore::FloatPoint& anchorOffset, double inverseScale);
    ...
    void suspendAnimations(enum SuspendAnimationCause);
    void resumeAnimations(enum SuspendAnimationCause);
    ...
    const WebCore::HitTestResult& hitTestResult(const WebCore::IntPoint& contentPos, WebCore::HitTestRequest::HitTestRequestType extraFlags = 0);
    ...
    void invokeCard(const BlackBerry::Platform::String&);

#if ENABLE(TEXT_AUTOSIZING)
    // Platform::FontInfoListener
    virtual void onFontInfoChange(const Platform::String& fontFamily, int fontSize);
#endif

    WebAccessibilityFunctionHandlerPrivate* accessibilityFunctionHandlerPrivate();

    void requestSpellCheckingOptions(imf_sp_text_t& spellCheckingOptionRequest, WebCore::IntRect& caretRect, bool shouldMoveDialog);
    void requestSpellCheckingOptionsUserInterfaceThread(imf_sp_text_t& spellCheckingOptionRequest, WebCore::IntRect& caretRect, bool shouldMoveDialog, bool isCascades);
    void requestSpellCheckingOptionsWebKitThread(imf_sp_text_t& spellCheckingOptionRequest, WebCore::IntRect& caretRect, bool shouldMoveDialog);
    void redrawSpellCheckDialogTimerFired(WebCore::Timer<WebPagePrivate> *);
    void convertToWindowCoordinates(WebCore::IntRect& documentRect);
    ...
    OwnPtr<WebPagePreDownloadManager> m_predownloadManager;
    ...
    ZoomViewportAccessor* m_zoomViewportAccessor;
    ...
    bool m_pendingBlockZoom; // Double tap zoom pending.
    double m_zoomFactorAtViewportChange;
    ...
    typedef HashMap<RefPtr<WebCore::Document>, int> CachedHitTestRequestType;
    CachedRectHitTestResults m_cachedRectHitTestResults;
    CachedHitTestRequestType m_cachedHitTestRequestType;
    ...
    WebAccessibilityFunctionHandler* m_accessibilityFunctionHandler;
    WebCore::Timer<WebPagePrivate> m_redrawSpellCheckDialogTimer;

    int m_suspendAnimationCauses;

    const char* doFatFingersTestPrivate(Platform::TouchPoint, const int);
