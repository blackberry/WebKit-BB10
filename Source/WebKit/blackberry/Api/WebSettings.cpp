...
WebSettings* WebSettings::standardSettings()
{
    ...
    settings->m_private->setString(WebKitFixedFontFamily, BlackBerry::Platform::FontInfo::instance()->fontFamily(WebKitMonospace, BlackBerry::Platform::String::emptyString()));
    settings->m_private->setString(WebKitSansSeriffFontFamily, BlackBerry::Platform::FontInfo::instance()->fontFamily(WebKitSansSerif, BlackBerry::Platform::String::emptyString()));
    settings->m_private->setString(WebKitSeriffFontFamily, BlackBerry::Platform::FontInfo::instance()->fontFamily(WebKitSerif, BlackBerry::Platform::String::emptyString()));
    settings->m_private->setString(WebKitStandardFontFamily, BlackBerry::Platform::FontInfo::instance()->fontFamily(WebKitStandard, BlackBerry::Platform::String::emptyString()));
...
bool WebSettings::isSupportedObjectMIMEType(const BlackBerry::Platform::String& mimeType)
{
    ...
    return s_supportedObjectMIMETypes->contains(WebCore::MIMETypeRegistry::getNormalizedMIMEType(mimeType));
}
...
{
    if (arePluginsEnabled() != enabled) {
        m_private->setBoolean(WebKitPluginsEnabled, enabled);
        if (!enabled)
            return;
        WebCore::PluginDatabase* database = WebCore::PluginDatabase::installedPlugins(true);
        if (!database)
            return;
        database->refresh();
    }
