...
void AutofillManager::didChangeInTextField(PassRefPtr<HTMLInputElement> element)
{
    if (!isAutofillable(element.get()))
        return;

    if (m_element != element)
        m_element = element;

    Vector<String> candidates = autofillBackingStore().get(m_element->getAttribute(HTMLNames::nameAttr).string(), m_element->value());
