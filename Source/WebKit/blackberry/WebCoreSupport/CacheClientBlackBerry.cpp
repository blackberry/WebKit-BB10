...
void CacheClientBlackBerry::initialize()
{
    ...
    if (!isDisabled) {
        ...
        memoryCache()->setDeadDecodedDataDeletionInterval(60);
        updateCacheCapacity();
    }
}
...
#else
    unsigned cacheMaxDeadCapacity = 32 * 1024 * 1024;
    unsigned cacheMinDeadCapacity = cacheMaxDeadCapacity / 4;
    unsigned cacheTotalCapacity = cacheMaxDeadCapacity + cacheMaxDeadCapacity / 2;
