...
void ChromeClientBlackBerry::mouseDidMoveOverElement(const HitTestResult& result, unsigned flags)
{
    m_webPagePrivate->m_touchEventHandler->mouseDidMoveOverElement(result, flags);
}

void ChromeClientBlackBerry::notifyVisibilityChange(VisibilityChangeType type)
{
    m_webPagePrivate->m_touchEventHandler->notifyVisibilityChange(type);
}
...
static inline BlackBerry::Platform::StorageType platformTypeFromChromeClientStorageType(WebCore::ChromeClient::StorageType type)
{
    switch (type) {
    case WebCore::ChromeClient::StorageLocal:
        return BlackBerry::Platform::StorageLocal;
    case WebCore::ChromeClient::StorageSession:
        return BlackBerry::Platform::StorageSession;
    case WebCore::ChromeClient::StorageIndexedDB:
        return BlackBerry::Platform::StorageDatabase;
    case WebCore::ChromeClient::StorageWebFileSystem:
        return BlackBerry::Platform::StorageWebFileSystem;
    // No default: let the compiler warn us if a new type is added.
    }

    BLACKBERRY_ASSERT_NOT_REACHED();
    return BlackBerry::Platform::StorageTypeCount;
}
...
{
    return new BlackBerry::Platform::SecurityOrigin(origin.protocol(), origin.host(), origin.port());
}

bool ChromeClientBlackBerry::shouldAllowStorageRequest(WebCore::ChromeClient::StorageType type, WebCore::SecurityOrigin* origin, unsigned long long quota)
{
    std::auto_ptr<BlackBerry::Platform::SecurityOrigin> platformOrigin(platformSecurityOriginFromWebCoreSecurityOrigin(*origin));
    if (!platformOrigin.get())
        return false;

    BlackBerry::Platform::StorageType platformType = platformTypeFromChromeClientStorageType(type);
    return m_webPagePrivate->m_client->shouldAllowStorageRequest(platformType, *(platformOrigin.get()), quota);
}
...
PassRefPtr<DateTimeChooser> ChromeClientBlackBerry::openDateTimeChooser(DateTimeChooserClient*, const DateTimeChooserParameters&)
{
    notImplemented();
...
void ChromeClientBlackBerry::scroll(const IntSize& delta, const IntRect& scrollViewRect, const IntRect& clipRect)
{
    ...
    // We also need to check m_mainFrame since it could be set to null when destroying
    // the WebPage.
    if (!m_webPagePrivate->m_mainFrame || !m_webPagePrivate->m_mainFrame->view())
        return;
...
#if ENABLE(WEBGL)
void ChromeClientBlackBerry::requestWebGLPermission(Frame*)
{
    m_webPagePrivate->adjustFullScreenElementDimensionsIfNeeded();
}
#endif
...
void ChromeClientBlackBerry::invokeCard(const BlackBerry::Platform::String& mimeType)
{
    m_webPagePrivate->invokeCard(mimeType);
}
