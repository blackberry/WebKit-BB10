...
void ChromeClientBlackBerry::setPagePopupDriver(PagePopupDriver* driver)
{
    notImplemented();
}

void ChromeClientBlackBerry::resetPagePopupDriver()
{
    notImplemented();
}
#endif
...
{
    BlackBerry::Platform::String protocol = BlackBerry::Platform::String(origin.protocol().utf8().data());
    BlackBerry::Platform::String host = BlackBerry::Platform::String(origin.host().utf8().data());

    return new BlackBerry::Platform::SecurityOrigin(protocol, host, origin.port());
}
...
{
    Frame* frame = m_webPagePrivate->m_mainFrame;
    if (!frame || !frame->view())
        return;

    bool hasAtLeastOneInRegionScrollableArea = false;
    for ( ; frame && !hasAtLeastOneInRegionScrollableArea; frame = frame->tree()->traverseNext()) {
        if (!frame->view() || !frame->view()->scrollableAreas())
            continue;

        hasAtLeastOneInRegionScrollableArea = frame->view()->scrollableAreas()->size();
    }
