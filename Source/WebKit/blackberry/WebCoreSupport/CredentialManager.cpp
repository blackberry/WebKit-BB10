...
static ProtectionSpaceToCredentialMap& ignoredCredentials()
{
...
void CredentialManager::autofillAuthenticationChallenge(PageClientBlackBerry* pageClient, const ProtectionSpace& protectionSpace, BlackBerry::Platform::String& username, BlackBerry::Platform::String& password)
{
    ASSERT(pageClient);

    if (!pageClient->isCredentialAutofillEnabled() || pageClient->isPrivateBrowsingEnabled())
        return;
...
void CredentialManager::autofillPasswordForms(PageClientBlackBerry* pageClient, PassRefPtr<HTMLCollection> docForms)
{
    ASSERT(pageClient);
    ASSERT(docForms);
...
void CredentialManager::saveCredentialIfConfirmed(PageClientBlackBerry* pageClient, const CredentialTransformData& data)
{
    ...
    ProtectionSpaceToCredentialMap& ignored = ignoredCredentials();
    ProtectionSpaceToCredentialMap::iterator iter = ignored.find(data.protectionSpace());
    if (ignored.end() != iter && iter->value == data.credential())
        return;

    ignored.remove(data.protectionSpace());
    ...
    if (type == PageClientBlackBerry::SaveCredentialYes) {
        ...
        ignored.add(data.protectionSpace(), data.credential());
}
