...
static JSValueRef addSearchProviderMethod(
    JSContextRef ctx, JSObjectRef, JSObjectRef thisObject,
    size_t argumentCount, const JSValueRef arguments[], JSValueRef*)
{
    ...
    if (frame) {
        ...
        chrome->addSearchProvider(originURL, BlackBerry::Platform::String::fromUtf8(newURL));
    }
    ...
    size_t argumentCount, const JSValueRef arguments[], JSValueRef*)
{
    ...
    if (frame) {
        ...
        ChromeClientBlackBerry* chrome = static_cast<ChromeClientBlackBerry*> (page->chrome()->client());
        int retVal = chrome->isSearchProviderInstalled(BlackBerry::Platform::String::fromUtf8(newURL));
