...
bool FrameLoaderClientBlackBerry::cookiesEnabled()
{
    if (m_webPagePrivate)
        m_cookiesEnabled = m_webPagePrivate->m_webSettings->areCookiesEnabled();
...
{
    if (m_webPagePrivate && m_webPagePrivate->m_dumpRenderTree)
...
void FrameLoaderClientBlackBerry::dispatchDecidePolicyForResponse(FramePolicyFunction function, const ResourceResponse& response, const ResourceRequest& request)
{
    ...
    if (contentDispositionType(response.httpHeaderField("Content-Disposition")) == ContentDispositionAttachment)
        ...
        policy = PolicyUse;
    else if ((ResourceRequest::TargetIsMainFrame == request.targetType() || ResourceRequest::TargetIsSubframe == request.targetType())
        && m_webPagePrivate && m_webPagePrivate->m_client->downloadAllowed(request.url().string()))
        policy = PolicyDownload;
...
void FrameLoaderClientBlackBerry::dispatchDecidePolicyForNavigationAction(FramePolicyFunction function, const NavigationAction& action, const ResourceRequest& request, PassRefPtr<FormState>)
{
    ...
    if (!url.isNull()) {
        ...
        NetworkRequest platformRequest(request.url().string());
        ...
        if (!platformRequest.getUrlRef().empty()) { // Some invalid URLs will result in empty URL in platformRequest
            ...
            if (!m_webPagePrivate || !m_webPagePrivate->m_client->acceptNavigationRequest(platformRequest, BlackBerry::Platform::NavigationType(action.type()))) {
...
{
    if (ScriptController::processingUserGesture() && (!m_webPagePrivate || !m_webPagePrivate->m_pluginMayOpenNewTab)) {
...
bool FrameLoaderClientBlackBerry::shouldUsePluginDocument(const String& mimeType) const
{
    ...
    const Vector<String>& paramValues, const String& mimeTypeIn, bool loadManually)
{
    ...
    if (mimeType.isEmpty()) {
        ...
        if (!shouldUsePluginDocument(mimeType))
            mimeType = mimeTypeIn;
    }
    ...
    if (!shouldUsePluginDocument(mimeType) && m_frame->loader() && m_frame->loader()->subframeLoader() && !url.isNull())
...
PassRefPtr<Widget> FrameLoaderClientBlackBerry::createJavaAppletWidget(const IntSize&, HTMLAppletElement*, const KURL&, const Vector<String>&, const Vector<String>&)
{
    notImplemented();
    return 0;
}
...
void FrameLoaderClientBlackBerry::receivedData(DocumentLoader* loader, const char* data, int length)
{
    ...
    if (m_cancelLoadOnNextData) {
        if (m_frame->document() && !m_frame->document()->isDelayingLoadEvent()) {
...
//        BlackBerry::Platform::logAlways(BlackBerry::Platform::LogLevelCritical, "frameloaderclient -transitionToCommittedForNewPage");
//        m_webPagePrivate->initAccessibilityBridge();
    }
}
...
{
    if (m_webPagePrivate)
        m_userAgent = m_webPagePrivate->m_webSettings->userAgentString();
    return m_userAgent;
}
...
void FrameLoaderClientBlackBerry::dispatchDidFinishLoad()
{
    ...
    ASSERT(forms);
    AuthenticationChallengeManager::instance()->autofillPasswordForms(m_webPagePrivate, *forms);
#endif
...
void FrameLoaderClientBlackBerry::dispatchWillSubmitForm(FramePolicyFunction function, PassRefPtr<FormState>)
{
    ...
    (m_frame->loader()->policyChecker()->*function)(PolicyUse);
#if ENABLE(BLACKBERRY_CREDENTIAL_PERSIST)
    if (m_formCredentials.isValid() && m_webPagePrivate)
        AuthenticationChallengeManager::instance()->saveCredentialIfConfirmed(m_webPagePrivate, m_formCredentials);
#endif
}
...
#endif
    if (m_webPagePrivate && !m_webPagePrivate->m_webSettings->isPrivateBrowsingEnabled()) {
...
void FrameLoaderClientBlackBerry::dispatchDidClearWindowObjectInWorld(DOMWrapperWorld* world)
{
    ...
    if (m_webPagePrivate && m_webPagePrivate->m_enableQnxJavaScriptObject)
...
void FrameLoaderClientBlackBerry::dispatchWillSendRequest(DocumentLoader* docLoader, long unsigned, ResourceRequest& request, const ResourceResponse& redirectResponse)
{
    ...
    if (m_webPagePrivate) {
        // Add custom headers from the WebPageClient.
        NetworkRequest platformRequest(request.url().string());
        request.initializePlatformRequest(platformRequest, cookiesEnabled());
...
PolicyAction FrameLoaderClientBlackBerry::decidePolicyForExternalLoad(const ResourceRequest& request, bool isFragmentScroll)
{
    ...
    if (isMainFrame()
        && m_webPagePrivate->m_webSettings->areLinksHandledExternally()
...
void FrameLoaderClientBlackBerry::startDownload(const ResourceRequest& request, const String& suggestedName)
{
    ...
    requestCopy.setTargetType(ResourceRequest::TargetIsDownload);
    m_webPagePrivate->download(m_webPagePrivate->m_mainFrame, requestCopy, BlackBerry::WebKit::WebPagePrivate::DownloadWithRedirectAndAuth, 0 /* parentDownloadId */, 0 /* WebPagePreDownloadListener* */, false /* confirmed */);
}
...
void FrameLoaderClientBlackBerry::convertMainResourceLoadToDownload(DocumentLoader* documentLoader, const ResourceRequest& request, const ResourceResponse& response)
{
    ...
    BlackBerry::Platform::FilterStream* stream;
    if (!buff) {
        ...
        ASSERT(stream);
        NetworkRequest* platformRequest = stream->mutableRequest();
        if (platformRequest)
            platformRequest->setTargetType(NetworkRequest::TargetIsDownload);

        m_webPagePrivate->m_client->downloadRequested(stream, filename, 0 /* parentDownloadId */, false /* confirmed */);
        return;
    }

    // For main page resource which has cached, since no stream could be reused,
    // create download stream to send the data buffer.
    stream = new CachedResourceStream(response.url().string(), response.mimeType(), buff);

    m_webPagePrivate->m_client->downloadRequested(stream, filename, 0 /* parentDownloadId */, false /* confirmed */);
    stream->streamOpen();
    BLACKBERRY_ASSERT(!documentLoader->mainResourceLoader());
    // if mainResourceLoader is non-zero, DocumentLoader will call didFail() to wrap current page load after
    // convertMainResourceLoadToDownload; but here when using cachedResource, mainResourceLoader is 0, so
    // we should call cancelMainResourceLoad instead, otherwise documentLoader->dataReceived() will be called
    // load the page.
    documentLoader->cancelMainResourceLoad(interruptedForPolicyChangeError(request));
}
...
{
    // willRestoreFromPageCache() must have been called and m_webPagePrivate must have been
    // restored for sub frames. m_webPagePrivate was only set to 0 by didSaveToPageCache()
    // for sub frames after the page went to page cache.
    ASSERT(m_webPagePrivate);
    m_webPagePrivate->m_didRestoreFromPageCache = true;
}

void FrameLoaderClientBlackBerry::willRestoreFromPageCache()
{
    if (!isMainFrame()) {
...
void FrameLoaderClientBlackBerry::dispatchWillUpdatePersistentCache(const ResourceRequest&)
{
    ASSERT(isMainFrame());
    if (!isMainFrame())
        return;

    m_webPagePrivate->m_client->notifyWillUpdatePersistentCache();
}

void FrameLoaderClientBlackBerry::dispatchDidLoadFromPersistentCache(const ResourceRequest&)
{
    ASSERT(isMainFrame());
    if (!isMainFrame())
        return;

    m_webPagePrivate->m_client->notifyDidLoadFromPersistentCache();
}
...
static inline BlackBerry::Platform::SecurityOrigin* platformSecurityOriginFromWebCoreSecurityOrigin(const WebCore::SecurityOrigin* origin)
{
    return origin ? new BlackBerry::Platform::SecurityOrigin(origin->protocol(), origin->host(), origin->port()) : 0;
}

bool FrameLoaderClientBlackBerry::allowDisplayingInsecureContent(bool enabledPerSettings, SecurityOrigin* origin, const KURL& url)
{
    if (!m_webPagePrivate)
        return false;

    std::auto_ptr<BlackBerry::Platform::SecurityOrigin> platformOrigin(platformSecurityOriginFromWebCoreSecurityOrigin(origin));
    BlackBerry::Platform::String urlString = BlackBerry::Platform::String::fromUtf8(url.string().utf8().data());

    return m_webPagePrivate->m_client->allowDisplayingInsecureContent(enabledPerSettings, platformOrigin.get(), urlString);
}

bool FrameLoaderClientBlackBerry::allowRunningInsecureContent(bool enabledPerSettings, SecurityOrigin* origin, const KURL& url)
{
    if (!m_webPagePrivate)
        return false;
    ...
    return m_webPagePrivate->m_client->allowRunningInsecureContent(enabledPerSettings, platformOrigin.get(), urlString);
}

void FrameLoaderClientBlackBerry::didDisplayInsecureContent()
{
    if (!m_webPagePrivate)
        return;

    m_webPagePrivate->m_client->didDisplayInsecureContent();
}

void FrameLoaderClientBlackBerry::didRunInsecureContent(SecurityOrigin* origin, const KURL& url)
{
    if (!m_webPagePrivate)
        return;
    ...
    m_webPagePrivate->m_client->didRunInsecureContent(platformOrigin.get(), urlString);
}

ResourceError FrameLoaderClientBlackBerry::interruptedForPolicyChangeError(const ResourceRequest& request)
{
    return ResourceError(ResourceError::platformErrorDomain, BlackBerry::Platform::FilterStream::StatusErrorAlreadyHandled, request.url().string(), emptyString());
}

ResourceError FrameLoaderClientBlackBerry::cancelledError(const ResourceRequest& request)
{
    return ResourceError(ResourceError::platformErrorDomain, BlackBerry::Platform::FilterStream::StatusErrorAlreadyHandled, request.url().string(), emptyString());
}

ResourceError FrameLoaderClientBlackBerry::blockedError(const ResourceRequest& request)
{
    return ResourceError(ResourceError::platformErrorDomain, BlackBerry::Platform::FilterStream::StatusErrorAlreadyHandled, request.url().string(), emptyString());
}

ResourceError FrameLoaderClientBlackBerry::cannotShowMIMETypeError(const ResourceResponse& response)
{
    return ResourceError(ResourceError::platformErrorDomain, BlackBerry::Platform::FilterStream::StatusErrorAlreadyHandled, response.url().string(), emptyString());
}

ResourceError FrameLoaderClientBlackBerry::fileDoesNotExistError(const ResourceResponse& response)
{
    return ResourceError(ResourceError::platformErrorDomain, BlackBerry::Platform::FilterStream::StatusErrorAlreadyHandled, response.url().string(), emptyString());
}

ResourceError FrameLoaderClientBlackBerry::pluginWillHandleLoadError(const ResourceResponse& response)
{
    return ResourceError(ResourceError::platformErrorDomain, BlackBerry::Platform::FilterStream::StatusErrorAlreadyHandled, response.url().string(), emptyString());
}

void FrameLoaderClientBlackBerry::didChangeScrollOffset()
{
    // If we have a selection, we need to update the position of the overlay.
    // Checking the document to make sure the page isn't being torn down while making this check.
    if (m_frame && m_frame->document() && m_frame->selection()->isRange())
