...
#else
    String url = m_frame->document()->url().string();
    Image* bitmap = iconDatabase().synchronousIconForPageURL(url, IntSize(10, 10));
    if (!bitmap)
        return;
    SharedBuffer* data = bitmap->data();
    if (!data)
        return;
    if (data->size() < bitmap->size().width() * bitmap->size().height() * 4)
        return;

    Vector<char> pngData;
    compressRGBABigEndianToPNG((unsigned char*)data->data(), bitmap->size(), pngData);
    Vector<char> out;
