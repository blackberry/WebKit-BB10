...
void GeolocationClientBlackBerry::requestPermission(Geolocation* location)
{
    ...
    if (!BlackBerry::Platform::GeolocationHandler::instance()->isGlobalServiceActive()) {
        // We only want to ask the user once per session.
        ...
        // Fail it regardless. If the user choose to open settings and change location services, the page will
        // reload and restart the requestPermission method call.
        location->setIsAllowed(false);
        return;
    }
