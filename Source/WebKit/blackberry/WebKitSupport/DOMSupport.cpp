...
void visibleTextQuads(const VisibleSelection& selection, Vector<FloatQuad>& quads, SelectionOverlay::Selection* overlaySelection, GraphicsLayer* rootOverlay)
{
    ...
    visibleTextQuads(*(selection.firstRange()), quads, overlaySelection, rootOverlay, true /* useSelectionHeight */);
}

void visibleTextQuads(const Range& range, Vector<FloatQuad>& quads, SelectionOverlay::Selection* selection, GraphicsLayer* rootLayer, bool useSelectionHeight)
{
    ...
    for (Node* node = range.firstNode(); node && node != stopNode; node = NodeTraversal::next(node)) {
        ...
        if (selection) {
            RenderLayerModelObject* container = renderText->containerForRepaint();
            if (container) {
                Vector<FloatQuad>& containerQuads = selection->add(container->layer()->backing()->parentForSublayers(), Vector<FloatQuad>()).iterator->value;
                renderText->containerQuadsForRange(containerQuads, container, startOffset, endOffset, useSelectionHeight);
            } else
                selection->add(rootLayer, Vector<FloatQuad>()).iterator->value.appendRange(quads.begin() + i, quads.end());
        }
    }
}

void clipSelectionToRenderer(SelectionOverlay::Selection& selection, WebCore::RenderObject* renderer, GraphicsLayer* rootLayer)
{
    if (!renderer->isBox())
        return;
    GraphicsLayer* containingLayer = rootLayer;
    RenderBox* renderBox = toRenderBox(renderer);
    RenderLayerModelObject* container = renderer->containerForRepaint();
    if (container)
        containingLayer = container->layer()->backing()->parentForSublayers();
    SelectionOverlay::Selection::iterator iterator = selection.find(containingLayer);
    if (iterator == selection.end())
        return;

    Vector<FloatQuad>& quads = iterator->value;
    FloatQuad clipQuad = renderer->localToContainerQuad(FloatRect(0, 0, renderBox->width(), renderBox->height()), container, false);
    FloatRect clipRect = clipQuad.boundingBox();
    for (size_t i = 0; i < quads.size(); ++i) {
        FloatRect rect = quads[i].boundingBox();
...
bool isTextBasedContentEditableElement(Element* element)
{
    ...
    if (element->isReadOnlyNode() || !element->isEnabledFormControl() || isElementReadOnly(element))
        return false;
...
bool shouldLearnText(const WebCore::Element* element)
{
    if (!element)
        return false;

    DEFINE_STATIC_LOCAL(QualifiedName, qualifiedAttrNameForLearnText, (nullAtom, "data-blackberry-learn-field-text", nullAtom));
    AtomicString attributeString;
    if (!element->fastHasAttribute(qualifiedAttrNameForLearnText))
        return true;

    attributeString = element->fastGetAttribute(qualifiedAttrNameForLearnText);

    if (attributeString.isNull() || attributeString.isEmpty())
        return true;

    if (equalIgnoringCase(attributeString, "false"))
        return false;
...
// Helper function to check if user-select:None has set.
bool isUserSelectNone(WebCore::Node* node)
{
    if (Position::nodeIsUserSelectNone(node))
        return true;
    else if (node->hasTagName(HTMLNames::textareaTag)
        || (node->hasTagName(HTMLNames::inputTag) && node->toInputElement()->isTextField()))
        return false;

    Node* parent = node->parentOrShadowHostNode();
    return parent ? isUserSelectNone(parent) : Position::nodeIsUserSelectNone(node);
}
