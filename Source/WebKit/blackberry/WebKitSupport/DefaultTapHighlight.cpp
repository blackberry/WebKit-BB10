...
void DefaultTapHighlight::paintContents(const GraphicsLayer*, GraphicsContext& c, GraphicsLayerPaintingPhase, const IntRect& /*inClip*/)
{
    ...
    Platform::IntRect unifiedRectToPaint;
    const std::vector<Platform::IntRect>& rects = m_region.rects();
    for (unsigned i = 0; i < m_region.numRects(); ++i) {
        unifiedRectToPaint = Platform::unionOfRects(unifiedRectToPaint, rects[i]);
    }

    if (unifiedRectToPaint.area() <= 0)
        return;
    ...
    // Draw tap highlight
    c.setFillColor(m_color, ColorSpaceDeviceRGB);
    c.fillRect(Platform::FloatRect(unifiedRectToPaint));
    Color darker = Color(m_color.red(), m_color.green(), m_color.blue()); // Get rid of alpha.
    c.setStrokeColor(darker, ColorSpaceDeviceRGB);
    c.strokeRect(Platform::FloatRect(unifiedRectToPaint), 1);
    c.restore();
#endif
}
