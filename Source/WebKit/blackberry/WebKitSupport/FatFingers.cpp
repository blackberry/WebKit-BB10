...
bool FatFingers::isElementClickable(Element* element)
{
    ASSERT(element);
    ExceptionCode ec = 0;
...
FatFingers::FatFingers(WebPagePrivate* webPage, const WebCore::IntPoint& contentPos, TargetType targetType, const IntRect& viewport /* = IntRect() */)
    : m_webPage(webPage)
...
const FatFingersResult FatFingers::findBestPoint()
{
    ...
    IntRect viewportRect = m_viewport.isEmpty() ? m_webPage->mainFrame()->view()->visibleContentRect() : m_viewport;
    ...
    const HitTestResult& hitResult = m_webPage->hitTestResult(m_contentPos, m_viewport.isEmpty() ? 0 : HitTestRequest::IgnoreClipping);
...
bool FatFingers::findIntersectingRegions(Document* document, Vector<IntersectingRegion>& intersectingRegions, IntRectRegion& remainingFingerRegion)
{
    ...
    if (m_webPage->m_cachedRectHitTestResults.contains(document) && m_webPage->m_cachedHitTestRequestType.contains(document) && (m_webPage->m_cachedHitTestRequestType.get(document) == m_targetType))
...
void FatFingers::getNodesFromRect(Document* document, const IntPoint& contentPos, ListHashSet<RefPtr<Node> >& intersectedNodes)
{
    ...
    m_webPage->m_cachedHitTestRequestType.add(document, m_targetType);
}
