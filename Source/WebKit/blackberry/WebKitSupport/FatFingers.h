...
public:
    ...
    FatFingers(WebPagePrivate* webpage, const WebCore::IntPoint& contentPos, TargetType, const WebCore::IntRect& viewport = WebCore::IntRect());
    ~FatFingers();

    const FatFingersResult findBestPoint();

    static bool isElementClickable(WebCore::Element*);
