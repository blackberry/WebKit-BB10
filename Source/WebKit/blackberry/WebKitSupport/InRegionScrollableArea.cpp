    ...
    , m_hasWindowVisibleRectCalculated(false)
{
    ...
    if (layerRenderer->isRenderView()) { // #document case
        ...
        if (m_layer->usesCompositedScrolling()) {
            // We don't force content to be vertically scrollable when
            // m_scrollsHorizontally is true and m_scrollsVertically is false.
            // Otherwise, force it to be vertically scrollable.
