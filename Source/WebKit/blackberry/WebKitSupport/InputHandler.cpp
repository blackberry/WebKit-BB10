...
{
    SpellingLog(Platform::LogLevelInfo, "InputHandler::requestCheckingOfString '%s'", spellCheckRequest->text().latin1().data());
    ...
    if (spellCheckRequest->sequence() <= m_minimumSpellCheckingRequestSequence) {
        SpellingLog(Platform::LogLevelWarn, "InputHandler::requestCheckingOfString rejecting stale request with sequenceId=%d. Sentinal currently at %d."
            , spellCheckRequest->sequence(), m_minimumSpellCheckingRequestSequence);
        spellCheckRequest->didCancel();
        return;
    }

    unsigned requestLength = spellCheckRequest->text().length();
    ...
    if (requestLength >= MaxSpellCheckingStringLength) {
        ...
        ASSERT(spellCheckRequest->processType() == TextCheckingProcessIncremental);
    ...
    if (!convertStringToWchar(spellCheckRequest->text(), checkingString, requestLength + 1, &paragraphLength)) {
...
WebCore::RenderObject* InputHandler::rendererForInputField()
{
    if (!isActiveTextEdit())
        return 0;
    ...
    if (HTMLInputElement* element = m_currentFocusElement->toInputElement()) {
        ...
        return m_currentFocusElement->renderer();
    }
...
{
    style.setBackgroundColor(Color(0, 0, 150)); // Dark Blue
    style.setTextColor(Color(255, 255, 255)); // White
}
