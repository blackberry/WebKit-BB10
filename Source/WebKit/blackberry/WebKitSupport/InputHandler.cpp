...
void InputHandler::focusedNodeChanged()
{
    ...
    if (node && node->isElementNode()) {
        ...
        if (DOMSupport::isTextBasedContentEditableElement(element)) {
            ...
            HTMLInputElement* inputElement = static_cast<HTMLInputElement*>(m_currentFocusElement->toInputElement());
            if (inputElement && inputElement->isTextField())
                showTextInputTypeSuggestionBox(inputElement);
            return;
        }
    }
...
void InputHandler::updateFormState()
{
    ...
    for (int i = 0; i < formElementCount; i++) {
        if (toHTMLElement(formElementList[i]) != m_currentFocusElement)
             continue;
         ...
         // Previous
         for (int j = i - 1; j >= 0; j--) {
             Element* element = const_cast<HTMLElement*>(toHTMLElement(formElementList[j]));
             if (DOMSupport::isTextBasedContentEditableElement(element)) {
...
void InputHandler::setElementFocused(Element* element)
{
    ...
    if (keyboardType == VKBTypeUrl)
        m_currentFocusElementTextEditMask |= IMF_URL_TYPE;
...
{
    style.setBackgroundColor(Color(0, 0, 255)); // Bright Blue
...
{
    style.setBackgroundColor(Color(255, 0, 0)); // Red
    style.setTextColor(Color(0, 0, 0)); // Black
}
...
bool InputHandler::setTextAttributes(int insertionPoint, spannable_string_t* spannableString)
{
    ...
    for (unsigned int i = 0; i < spannableString->spans_count; i++) {
        ...
        if (spanContainsTimedAttribute) {
            ...
            // Only add the revert/correction marker, which are mutually exclusive.
...
void InputHandler::showTextInputTypeSuggestionBox(HTMLInputElement* focusedInputElement)
{
    ...
    if ((m_suggestionDropdownBoxHandler->focusedElement()) && m_suggestionDropdownBoxHandler->focusedElement() != focusedInputElement)
        ...
        m_suggestionDropdownBoxHandler->showDropdownBox();
}
