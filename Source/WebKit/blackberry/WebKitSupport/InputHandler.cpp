...
InputHandler::InputHandler(WebPagePrivate* page)
    ...
    , m_attributeTimer(this, &InputHandler::attributeTimerFired)
    , m_currentFocusElementType(TextEdit)
    , m_currentFocusElementTextEditMask(DEFAULT_STYLE)
    , m_keyboardType(VKBTypeNotSet)
    , m_enterKeyType(VKBEnterKeyNotSet)
    ...
    case InputTypeTextArea:
        {
            ...
            if (type == InputTypeEmail || DOMSupport::elementIdOrNameIndicatesNoAutocomplete(element)) {
                if (autoCorrectState == DOMSupport::On)
                    return NO_PREDICTION;
                return NO_AUTO_TEXT | NO_PREDICTION | NO_AUTO_CORRECTION;
            }
...
static VirtualKeyboardType convertStringToKeyboardType(const AtomicString& string)
{
    ...
    DEFINE_STATIC_LOCAL(AtomicString, NumPunc, ("number-punc"));
    ...
    DEFINE_STATIC_LOCAL(AtomicString, Alphanumeric, ("alphanumeric"));
    ...
    if (equalIgnoringCase(string, Web))
        return VKBTypeWeb;
    if (equalIgnoringCase(string, NumPunc))
    ...
    if (equalIgnoringCase(string, Pin) || equalIgnoringCase(string, Hex))
        return VKBTypePin;
    if (equalIgnoringCase(string, Alphanumeric))
        return VKBTypeAlphanumeric;
    return VKBTypeNotSet;
}
...
bool InputHandler::convertStringToWchar(const WTF::String& string, wchar_t* dest, int destCapacity, int* destLength)
{
    ASSERT(dest);
...
bool InputHandler::convertStringToWcharVector(const WTF::String& string, WTF::Vector<wchar_t>& wcharString)
{
...
WTF::String InputHandler::convertSpannableStringToString(spannable_string_t* src)
{
...
{
    if (!isActiveTextEdit() || !(m_webPage->m_webSettings && !m_webPage->m_webSettings->isPrivateBrowsingEnabled()) || !DOMSupport::shouldLearnText(m_currentFocusElement.get()))
        return;
...
void InputHandler::requestSpellCheckingOptions(imf_sp_text_t& spellCheckingOptionRequest, bool shouldMoveDialog)
{
    ...
    WebCore::IntRect caretRect = caretSelection.visibleStart().absoluteCaretBounds();
    ...
    SpellingLog(Platform::LogLevelInfo,
        "InputHandler::requestSpellCheckingOptions caretRect topLeft=%s, bottomRight=%s, startTextPosition=%d, endTextPosition=%d",
        Platform::IntPoint(caretRect.minXMinYCorner()).toString().c_str(),
        Platform::IntPoint(caretRect.maxXMaxYCorner()).toString().c_str(),
        spellCheckingOptionRequest.startTextPosition,
        spellCheckingOptionRequest.endTextPosition);

    m_webPage->requestSpellCheckingOptions(spellCheckingOptionRequest, caretRect, shouldMoveDialog);
}
...
void InputHandler::setElementUnfocused(bool refocusOccuring)
{
    if (isActiveTextEdit() && DOMSupport::isElementAndDocumentAttached(m_currentFocusElement.get())) {
        ...
        if (!refocusOccuring) {
            ...
            m_webPage->m_client->hideFormControls();
        }
    ...
    m_spellingHandler->cancelRequest();
    m_isFocusChange = false;
...
static void addInputStyleMaskForKeyboardType(int64_t& inputMask, VirtualKeyboardType keyboardType)
{
    ...
    case VKBTypeEmail:
        ...
        break;
    case VKBTypeNumber:
        inputMask |= IMF_NUMERIC_FULL_TYPE;
        break;
    case VKBTypeSymbol:
        inputMask |= IMF_SYMBOL_TYPE;
        break;
    case VKBTypeNumPunc:
        inputMask |= IMF_NUMBER_AND_PUNCTUATION_TYPE;
        break;
    default:
        break;
    }
...
void InputHandler::setElementFocused(Element* element)
{
    ...
    if (isInputModeEnabled() && !m_delayKeyboardVisibilityChange && (m_previousFocusableTextElement || m_nextFocusableTextElement))
        m_webPage->m_client->showFormControls(m_previousFocusableTextElement, m_nextFocusableTextElement);
    else
    ...
    m_keyboardType = keyboardTypeAttribute(element);
    if (m_keyboardType == VKBTypeNotSet)
        m_keyboardType = convertInputTypeToVKBType(type);

    addInputStyleMaskForKeyboardType(m_currentFocusElementTextEditMask, m_keyboardType);

    m_enterKeyType = keyboardEnterKeyTypeAttribute(element);

    if (m_enterKeyType == VKBEnterKeyNotSet && type != InputTypeTextArea) {
        if (element->isFormControlElement()) {
            const HTMLFormControlElement* formElement = static_cast<const HTMLFormControlElement*>(element);
            if (formElement->form() && formElement->form()->defaultButton())
                m_enterKeyType = VKBEnterKeySubmit;
        }
    }

    FocusLog(Platform::LogLevelInfo,
        "InputHandler::setElementFocused, Type=%d, Style=%lld, Keyboard Type=%d, Enter Key=%d",
        type, m_currentFocusElementTextEditMask, m_keyboardType, m_enterKeyType);

    sendInputStateNotifications();

    m_isFocusChange = true;
}

void InputHandler::sendInputStateNotifications(bool sendFormStateNotification, bool sendOnlyIfSessionDisconnected)
{
    if (!isActiveTextEdit())
        return;

    if (sendOnlyIfSessionDisconnected && !m_sessionDisconnected)
        return;

    m_webPage->m_client->inputFocusGained(m_currentFocusElementTextEditMask, m_keyboardType, m_enterKeyType);

    // update the suggestion box
    showTextInputTypeSuggestionBox();

    if (sendFormStateNotification)
        m_sendFormStateOnNextKeyboardRequest = true;
...
void InputHandler::inputSessionDisconnected()
{
    finishComposition();
    m_sessionDisconnected = true;
}
...
void InputHandler::redrawSpellCheckDialogIfRequired(bool shouldMoveDialog)
{
    if (didSpellCheckWord()) {
        ...
        requestSpellCheckingOptions(spellCheckingOptionRequest, shouldMoveDialog);
    }
}
...
WebCore::RenderObject* InputHandler::rendererForInputField(const bool excludeContentEditable)
{
    Element* inputElement = 0;
    VisibleSelection selection = m_webPage->focusedOrMainFrame()->selection()->selection();
    if (isActiveTextEdit())
        inputElement = m_currentFocusElement.get();
    else if (Element* element = DOMSupport::selectionContainerElement(selection)) {
        if (DOMSupport::isTextInputElement(element))
    ...
    if (!inputElement->renderer())
        return 0;

    // type="search" can have a 'X', so take the inner block bounding box to not include it.
    if (HTMLInputElement* element = inputElement->toInputElement()) {
        if (element->isSearchField())
            return element->innerBlockElement()->renderer();
        return inputElement->renderer();
    }

    if (inputElement->hasTagName(HTMLNames::textareaTag) || (!excludeContentEditable && inputElement->isContentEditable()))
        return inputElement->renderer();
...
{
    if (RenderObject* renderer = rendererForInputField())
...
void InputHandler::ensureFocusTextElementVisible(CaretScrollType scrollType)
{
    ...
    case VisibleSelection::RangeSelection: {
        ...
        if (m_webPage->m_selectionHandler->lastUpdatedExtentPointIsValid() && elementFrame->selection()->selection().isBaseFirst())
...
void InputHandler::ensureFocusElementVisible(bool centerInView)
{
    if (isActivePlugin())
        ...
        ensureFocusTextElementVisible(centerInView ? CenterIfNeeded : EdgeIfNeeded);
}
...
void InputHandler::notifyClientOfKeyboardVisibilityChange(bool visible, bool triggeredByFocusChange)
{
    ...
    if (!m_delayKeyboardVisibilityChange) {
        if (m_sendFormStateOnNextKeyboardRequest) {
            if (m_previousFocusableTextElement || m_nextFocusableTextElement)
                m_webPage->m_client->showFormControls(m_previousFocusableTextElement, m_nextFocusableTextElement);
            m_sendFormStateOnNextKeyboardRequest = false;
        }
...
void InputHandler::selectionChanged()
{
    ...
    if (m_isFocusChange) {
        m_isFocusChange = false;
        m_spellingHandler->spellCheckTextBlock(m_currentFocusElement.get());
    }
...
bool InputHandler::getSelection(int* start, int* end)
{
    if (!isActiveTextEdit())
        return false;
...
bool InputHandler::isTextSelected()
{
    if (!isActiveTextEdit())
        return false;
...
bool InputHandler::isAllTextSelected()
{
    if (!isActiveTextEdit())
        return false;

    int textLength = static_cast<int>(elementText().length());
    if (!textLength)
        return false;

    return !selectionStart() && (selectionEnd() == textLength);
}
...
bool InputHandler::handleKeyboardInput(const Platform::KeyboardEvent& keyboardEvent, bool changeIsPartOfComposition)
{
    ...
    unsigned flags = keyboardEvent.flags();
    /*
    ...
    */
    ...
    if (Frame* focusedFrame = m_webPage->m_page->focusController()->focusedFrame()) {
        ...
        Platform::KeyboardEvent adjustedKeyboardEvent(keyboardEvent.character(), type, adjustedModifiers, keyboardEvent.keycode(), flags, keyboardEvent.alternateCharacter(), keyboardEvent.sequenceId(), keyboardEvent.sourceDevice());
        ...
        if (isKeyChar) {
            ...
            adjustedKeyboardEvent = Platform::KeyboardEvent(keyboardEvent.character(), type, adjustedModifiers, keyboardEvent.keycode(), flags, keyboardEvent.alternateCharacter(), keyboardEvent.sequenceId(), keyboardEvent.sourceDevice());
...
bool InputHandler::shouldNotifyWebView(const Platform::KeyboardEvent& keyboardEvent)
{
    ...
    return !(keyboardEvent.character() == KEYCODE_BACKSPACE || keyboardEvent.character() == KEYCODE_DELETE || keyboardEvent.character() == KEYCODE_RETURN || keyboardEvent.character() == KEYCODE_KP_ENTER);
}
...
{
    InputLog(LogLevelInfo, "InputHandler::cut() %s", m_webPage->m_selectionHandler->selectedText().data());
    executeTextEditCommand("Cut");
}

void InputHandler::copy()
{
    InputLog(LogLevelInfo, "InputHandler::copy() %s", m_webPage->m_selectionHandler->selectedText().data());
    executeTextEditCommand("Copy");
}

void InputHandler::paste()
{
    InputLog(LogLevelInfo, "InputHandler::paste() %s", m_currentFocusElement ? Pasteboard::generalPasteboard()->plainText(m_currentFocusElement->document()->frame()).latin1().data() : "");
...
void InputHandler::removeAttributedTextMarker(bool clearComposingRegion)
{
    if (m_attributeTimer.isActive())
        m_attributeTimer.stop();
...
bool InputHandler::deleteTextRelativeToCursor(int leftOffset, int rightOffset)
{
    ...
    if (leftOffset == 1 && !rightOffset) {
        ...
        if (!handleKeyboardInput(Platform::KeyboardEvent(KEYCODE_BACKSPACE, Platform::KeyboardEvent::KeyChar, 0), true /* changeIsPartOfComposition */))
            return false;
...
static void addAutoCorrectionStyleToAttributeTextStyle(AttributeTextStyle& style)
{
    style.setBackgroundColor(Color(0, 184, 0, 77)); // 30% #00b800 light green
}

static void addRevertCorrectionStyleToAttributeTextStyle(AttributeTextStyle& style)
{
    style.setBackgroundColor(Color(0, 168, 223, 51)); // 20% #00a8df light blue.
}
...
static AttributeTextStyle textStyleFromMask(int64_t mask)
{
    ...
    if (mask & AUTO_CORRECTION_ATTRIB)
        addAutoCorrectionStyleToAttributeTextStyle(style);

    if (mask & REVERT_CORRECTION_ATTRIB)
        addRevertCorrectionStyleToAttributeTextStyle(style);

    return style;
}

void InputHandler::attributeTimerFired(WebCore::Timer<InputHandler>*)
{
    InputLog(Platform::LogLevelInfo, "InputHandler::attributeTimerFired fired removing attributes");
    removeAttributedTextMarker(false /* clearComposingRegion */);

    // Apply cached attributes if they exist.
    InputLog(Platform::LogLevelInfo, "InputHandler::attributeTimerFired applying cached attributes count %d", m_cachedSpannableAttributes.size());
    for (unsigned i = 0; i < m_cachedSpannableAttributes.size(); i++)
        addAttributedTextMarker(m_cachedSpannableAttributes[i].start, m_cachedSpannableAttributes[i].end, textStyleFromMask(m_cachedSpannableAttributes[i].attribute));

    m_cachedSpannableAttributes.clear();
}
...
span_t* InputHandler::firstSpanInString(spannable_string_t* spannableString, uint64_t attrib)
{
...
bool InputHandler::setText(spannable_string_t* spannableString)
{
    ...
    if (requiresSpaceKeyPress)
        handleKeyboardInput(Platform::KeyboardEvent(KEYCODE_SPACE, Platform::KeyboardEvent::KeyChar, 0), true /* changeIsPartOfComposition */);
...
bool InputHandler::setTextAttributes(int insertionPoint, spannable_string_t* spannableString)
{
    ...
    bool spanContainsTimedAttribute = firstSpanInString(spannableString, AUTO_CORRECTION_ATTRIB | REVERT_CORRECTION_ATTRIB);
    m_cachedSpannableAttributes.clear();
    ...
    for (unsigned i = 0; i < spannableString->spans_count; i++) {
        ...
        if (spanContainsTimedAttribute) {
            SpannableAttributeDetails attributesToCache = {startPosition, endPosition, span->attributes_mask & ~AUTO_CORRECTION_ATTRIB & ~REVERT_CORRECTION_ATTRIB};
            m_cachedSpannableAttributes.append(attributesToCache);
            // Only add the revert/correction marker, which are mutually exclusive. All other attributes are not rendered.
            if (span->attributes_mask & AUTO_CORRECTION_ATTRIB || span->attributes_mask & REVERT_CORRECTION_ATTRIB)
                addAttributedTextMarker(startPosition, endPosition, textStyleFromMask(span->attributes_mask & AUTO_CORRECTION_ATTRIB ? AUTO_CORRECTION_ATTRIB : REVERT_CORRECTION_ATTRIB));
        } else
    ...
    if (spanContainsTimedAttribute) {
        m_attributeTimer.startOneShot(0.250);
        InputLog(Platform::LogLevelInfo, "InputHandler::setTextAttributes add correction or revert marker timer started");
    }
