...
void InspectorOverlay::notifyFlushRequired(const GraphicsLayer* layer)
{
    m_webPage->notifyFlushRequired(layer);
}

void InspectorOverlay::paintContents(const GraphicsLayer*, GraphicsContext& context, GraphicsLayerPaintingPhase, const IntRect&)
{
    context.save();
...
#if USE(ACCELERATED_COMPOSITING)
    if (!m_overlay) {
        m_overlay = GraphicsLayer::create(m_webPage->graphicsLayerFactory(), this);
        const IntSize size = m_webPage->contentsSize();
        m_overlay->setSize(FloatSize(size.width(), size.height()));
        m_webPage->overlayLayer()->platformLayer()->addOverlay(m_overlay->platformLayer());
    }
