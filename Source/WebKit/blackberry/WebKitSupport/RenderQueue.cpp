...
bool RenderQueue::isCurrentRegularRenderJob(const TileIndex& index, BackingStoreGeometry* geometry) const
{
    ...
    return m_regularRenderJobsRegion.intersects(tileRect) || m_currentRegularRenderJobsBatchRegion.intersects(tileRect);
}
...
void RenderQueue::addToQueue(JobType type, const Platform::IntRectRegion& region)
{
    if (type == RegularRender) {
        ...
        for (size_t i = 0; i < rectsInRegion.size(); ++i) {
            if (m_currentRegularRenderJobsBatchRegion.intersects(rectsInRegion[i])) {
...
{
    return m_regularRenderJobsNotRenderedRegion.intersects(rect);
}

void RenderQueue::saveAndClearScrollRenderJobs()
{
    m_savedVisibleScrollRegion = tileRegion(m_visibleScrollJobs, m_parent->frontState());
    m_visibleScrollJobs.clear();
    m_savedNonVisibleScrollRegion = tileRegion(m_nonVisibleScrollJobs, m_parent->frontState());
    m_nonVisibleScrollJobs.clear();
}

void RenderQueue::restoreSavedScrollRenderJobs()
{
    addToQueue(RenderQueue::VisibleScroll, m_savedVisibleScrollRegion);
    addToQueue(RenderQueue::NonVisibleScroll, m_savedNonVisibleScrollRegion);
}
