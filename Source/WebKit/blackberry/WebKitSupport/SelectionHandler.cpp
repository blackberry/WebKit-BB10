...
void SelectionHandler::selectAtPoint(const WebCore::IntPoint& location, SelectionExpansionType selectionExpansionType)
{
    ...
    if (!fatFingersResult.positionWasAdjusted()) {
        if (isSelectionActive())
            cancelSelection();
        m_webPage->m_client->notifySelectionDetailsChanged(Platform::IntRect(), Platform::IntRect(), Platform::IntRectRegion());
        return;
    }

    targetNode = fatFingersResult.node(FatFingersResult::ShadowContentNotAllowed);
    ...
    if (selectionExpansionType == NextParagraph) {
        Frame* focusedFrame = m_webPage->focusedOrMainFrame();
        FrameSelection* controller = focusedFrame->selection();
        ...
        endPos = endOfParagraph(endPos); // Find the end of next paragraph
        ensureSelectedTextVisible(endPos); // Focus on the end point and ensure it visible
...
bool SelectionHandler::isEndOfDocumentSelected() const
{
    Frame* focusedFrame = m_webPage->focusedOrMainFrame();
    if (!focusedFrame)
        return false;
    return isEndOfDocument(focusedFrame->selection()->end());
}

void SelectionHandler::ensureSelectedTextVisible(const VisiblePosition endPos)
{
    if (Platform::Settings::instance()->allowedScrollAdjustmentForInputFields() == Platform::Settings::ScrollAdjustmentNone)
        return;

    WebCore::IntRect endLocation = endPos.absoluteCaretBounds();
    WebCore::IntPoint scrollPosition = m_webPage->scrollPosition();
    WebCore::IntSize actualVisibleSize = m_webPage->actualVisibleSize();

    Node* anchorNode = endPos.deepEquivalent().anchorNode();
...
bool SelectionHandler::expandSelectionToGranularity(Frame* frame, VisibleSelection selection, TextGranularity granularity, bool isInputMode)
{
    ...
    ensureSelectedTextVisible(selection.visibleEnd());
