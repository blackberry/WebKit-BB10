...
SelectionHandler::SelectionHandler(WebPagePrivate* page)
    ...
    , m_caretActive(false)
    , m_lastUpdatedExtentPointIsValid(false)
...
bool SelectionHandler::shouldUpdateSelectionOrCaretForPoint(const WebCore::IntPoint& point, const WebCore::IntRect& caretRect, bool startCaret) const
{
    ...
    const VisibleSelection& currentSelection = m_webPage->focusedOrMainFrame()->selection()->selection();
...
unsigned SelectionHandler::extendSelectionToFieldBoundary(bool isStartHandle, const WebCore::IntPoint& selectionPoint, VisibleSelection& newSelection)
{
    ...
    const VisibleSelection& activeSelection = focusedFrame->selection()->selection();
    ...
    if (pointIsOutsideOfBoundingBoxInDirection(character, selectionPoint, nodeBoundingBox)) {
        VisiblePosition newVisiblePosition = directionalVisiblePositionAtExtentOfBox(focusedFrame, nodeBoundingBox, character, selectionPoint);
...
bool SelectionHandler::updateOrHandleInputSelection(VisibleSelection& newSelection, const WebCore::IntPoint& relativeBase, const WebCore::IntPoint& relativeExtent, const bool isBaseHandleGrabbed)
{
    ASSERT(m_webPage->m_inputHandler->isInputMode());
    ...
    const VisibleSelection& selection = focusedFrame->selection()->selection();

    bool isExtentFirst = !selection.isBaseFirst();
    bool shouldBaseBeEnd = (isBaseHandleGrabbed || isExtentFirst) && (isBaseHandleGrabbed != isExtentFirst);
    WebCore::IntRect currentBaseCaretRect = shouldBaseBeEnd ? selection.visibleEnd().absoluteCaretBounds() : selection.visibleStart().absoluteCaretBounds();
    WebCore::IntRect currentExtentCaretRect = shouldBaseBeEnd ? selection.visibleStart().absoluteCaretBounds() : selection.visibleEnd().absoluteCaretBounds();

    // Check if the handle movement is valid.
    if (!shouldUpdateSelectionOrCaretForPoint(relativeBase, currentBaseCaretRect, !isExtentFirst /* startCaret */)
        || !shouldUpdateSelectionOrCaretForPoint(relativeExtent, currentExtentCaretRect, isExtentFirst /* startCaret */)) {
        selectionPositionChanged(true /* forceUpdateWithoutChange */);
        return true;
    }
    ...
    bool baseIsOutsideOfField = relativeBase != DOMSupport::InvalidPoint && !nodeBoundingBox.contains(relativeBase);
    bool extentIsOutsideOfField = relativeExtent != DOMSupport::InvalidPoint && !nodeBoundingBox.contains(relativeExtent);

    if (baseIsOutsideOfField && extentIsOutsideOfField)
        return false;

    unsigned character = 0;
    if (baseIsOutsideOfField)
        character = extendSelectionToFieldBoundary(!isExtentFirst /* isStartHandle */, relativeBase, newSelection);
    else if (extentIsOutsideOfField)
        character = extendSelectionToFieldBoundary(isExtentFirst /* isStartHandle */, relativeExtent, newSelection);
    ...
    if (shouldExtendSelectionInDirection(selection, character))
...
void SelectionHandler::setSelection(WebCore::IntPoint base, WebCore::IntPoint extent, const bool isBaseHandleGrabbed, const bool limitedToBoundingBox)
{
    ...
    SelectionLog(Platform::LogLevelInfo,
        ...
        Platform::IntPoint(base).toString().c_str(),
        Platform::IntPoint(extent).toString().c_str());

    // Note that IntPoint(-1, -1) is being our sentinel so far for
    // clipped out selection starting or ending location.
    bool baseIsValid = base != DOMSupport::InvalidPoint;
    m_lastUpdatedExtentPointIsValid = extent != DOMSupport::InvalidPoint;

    if (limitedToBoundingBox && m_lastUpdatedExtentPointIsValid) {
        Node* focusedNode = focusedFrame->document()->focusedNode();
        if (focusedNode && focusedNode->renderer() && !focusedNode->renderer()->absoluteBoundingBoxRect().contains(extent))
            return;
    }
    ...
    if (m_webPage->m_inputHandler->isInputMode() && !m_webPage->m_inputHandler->isMultilineInputMode()) {
        ...
        if (!caret.isEmpty()) {
            ...
            if (baseIsValid)
                base.setY(centerOfCaretY);
            if (m_lastUpdatedExtentPointIsValid)
                extent.setY(centerOfCaretY);
        }
    }

    WebCore::IntPoint relativeBase = base;
    WebCore::IntPoint relativeExtent = extent;

    // Initialize the new start and extent of our selection at the current positions.
    bool isExtentFirst = !selection.isBaseFirst();
    bool shouldBaseBeEnd = (isBaseHandleGrabbed || isExtentFirst) && (isBaseHandleGrabbed != isExtentFirst);
    VisiblePosition newBase = shouldBaseBeEnd ? selection.visibleEnd() : selection.visibleStart();
    VisiblePosition newExtent = shouldBaseBeEnd ? selection.visibleStart() : selection.visibleEnd();
    ...
    // scrolling as well
    if (baseIsValid) {
        relativeBase = DOMSupport::convertPointToFrame(m_webPage->mainFrame(), focusedFrame, base);
        VisiblePosition basePosition = visiblePositionForPointIgnoringClipping(*focusedFrame, clipPointToVisibleContainer(base));
        if (basePosition.isNotNull())
            newBase = basePosition;
    }

    if (m_lastUpdatedExtentPointIsValid) {
        relativeExtent = DOMSupport::convertPointToFrame(m_webPage->mainFrame(), focusedFrame, extent);
        VisiblePosition extentPosition = visiblePositionForPointIgnoringClipping(*focusedFrame, clipPointToVisibleContainer(extent));
        if (extentPosition.isNotNull())
            newExtent = extentPosition;
    }

    VisibleSelection newSelection(newBase, newExtent, true /* isDirectional */);

    if (!selection.isRange())
        m_webPage->updateSelectionScrollView(newSelection.visibleEnd().deepEquivalent().anchorNode());

    if (m_webPage->m_inputHandler->isInputMode()) {
        if (updateOrHandleInputSelection(newSelection, relativeBase, relativeExtent, isBaseHandleGrabbed))
            return;
    }
...
bool SelectionHandler::ensureSelectedTextVisible(const WebCore::IntPoint& point, bool scrollIfNeeded)
{
    ...
    if (!scrollIfNeeded || Platform::Settings::instance()->allowedScrollAdjustmentForInputFields() == Platform::Settings::ScrollAdjustmentNone)
...
// param selection is an input parameter.
// param isStartRTL and isEndRTL are output parameters.
static void getDirectionOfSelection(FrameSelection* selection, bool& isStartRTL, bool& isEndRTL)
{
    isStartRTL = false;
    isEndRTL = false;
    ...
    VisiblePosition startPosition = visibleSelection.visibleStart();
    VisiblePosition endPosition = visibleSelection.visibleEnd();

    if (!(startPosition.deepEquivalent().containerNode() && endPosition.deepEquivalent().containerNode()
        && startPosition.deepEquivalent().containerNode()->renderer() && endPosition.deepEquivalent().containerNode()->renderer()))
        return;
    ...
    isStartRTL = startBox ? startBox->direction() == RTL : WebCore::directionOfEnclosingBlock(selection->extent()) == RTL;
    isEndRTL = endBox ? endBox->direction() == RTL : (startBox ? (WebCore::directionOfEnclosingBlock(selection->extent()) == RTL) : isStartRTL);
}
...
static void adjustCaretRects(WebCore::IntRect& startCaret, bool isStartCaretClippedOut, WebCore::IntRect& endCaret, bool isEndCaretClippedOut,
    const std::vector<Platform::IntRect> rectList, const WebCore::IntPoint& startReferencePoint, const WebCore::IntPoint& endReferencePoint, bool isStartRTL, bool isEndRTL)
{
    ...
    if (isStartCaretClippedOut)
        ...
        startCaret.setLocation(caretLocationForRect(startCaret, true /* isStartCaret */, isStartRTL));
    ...
    if (isEndCaretClippedOut)
        ...
        endCaret = rectList[0];
        endCaret.setLocation(caretLocationForRect(endCaret, false /* isStartCaret */, isEndRTL));
        // Reset width to 1 as we are strictly interested in caret location.
        endCaret.setWidth(1);
    }
    ...
    for (unsigned i = 1; i < rectList.size(); i++) {
        ...
        if (!isStartCaretClippedOut && comparePointsToReferencePoint(
            caretComparisonPointForRect(currentRect, true /* isStartCaret */, isStartRTL),
            caretComparisonPointForRect(startCaret, true /* isStartCaret */, isStartRTL),
            startReferencePoint, isStartRTL) > 0) {
                startCaret.setLocation(caretLocationForRect(currentRect, true /* isStartCaret */, isStartRTL));
                startCaret.setHeight(currentRect.height());
        }

        if (!isEndCaretClippedOut && comparePointsToReferencePoint(
            caretComparisonPointForRect(currentRect, false /* isStartCaret */, isEndRTL),
            caretComparisonPointForRect(endCaret, false /* isStartCaret */, isEndRTL),
            endReferencePoint, !isEndRTL) > 0) {
                endCaret.setLocation(caretLocationForRect(currentRect, false /* isStartCaret */, isEndRTL));
...
void SelectionHandler::selectionPositionChanged(bool forceUpdateWithoutChange, bool updateSelectionOverlay)
{
    SelectionLog(Platform::LogLevelInfo,
    ...
    notifyCaretPositionChangedIfNeeded(m_webPage->m_touchEventHandler->isUserTriggeredTouchPressed());
    ...
    Vector<FloatQuad> quads;
    DOMSupport::visibleTextQuads(currentSelection, quads, updateSelectionOverlay ? &selection : 0, m_webPage->overlayLayer());

    IntRectRegion unclippedRegion;
    regionForTextQuads(quads, unclippedRegion, false /* shouldClipToVisibleContent */);

    Color overlayColor = RenderTheme::defaultTheme()->activeSelectionBackgroundColor();
    if (currentSelection.start().anchorNode() && currentSelection.start().anchorNode()->renderer())
        overlayColor = currentSelection.start().anchorNode()->renderer()->selectionBackgroundColor();

    // If there is no change in selected text and the visual rects
    // have not changed then don't bother notifying anything.
    if (!forceUpdateWithoutChange && m_lastSelectionRegion.isEqual(unclippedRegion) && overlayColor == m_lastOverlayColor)
        return;

    m_lastSelectionRegion = unclippedRegion;
    m_lastOverlayColor = overlayColor;
    bool isStartRTL;
    bool isEndRTL;
    getDirectionOfSelection(frame->selection(), isStartRTL, isEndRTL);
    ...
    if (!unclippedRegion.isEmpty()) {
        ...
        WebCore::IntPoint startCaretReferencePoint = referencePoint(currentSelection.visibleStart(), unclippedRegion.extents(), framePos, true /* isStartCaret */, isStartRTL);
        WebCore::IntPoint endCaretReferencePoint = referencePoint(currentSelection.visibleEnd(), unclippedRegion.extents(), framePos, false /* isStartCaret */, isEndRTL);

        adjustCaretRects(unclippedStartCaret, false /* unclipped */, unclippedEndCaret, false /* unclipped */, unclippedRegion.rects(), startCaretReferencePoint, endCaretReferencePoint, isStartRTL, isEndRTL);
        ...
        if (!visibleSelectionRegion.isEmpty() || shouldCareAboutPossibleClippedOutSelection) {
            ...
            adjustCaretRects(startCaret, shouldClipStartCaret, endCaret, shouldClipEndCaret, visibleSelectionRegion.rects(), startCaretReferencePoint, endCaretReferencePoint, isStartRTL, isEndRTL);
        }
    }
    ...
    if (m_webPage->m_selectionOverlay && updateSelectionOverlay) {
        // Normally, the various selection regions are clipped by their parent GraphicsLayer,
        // but for input fields there is no clipping to the input field boundaries, so we need
        // to add some manual clipping.
        if (RenderObject* renderer = m_webPage->m_inputHandler->rendererForInputField(true /* excludeContentEditable */))
            DOMSupport::clipSelectionToRenderer(selection, renderer, m_webPage->overlayLayer());

        m_webPage->m_selectionOverlay->draw(selection, overlayColor);
    }

    SelectionDetails details(startCaret, endCaret, visibleSelectionRegion, m_lastSelection != currentSelection,
        requestedSelectionHandlePosition(currentSelection), isStartRTL, isEndRTL, m_webPage->m_touchEventHandler->isUserTriggeredTouchPressed() /* isTouchDriven */, currentSelection.isBaseFirst());
...
void SelectionHandler::notifyCaretPositionChangedIfNeeded(bool userTouchTriggeredOnTextField)
{
    ...
    // Don't notify during viewport change. Notifying caret position change triggers a scroll.
    if (m_webPage->backingStoreClient()->isScrollNotificationSuppressed())
        return;
...
void SelectionHandler::caretPositionChanged(bool userTouchTriggeredOnTextField)
{
    ...
    WebCore::IntRect nodeBoundingBox = m_caretActive ? m_webPage->m_inputHandler->boundingBoxForInputField() : WebCore::IntRect();
    ...
    Platform::IntPoint scrollPosition = DOMSupport::InvalidPoint;
    if (m_caretActive) {
        if (RenderObject* renderer = m_webPage->m_inputHandler->rendererForInputField()) {
            // Get the scroll position of the enclosing layer.
            // This is passed to the InteractionView so it should be relative to it's immediate
            // parent and ignore any additional parent offsets.
            if (RenderLayer* renderLayer = renderer->enclosingLayer())
                scrollPosition = WebCore::IntPoint(renderLayer->scrollOffset());
        }
    }

    SelectionLog(Platform::LogLevelInfo,
        "SelectionHandler::caretPositionChanged: %s line input, single line bounding box %s%s at scroll position %s",
        ...
        scrollPosition.toString().c_str());

    m_webPage->m_client->notifyCaretChanged(caretLocation, userTouchTriggeredOnTextField, isSingleLineInput, nodeBoundingBox, scrollPosition, m_webPage->m_inputHandler->elementText().isEmpty());
}
