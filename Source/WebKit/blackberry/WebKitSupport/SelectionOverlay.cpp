...
void SelectionOverlay::draw(const Selection& selection, const Color& color)
{
    ASSERT(BlackBerry::Platform::webKitThreadMessageClient()->isCurrentThread());

    m_selection = selection;
    m_color = Color(color.red(), color.green(), color.blue(), 127 /* always use 50% alpha since it's an overlay */);
    ...
    for (Selection::iterator it = m_selection.begin(); it != m_selection.end(); ++it, ++i) {
        ...
        if (!parent || !parent->platformLayer())
            continue;
