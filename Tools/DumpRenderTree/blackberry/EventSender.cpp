...
static JSValueRef doFatFingersCallback(JSContextRef context, JSObjectRef, JSObjectRef, size_t argumentCount, const JSValueRef arguments[], JSValueRef* exception)
{
    if (argumentCount < 3)
        return JSValueMakeBoolean(context, JSStringCreateWithUTF8CString("Wrong argument"));

    int x = static_cast<int>(JSValueToNumber(context, arguments[0], exception));
    ASSERT(!exception || !*exception);
    int y = static_cast<int>(JSValueToNumber(context, arguments[1], exception));
    ASSERT(!exception || !*exception);
    int targetType = static_cast<int>(JSValueToNumber(context, arguments[2], exception));
    ASSERT(!exception || !*exception);

    IntPoint pos(x, y);

    BlackBerry::Platform::TouchPoint touch(0/*id*/, BlackBerry::Platform::TouchPoint::TouchPressed, pos, pos, 0);

    touch.populateDocumentPosition(pos, pos);

    const char* resultNodeName = BlackBerry::WebKit::DumpRenderTree::currentInstance()->page()->doFatFingersTest(touch, targetType);
    JSStringRef resultNodeNameJSString = JSStringCreateWithUTF8CString(resultNodeName);
    return JSValueMakeString(context, resultNodeNameJSString);
}
...
static JSStaticFunction staticFunctions[] = {
    ...
    { "doFatFingers", doFatFingersCallback, kJSTypeUndefined },
    { 0, 0, 0 }
};
