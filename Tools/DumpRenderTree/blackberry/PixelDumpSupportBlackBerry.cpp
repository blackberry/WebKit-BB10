...
void readPixelsUserInterfaceThread(BlackBerry::Platform::Graphics::Buffer* buffer, const BlackBerry::Platform::IntRect& srcRect, unsigned char* pixels)
{
    BlackBerry::Platform::Graphics::PlatformGraphicsContext* context = BlackBerry::Platform::Graphics::lockBufferDrawable(buffer);
    if (!context)
        return;
...
PassRefPtr<BitmapContext> createBitmapContextFromWebView(bool /*onscreen*/, bool /*incrementalRepaint*/, bool /*sweepHorizontally*/, bool /*drawSelectionRect*/)
{
    ...
    BlackBerry::Platform::Graphics::Buffer* buffer = BlackBerry::Platform::Graphics::createBuffer(windowRect.size(), BlackBerry::Platform::Graphics::AlwaysBacked);
    Platform::ViewportAccessor* viewportAccessor = DumpRenderTree::currentInstance()->page()->webkitThreadViewportAccessor();

    if (backingStore->drawContents(buffer, windowRect, viewportAccessor->scale(), viewportAccessor->documentScrollPosition())) {
        BlackBerry::Platform::userInterfaceThreadMessageClient()->dispatchSyncMessage(
            BlackBerry::Platform::createFunctionCallMessage(&readPixelsUserInterfaceThread, buffer, windowRect, data));
    }
