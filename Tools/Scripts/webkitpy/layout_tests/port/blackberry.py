...
class BlackBerryPort(Port):
    ...
    def _path_to_webcore_library(self):
        webcore_library = self._build_path("lib", "libwebcore.so")
        webkit_library = self._build_path("lib", "libwebkit.so")
        if os.path.isfile(webcore_library):
            return webcore_library
        elif os.path.isfile(webkit_library):
            return webkit_library
        return None
...
class BlackBerryDriver(Driver):
    ...
    def cmd_line(self, pixel_tests, per_test_args):
        ...
        if self._port.get_option('wrapper'):
            cmd.append('--wrapper=%s' % self._port.get_option('wrapper'))
